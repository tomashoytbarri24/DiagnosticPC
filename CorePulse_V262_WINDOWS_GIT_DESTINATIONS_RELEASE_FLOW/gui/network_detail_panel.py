...
\
    ...
    def _apply_ookla_status(self):
        ...
        if status is None:
            if self._ookla_status_loading:
                self.lbl_speed_engine.configure(text='Motor: verificando Speedtest by Ookla…', text_color=AMBER)
            return
        if status.get('available'):
            version = _safe(status.get('version'))
            suffix = f' · {version}' if version != 'N/A' else ''
            self.lbl_speed_engine.configure(text=f'Motor oficial detectado: Speedtest by Ookla{suffix}', text_color=GREEN)
            self.btn_speed_setup.configure(text='CLI detectada')
        else:
            self.lbl_speed_engine.configure(
                text='Motor oficial no detectado · para resultados comparables con Speedtest.net instala/configura la CLI de Ookla.',
                text_color=AMBER,
            )
            self.btn_speed_setup.configure(text='Configurar Ookla')
    ...
    def _apply_speed_servers(self):
        self.btn_speed_servers.configure(text='Buscando…' if self._speed_servers_loading else 'Actualizar servidores', state='disabled' if self._speed_servers_loading else 'normal')
        ...
        self.speed_server_menu.configure(values=values)
        ...
        if not result.get('ok') and result.get('message'):
            self.lbl_speed_status.configure(text=_safe(result.get('message')), text_color=AMBER)
    ...
    def _apply_speed_test(self):
        ...
        self.lbl_speed_loaded_latency.configure(
        ...
        if provider_key == 'ookla':
            ...
            self.lbl_speed_engine.configure(text='Motor usado: Cloudflare · prueba alternativa, no Speedtest.net.', text_color=AMBER)

        self.lbl_speed_server.configure(text=self._speed_server_text(result))
        self.lbl_speed_isp.configure(text=self._speed_isp_text(result))
        self.lbl_speed_compare.configure(
        ...
        self.btn_speed_result.configure(state='normal' if self._speed_result_url else 'disabled')
