...
def _observed_metrics(result):
    ...
    metrics = [('CPU máx', _fmt(cpu_temp.get('max') if cpu_temp else None, 1, ' °C')), ('CPU GHz pico', _fmt(cpu_clock.get('max') if cpu_clock else None, 2, ' GHz')), ('GPU monitoreadas', str(gpu_count)), ('Unidades', str(storage_count))]
    return metrics
...
def _phase_statuses(result):
    ...
    return [(label, str((phases.get(key) or {}).get('status') or 'N/A'))
            for key, label in (('hardware', 'Componentes'), ('integrity', 'Integridad'),
                               ('benchmark', 'Benchmark'), ('stress', 'Estrés'))]


def _running_phase_values(state):
    """Actividad real del worker; el diagnóstico V163 sigue fases explícitas."""
...
def _complete_interpretation(result):
    ...
    ai = complete.get('ai_analysis') if isinstance(complete.get('ai_analysis'), dict) else result.get('_ai_analysis') if isinstance(result.get('_ai_analysis'), dict) else {}

    lines = []
    ai_summary = str(ai.get('executive_summary') or '').strip()
    if ai_summary:
        lines.append(f'IA: {ai_summary}')
    relevance = [x for x in (ai.get('hardware_relevance') or []) if isinstance(x, dict)]
    if relevance:
        parts = []
        for row in relevance[:5]:
            name = str(row.get('detected_hardware') or row.get('component_id') or 'Componente')
            classification = str(row.get('classification') or 'NO_EVALUABLE').replace('_', ' ')
            parts.append(f'{name}: {classification}')
        lines.append(f"Vigencia {ai.get('current_year') or ai.get('year') or 'actual'}: " + ' · '.join(parts))

    deleted_files = cleanup.get('deleted_files')
    deleted_bytes = cleanup.get('deleted_bytes')
    if deleted_files is not None and deleted_bytes is not None:
        try:
            mb = float(deleted_bytes) / (1024 * 1024)
            lines.append(f'Limpieza segura: {int(deleted_files)} archivo(s) temporales/caché · {mb:.1f} MB eliminados.')
        except Exception:
            pass

    if benchmark.get('safety_stop'):
        base = 'CorePulse detuvo una carga por seguridad. Revisa la evidencia térmica antes de repetir pruebas exigentes.'
    elif status == 'CRITICAL':
        base = 'El diagnóstico completo encontró evidencia crítica en al menos una fase. Revisa la evidencia antes de aplicar cualquier corrección.'
    elif status == 'WARNING':
        base = 'El diagnóstico completo encontró condiciones que conviene revisar. Salud, estabilidad y rendimiento se mantienen separados.'
    elif status == 'NORMAL':
        base = 'Las áreas evaluables finalizaron sin anomalías observadas. El PDF es opcional y reutiliza esta misma evidencia congelada.'
    else:
        base = 'Una o más fases no pudieron evaluarse por completo. CorePulse conserva N/A donde falta evidencia y no inventa resultados.'
    lines.append(base)
    return '\n'.join(lines)
...
class DiagnosticExperiencePanel(ctk.CTkFrame):
    ...
    def _build_body(self):
        ...
        self.details_scroll = StableScrollHost(
            shell, fg_color=SURFACE, wheel_pixels=96, scroll_hold_ms=150, backend='place'
        )
    ...
    def update_complete_progress(self, info):
        ...
        self.lbl_quality.configure(text='Componentes, integridad de Windows, benchmark y estrés se ejecutan como fases separadas del mismo diagnóstico.')
        ...
        if self._observed_title_labels:
            for label, name in zip(self._observed_title_labels, ('Componentes', 'Integridad', 'Benchmark', 'Estrés')):
                label.configure(text=name)
        phase_values = _running_phase_values(getattr(getattr(self.app, '_diagnostic_run', None), 'state', None))
        self._set_observed_values(list(zip(('Componentes', 'Integridad', 'Benchmark', 'Estrés'), phase_values)))
    ...
    def show_complete(self, result):
        ...
        self.lbl_quality.configure(text=(f'Diagnóstico Total 6.0 · {completed_phases}/{total_phases} fases · datos reales/NA · limpieza segura registrada · IA congelada · PDF opcional.' if complete and total_phases else ('Diagnóstico Total 6.0 · datos reales/NA · PDF opcional.' if complete else 'Sesión válida · datos reales certificados · PDF habilitado.')))
        cpu_stats = _session_stat(result, 'cpu', 'usage_percent')
        ram_stats = _session_stat(result, 'ram', 'usage_percent')
        self.lbl_summary_title.configure(text='PICOS OBSERVADOS DURANTE EL DIAGNÓSTICO')
        self.lbl_cpu_summary_title.configure(text='CPU pico')
        self.lbl_ram_summary_title.configure(text='RAM pico')
        if cpu_stats:
            cpu_peak = _pct(cpu_stats['max'])
            self.bar_cpu.set(cpu_peak / 100.0)
            self.lbl_cpu.configure(text=f'{cpu_peak:.1f}%')
        ...
        if ram_stats:
            ram_peak = _pct(ram_stats['max'])
            self.bar_ram.set(ram_peak / 100.0)
            self.lbl_ram.configure(text=f'{ram_peak:.1f}%')
        ...
        if complete:
            ...
            self.lbl_hint.configure(text='Revisa la evidencia, la vigencia IA y la limpieza registrada. CorePulse no ejecuta reparaciones de Windows automáticamente; el PDF sigue siendo opcional.')
        else:
            self._set_observed_values(_observed_metrics(result))
            self.lbl_interpretation.configure(text=_interpretation_text(result))
            self.lbl_hint.configure(text='Las barras muestran picos observados de esta sesión. El diagnóstico final conserva evidencia por etapa y el PDF está habilitado.')
        self.lbl_interpretation.grid_configure(columnspan=1, padx=(10, 8))
        self.set_pdf_ready()
        self.refresh_report_access()
        self.result_actions.grid()
