# CorePulse V262 — Git Destinations & Release Flow

CorePulse V262 parte de **V261** y mantiene intactos sus fixes de scroll, almacenamiento, benchmark, drivers y UI. Esta revisión se concentra en el flujo de publicación: permite elegir la rama remota desde la propia aplicación sin cambiar el checkout local y mantiene GitHub Actions/Releases como autoridad del empaquetado.

## Cambios de V262

- Selector **Enviar a** con destinos detectados desde `origin`: `Estable · main`, `Desarrollo · */corepulse-dev` y otras ramas remotas.
- Publicación directa a `main` permitida con confirmación explícita; no usa `force-push`.
- La rama local puede seguir en `maxi/corepulse-dev`: el commit para `main` se construye sobre el HEAD remoto de `origin/main` mediante índice temporal.
- Se conserva la protección estricta de `FASE 1`, `FASE 2`, `FASE 3`, `.github` y versiones anteriores.
- La UI indica si la rama representa desarrollo/estable y qué Release se espera (`V262-dev` o `V262`).
- Detección informativa de workflows locales compatibles. CorePulse **no modifica `.github`**.
- Se incluyen plantillas de Actions en `developer/github_actions_templates/` para que el propietario del repositorio pueda configurar una vez el flujo de prereleases y releases estables.

## Cambios de V261

- La insignia **Salud xx%** de cada unidad vuelve al extremo derecho real de su tarjeta. El botón contextual «Ver detalles» comparte ese mismo slot y sólo se superpone durante hover, sin reservar un hueco permanente.
- Scroll unificado mediante `StableScrollHost` en Temas, publicación Git, historial de benchmark y las páginas largas de Centro de salud, CPU, GPU, RAM, Red, Tweaks, Gaming, alertas y telemetría.
- Velocidad estándar de rueda elevada a **96 px por paso**; se eliminan los casos de 54/58 px que hacían algunas vistas sensiblemente más lentas.
- Las páginas densas usan el backend `place`, que desplaza un único contenedor y evita repintados GDI agresivos en cada paso.
- Los resultados de Benchmark conservan el backend Canvas probado en V259 para mantener el drag de la barra derecha, pero ahora usan la misma velocidad de 96 px.
- Se eliminan los `CTkScrollableFrame` restantes para que la barra derecha, la rueda y el comportamiento de drag sean consistentes en toda la aplicación.
- Benchmark V25, Driver Hub, salud NVMe, telemetría, temas, audio y publicación segura conservan su lógica funcional.

## Política de datos

CorePulse mantiene **REAL_OR_NA**: no inventa salud, sensores, drivers disponibles ni resultados de benchmark.
