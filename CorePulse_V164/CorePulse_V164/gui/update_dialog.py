...
class UpdatePanel:
    ...
    def _build_publish_view(self):
        ...
        ctk.CTkLabel(
            header,
            text=("Guarda una configuración una sola vez y publica la carpeta raíz completa del clon. "
                  "Git detecta altas, cambios y eliminaciones, respeta .gitignore y nunca hace force-push. "
                  "Las credenciales siguen en Git Credential Manager/SSH; CorePulse no guarda contraseñas ni tokens."),
        ...
        ctk.CTkButton(top, text="Nuevo", width=76, height=30, command=self.new_publish_profile).grid(row=0, column=1, padx=(8, 0))
        ctk.CTkButton(top, text="Guardar perfil", width=110, height=30, command=self.save_publish_profile).grid(row=0, column=2, padx=(8, 0))
        ...
        def field(row, label_text, variable, *, browse=None):
            ctk.CTkLabel(profile_card, text=label_text, font=(FONT, 9), text_color=_c("text_2"),
                         anchor="w").grid(row=row, column=0, sticky="w", padx=(14, 10), pady=4)
            ...
            if browse:
                ctk.CTkButton(profile_card, text=browse[0], width=browse[1], height=30,
                              command=browse[2]).grid(row=row, column=2, sticky="e", padx=(8, 14), pady=4)
            else:
                ctk.CTkFrame(profile_card, fg_color="transparent", width=8, height=1).grid(row=row, column=2, padx=(0, 14))
            return entry
        ...
        self.publish_repo_entry = field(3, "Carpeta raíz / clon", self.publish_repo_var,
                                        browse=("Elegir carpeta", 112, self.select_publish_repo))
        self.publish_remote_entry = field(4, "Repositorio GitHub / origin", self.publish_remote_var)

        ctk.CTkLabel(profile_card, text="Rama", font=(FONT, 9), text_color=_c("text_2"), anchor="w").grid(
            row=5, column=0, sticky="w", padx=(14, 10), pady=4)
    ...
    def _apply_publish_density(self):
        ...
        try:
            ...
            self.commit_text.configure(height=58 if compact else 68)
        except Exception:
            pass
    ...
    def _reload_publish_profiles(self, select_id=None):
        ...
        if profile is not None:
            ...
            self.new_publish_profile(prefill=True)

    def _populate_publish_profile(self, profile):
    ...
    def new_publish_profile(self, prefill=False):
        ...
        self._set_publish_log('El perfil guarda ruta, repositorio, rama e identidad Git. No guarda credenciales.')
        if prefill and self.publish_repo_hint:
            try:
        ...
        if ctx.get('can_publish'):
            ...
            if ctx.get('can_push'):
                ...
                lines = [
                    ...
                    f"Cambios detectados: {count} archivo(s) · nuevos {ctx.get('new_count', 0)} · modificados {ctx.get('modified_count', 0)} · eliminados {ctx.get('deleted_count', 0)}.",
                ]
                if pending:
                ...
                label = f'Publicar {count} cambio' + ('' if count == 1 else 's') if count else 'Enviar commits pendientes'
