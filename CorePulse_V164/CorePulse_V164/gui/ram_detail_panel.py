...
class RAMDetailPanel:
    ...
    def _build(self):
        ...
        hero = build_title_block(
            titles,
            eyebrow='Memoria · Inventario físico',
            title='Memoria RAM',
            subtitle='Inventario Windows/SMBIOS + uso físico real en tiempo real',
            accent=GREEN,
            badges=(('SMBIOS', CYAN), ('Uso real', GREEN), ('Slots WMI', AMBER)),
            title_size=19,
        )
    ...
    def refresh(self):
        ...
        try:
            ...
            configure_changed(self.usage_value, text=_fmt(usage, 1, '%'), text_color=GREEN if _num(usage) is not None else MUTED)
            configure_changed(self.used_value, text=_fmt(used, 2, ' GB'), text_color=GREEN if _num(used) is not None else MUTED)
            configure_changed(self.available_value, text=_fmt(available, 2, ' GB'), text_color=CYAN if _num(available) is not None else MUTED)
            configure_changed(self.total_value, text=_fmt(total, 2, ' GB'), text_color=CYAN if _num(total) is not None else MUTED)
            configure_changed(self.lbl_freshness, text=f'Actualización: {_age_text(stamp)}')

            if not self._is_scrolling():
                self._apply_identity()
                configure_changed(self._runtime_labels['usage'], text=_fmt(usage, 1, '%'))
                configure_changed(self._runtime_labels['used'], text=_fmt(used, 2, ' GB'))
                configure_changed(self._runtime_labels['available'], text=_fmt(available, 2, ' GB'))
                configure_changed(self._runtime_labels['total'], text=_fmt(total, 2, ' GB'))
                configure_changed(self._runtime_labels['snapshot'], text=_age_text(stamp), text_color=GREEN if stamp is not None else MUTED)
                configure_changed(self._runtime_labels['source'], text='psutil.virtual_memory · snapshot certificado')
        except Exception:
            pass
