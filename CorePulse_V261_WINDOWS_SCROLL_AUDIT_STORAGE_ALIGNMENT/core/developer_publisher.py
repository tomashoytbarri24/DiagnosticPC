...
V258 publica una instantánea aislada basada en el HEAD remoto actual. Así un
push no falla sólo porque otro integrante actualizó la misma rama antes: se
reconstruye el commit sobre origin/rama y se reintenta sin tocar HEAD, índice,
FASE 1/2/3, .github ni commits locales del usuario.
...
def inspect_publish_context(repo_hint: str | os.PathLike | None = None) -> dict:
    """Analiza destino y commit sin modificar el repositorio.

    Si no se proporciona ruta, intenta detectar el clon automáticamente. Si hay
    varios candidatos los devuelve para que la interfaz permita escoger.
    """
    ...
    if not branch or branch.casefold() in BLOCKED_BRANCHES:
        blockers.append("Usa una rama de desarrollo. main, master, trunk y detached HEAD están bloqueados.")
    ...
    if rel is not None:
        ...
        # Si se ejecuta una copia fuera del clon, conserva el nombre cuando ya es
        # un nombre de versión; de lo contrario usa CorePulse_V<versión>.
    ...
    # V258 usa un índice Git temporal, por lo que staging ajeno no se mezcla ni
    # se modifica. Se conserva sólo como información visible de diagnóstico.
    ...
    pending: list[str] = []
    tracking = "refs/remotes/origin/" + branch if branch else ""
    ...
    # Los commits locales pendientes NO forman parte del push V258: el commit se
    # construye con parent=origin/rama usando un índice temporal independiente.

    index = _index_path(root)
    index_hash = hashlib.sha256(index.read_bytes() if index.exists() else b"").hexdigest()

    if target.exists():
        changed = _paths(root, "diff", "--name-only", "--no-renames", "HEAD", "--", target_name)
        untracked = _paths(root, "ls-files", "--others", "--exclude-standard", "--", target_name)
        plan = sorted(set(changed + untracked))
    else:
        plan = sorted(_planned_source_files(source, target_name)) if _is_corepulse_project(source) else []
    ...
    return {
        ...
        "detection_note": discovery.get("note") or "",
    }


def publish_current_version(repo_hint, commit_message, *, progress: Callable | None = None, expected_context=None):
    """Publica sólo la carpeta ejecutada sin empujar la historia local.

    El commit se construye sobre el HEAD remoto más reciente en un índice
    temporal. De este modo FASE 1/2/3, .github, staging y commits locales quedan
    fuera de la operación. Si el remoto cambia durante el push, se vuelve a
    obtener y reconstruir una vez sobre el nuevo HEAD para evitar el clásico
    rechazo ``fetch first`` sin usar force-push.
    """
    ...
    if expected_context is not None and _context_key(ctx) != _context_key(expected_context):
        raise PublishError("El repositorio, rama, remoto o versión cambió desde el análisis. Pulsa Analizar otra vez.")
    ...
    def unchanged_destination():
        ...
        if current_branch != branch or current_remote != ctx["remote"]:
            raise PublishError("La rama o el remoto cambió durante la operación. No se hace push.")
    ...
    def fetch_remote_head() -> str:
        ...
        # Rama remota nueva: sólo se permite si fetch no falló por conectividad.
        if proc.returncode != 0:
            detail = (proc.stderr or proc.stdout or "").strip()
            raise PublishError(detail or f"No se pudo consultar origin/{branch}.")
        local = _run_git(repo, "rev-parse", "--verify", "HEAD", check=False).stdout.strip()
        if not local:
            raise PublishError("No existe una base Git para crear la publicación.")
        return local

    def prepare_snapshot(base_commit: str) -> tuple[str | None, list[str]]:
        # Si la copia se ejecuta fuera del clon, se copia únicamente la carpeta
        # de versión. Nunca se reemplaza un destino distinto ya existente.
        ...
        temp_index.unlink(missing_ok=True)  # read-tree crea un índice Git válido.
        env = {"GIT_INDEX_FILE": str(temp_index)}
        try:
            _run_git(repo, "read-tree", base_commit, env=env)
            _run_git(repo, "add", "-A", "--", target_name, env=env)
            # .github nunca pertenece a una versión publicable de CorePulse. Si
            # ya existía en remoto se conserva exactamente como estaba; si sólo
            # existe localmente queda fuera del árbol nuevo.
    ...
    note(f"Rama remota: origin/{branch}")
    note(f"Versión: {target_name}")
    note("Publicación aislada: no modifica HEAD, staging ni commits locales.")
    ...
    for attempt in range(2):
        ...
        if push.returncode == 0:
            # Actualiza sólo la referencia remota local; HEAD/rama/índice siguen
            # exactamente como estaban antes de publicar.
        ...
        # Único reintento seguro: alguien movió la rama entre fetch y push. Se
        # reconstruye sobre el nuevo HEAD remoto; nunca se usa --force.
...
__all__ = [
    "PROTECTED_REPO_DIRS", "PROTECTED_REPO_ITEMS", "BLOCKED_BRANCHES", "TARGET_FOLDER", "PUBLISH_FOLDER",
