...
usa las credenciales existentes, bloquea ramas principales y limita cada commit
a una sola carpeta CorePulse versionada.
"""
...
from core.developer_publisher import (
    PublishError, inspect_publish_context, publish_current_version,
)
...
class UpdatePanel:
    ...
    def _build_publish_view(self):
        ...
        ctk.CTkLabel(
            header,
            text="CorePulse detecta el clon, la rama y la carpeta que estás ejecutando. La publicación se construye sobre la rama remota actual e incluye únicamente esa versión.",
            font=(FONT, 10), text_color=_c("text_2"), anchor="w", justify="left", wraplength=980,
        ...
        for row, variable in enumerate((self.publish_repo_var, self.publish_remote_var, self.publish_branch_var, self.publish_remote_branch_var), 2):
            label = ctk.CTkLabel(
        ...
        ctk.CTkLabel(
            actions,
            text="Sólo la carpeta de versión analizada entra al commit. main/master/trunk permanecen bloqueadas.",
            font=(FONT, 8), text_color=_c("muted"), anchor="w",
    ...
    def _show_publish_context(self, ctx, ticket):
        ...
        self.publish_branch_var.set(f"Rama destino: {ctx.get('branch') or '—'}")
        ...
        if ctx.get("can_publish"):
            ...
            lines = [
                f"Se subirá: {target}",
                f"Rama: {branch_label}",
                f"Archivos detectados: {ctx.get('planned_count', 0)}",
                ...
                "La publicación no modifica tu HEAD, staging ni commits locales.",
            ]
            ...
            self.do_publish_button.configure(text=f"Subir {target} a {branch_label}", state="normal")
        else:
    ...
    def _finish_publish(self, result):
        ...
        self.publish_status_var.set(f"SUBIDO CORRECTAMENTE · {target} → {branch}")
        self._set_publish_log(
            ...
            "Tu HEAD, staging y commits locales no fueron modificados.",
            append=True,
        )
        cp_info(
            self.app,
            ...
            "Tu rama local y su staging permanecieron intactos.\n\n"
