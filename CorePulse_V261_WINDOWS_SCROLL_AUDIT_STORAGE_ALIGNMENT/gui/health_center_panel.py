...
class HealthCenterPanel:
    ...
    def _render_analyzer(self, title, data, kind):
        ...
        if kind == 'startup':
            ...
            summary_items = (
                ...
                ('PROBLEMAS', str(data.get('device_problems', 0)), RED if data.get('device_problems') else GREEN),
            ...
            ctk.CTkLabel(
                left,
                text=f'{len(relevant_local)} controladores relevantes o con incidencias · los dispositivos genéricos de Windows quedan ocultos',
                font=(FONT, 8), text_color=MUTED, anchor='w', justify='left'
            ).pack(fill='x', pady=(1, 0))
            self._button(
            ...
            if self._show_driver_inventory:
                ...
                self._render_compact_table(
                    ...
                    columns=[
                        ...
                        {'title': 'Estado', 'weight': 4, 'wrap': 180},
                    ],
                    rows=rows,
    ...
    def _render_driver_update_center(self, parent):
        ...
        ctk.CTkLabel(
            title, text='DRIVER HUB', font=(FONT, 10, 'bold'),
        ...
        scan = self._button(
            head, 'Buscando…' if scanning else 'Buscar drivers',
        ...
        if result is None:
            ctk.CTkLabel(
                box,
                text='Pulsa “Buscar drivers”. CorePulse revisará sólo hardware importante (GPU, red, audio, almacenamiento, chipset, Bluetooth, USB y componentes asociados).',
                font=(FONT, 9), text_color=MUTED, anchor='w', justify='left', wraplength=1040,
        ...
        ctk.CTkLabel(
            ...
            text=(f'{count} actualización' + ('es' if count != 1 else '') + f' encontrada' + ('s' if count != 1 else '') + f' · {checked} dispositivos principales revisados'
                  if count else f'Sin actualizaciones confirmadas · {checked} dispositivos principales revisados'),
