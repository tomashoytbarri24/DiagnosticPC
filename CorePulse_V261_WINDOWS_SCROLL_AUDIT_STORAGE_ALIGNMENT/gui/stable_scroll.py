...
class StableScrollHost(ctk.CTkFrame):
    ...
    def __init__(
        ...
        *,
        ...
        scrollbar_track_color=DEFAULT_SCROLLBAR_TRACK,
        scroll_hold_ms=150,
        wheel_pixels=96,
    ...
    def _on_scrollbar_drag_end(self):
        ...
        # Canvas conserva el borrado fuerte anti-ghosting. El backend ``place``
        # no lo necesita y evitarlo reduce de forma importante el retardo al
        # soltar la barra en páginas con muchas tarjetas.
        self._flush_repaint(strong=bool(sys.platform == 'win32' and self._backend == 'canvas'))
    ...
    def set_palette(self, *, fg_color=None, scrollbar_button_color=None, scrollbar_button_hover_color=None, scrollbar_track_color=None):
        """Actualiza colores sin reconstruir el viewport ni perder el scroll."""
        if self._destroyed:
            return
        if fg_color is not None:
            try:
                ...
                pass
            if isinstance(fg_color, str) and fg_color != 'transparent':
                self._bg = fg_color
                try:
                    self.viewport.configure(bg=fg_color)
                except Exception:
                    pass
                try:
                    self.content.configure(fg_color=fg_color)
                except Exception:
                    pass
                try:
                    self.scrollbar.configure(bg=fg_color)
                except Exception:
                    pass
        if scrollbar_button_color is not None:
            self.scrollbar._thumb_color = scrollbar_button_color
        if scrollbar_button_hover_color is not None:
            self.scrollbar._thumb_hover_color = scrollbar_button_hover_color
        if scrollbar_track_color is not None:
            self.scrollbar._track_color = scrollbar_track_color
        try:
    ...
    def _after_direct_scroll(self, *, immediate=False):
        ...
        # El backend ``place`` mueve un único frame normal y no necesita el
        # borrado GDI agresivo usado por Canvas. Evitarlo hace que rueda y drag
        # respondan de forma inmediata incluso en páginas densas.
        strong = bool(sys.platform == 'win32' and self._backend == 'canvas')
        if immediate:
            self._flush_repaint(strong=strong)
        else:
            self._schedule_repaint(strong=strong)
