...
def stage_internal_page(app, page_key, host, panel=None):
    """Pre-mapea una página detrás de la vista actual antes del commit final.

    Esta fase permite que Tk/Windows resuelva geometría y recursos visuales sin
    retirar todavía la página que el usuario está viendo. El siguiente frame sólo
    necesita elevar el host ya mapeado, evitando frames negros o vacíos.
    """
    page_key = str(page_key or '').strip().lower()
    pending = _pending(app)
    ...
    try:
        host.place_configure(x=0, y=0, relx=0, rely=0, relwidth=1, relheight=1)
        # Mantener la nueva página debajo de los widgets actuales mientras se
        # resuelve su primer layout/pintado.
        host.lower()
...
def commit_internal_page(app, page_key, host, panel=None):
    ...
    if not pending.get('staged'):
        try:
            polish_widget_tree(host)
        except Exception:
            pass
    try:
        # Si la página fue pre-mapeada, aquí sólo se eleva el host ya resuelto.
        # Si no, se conserva la ruta atómica original para el resto de vistas.
        host.place_configure(x=0, y=0, relx=0, rely=0, relwidth=1, relheight=1)
        host.update_idletasks()
        host.lift()
    except Exception:
