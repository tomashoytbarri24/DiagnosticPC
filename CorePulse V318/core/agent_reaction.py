...
def agent_display_state(state, live_health=None, *, alive=True):
    ...
    if instant_level in {'CRITICAL', 'ERROR'}:
        # Una sola muestra crítica es evidencia instantánea, no una alerta
        # sostenida. V190 evita comunicarla en rojo como si el gatillo ya se
        # hubiera confirmado; la seguridad real sigue siendo autoridad aparte.
        detail = instant_reason or 'Condición térmica alta instantánea.'
        return {'status': 'OBSERVANDO', 'tone': 'AMBER', 'prefix': 'CONDICIÓN', 'label': 'ATENCIÓN INSTANTÁNEA', 'detail': f'{detail} · confirmando persistencia', 'border_level': 'WARNING'}
