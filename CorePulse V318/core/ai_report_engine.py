...
def _diagnostic_digest(diagnostic: Dict[str, Any]) -> Dict[str, Any]:
    ...
    complete = diagnostic.get('complete_diagnostic') if isinstance(diagnostic.get('complete_diagnostic'), dict) else {}

    benchmark_digest = {}
    benchmark = complete.get('benchmark') if isinstance(complete.get('benchmark'), dict) else {}
    for key in ('cpu', 'gpu', 'ram', 'ssd'):
        block = benchmark.get(key) if isinstance(benchmark.get(key), dict) else {}
        benchmark_digest[key] = {k: block.get(k) for k in (
            'status', 'value', 'unit', 'provider', 'duration_s', 'single_thread_ops_s',
            'multi_thread_ops_s', 'threads', 'throughput_mbps', 'transferred_mb',
            'buffer_mb', 'rounds', 'write_mbps', 'read_mbps', 'renderer', 'frames_per_s',
            'fps_1pct_low', 'frametime_avg_ms', 'frametime_p95_ms', 'frametime_p99_ms',
            'gpu_frame_time_avg_ms', 'gpu_frame_time_p95_ms', 'gpu_frame_time_p99_ms',
            'gpu_timestamp_samples', 'gpu_timestamp_coverage'
        ) if k in block}
    stress = complete.get('stress') if isinstance(complete.get('stress'), dict) else {}
    stress_digest = {'status': stress.get('status'), 'safety_stop': stress.get('safety_stop'), 'components': {}}
    for key, block in ((stress.get('components') or {}).items() if isinstance(stress.get('components'), dict) else []):
        if isinstance(block, dict):
            stress_digest['components'][str(key)] = {k: block.get(k) for k in ('status', 'duration_s', 'provider', 'reason') if k in block}
    cleanup = complete.get('cleanup') if isinstance(complete.get('cleanup'), dict) else {}
    cleanup_digest = {k: cleanup.get(k) for k in ('status', 'deleted_files', 'deleted_bytes', 'free_space_delta_bytes', 'summary', 'policy') if k in cleanup}
    fans = complete.get('fan_test') if isinstance(complete.get('fan_test'), dict) else {}
    fan_digest = {k: fans.get(k) for k in ('status', 'fan_rpm_min', 'fan_rpm_max', 'fan_rpm_delta', 'fan_control_min_percent', 'fan_control_max_percent', 'response_observed', 'interpretation', 'reason') if k in fans}
    audio = complete.get('audio_test') if isinstance(complete.get('audio_test'), dict) else {}
    audio_digest = {k: audio.get(k) for k in ('status', 'user_action_required', 'technical_playback_executed', 'acoustic_confirmation', 'policy') if k in audio}
    integrity = complete.get('integrity') if isinstance(complete.get('integrity'), dict) else {}
    dism = integrity.get('dism_sfc') if isinstance(integrity.get('dism_sfc'), dict) else {}
    fs = integrity.get('filesystem') if isinstance(integrity.get('filesystem'), dict) else {}
    integrity_digest = {
        'dism_sfc_state': dism.get('overall_state'), 'dism_sfc_summary': dism.get('summary'),
        'filesystem_status': fs.get('status'), 'filesystem_drive': fs.get('drive'),
        'repair_actions_executed': integrity.get('repair_actions_executed'),
    }
    return {
        'overall_status': _text(diagnostic.get('overall_status'), 'NO_EVALUABLE').upper(),
        'session_valid': bool(diagnostic.get('session_valid')),
        'context': _text(adaptive.get('context'), 'N/A'),
        'findings': findings,
        'statistics': _stats_digest(diagnostic),
        'benchmark': benchmark_digest,
        'stress': stress_digest,
        'fan_test': fan_digest,
        'audio_test': audio_digest,
        'cleanup': cleanup_digest,
        'integrity': integrity_digest,
        'policy': 'CURRENT_DIAGNOSTIC_ONLY_NUMERIC_EVIDENCE_ALLOWED',
    }
