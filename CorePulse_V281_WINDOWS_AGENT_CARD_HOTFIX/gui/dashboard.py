...
def _ensure_storage_card_decor(card, widgets=None):
    ...
    # Salud ocupa el extremo derecho real; el botón contextual se superpone
    # en el mismo slot sólo durante hover para no desplazar la insignia.
...
def _build_header(app):
    ...
    # V279: la tarjeta del agente usa la misma firma visual del Resumen:
    # superficie estructural + acento lateral, sin una cápsula blanca aislada.
    agent_accent = ctk.CTkFrame(agent, fg_color=COLORS['primary'], width=4, corner_radius=14)
    agent_accent.place(x=1, rely=0.02, relheight=0.96)
    app._header_agent_accent = agent_accent
