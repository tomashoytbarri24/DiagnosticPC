...
from core.developer_publisher import (
    PublishError, inspect_publish_context, publish_current_version, retarget_publish_context, TARGET_FOLDER,
)
...
class UpdatePanel:
    ...
    def _build(self):
        ...
        mode_text = {
            ...
            "source-portable": "Fuente portable · copia de seguridad antes de actualizar",
        ...
        self._flow_label = ctk.CTkLabel(
            controls,
            text="1 · Buscar  →  2 · Descargar y verificar SHA-256  →  3 · Instalar / probar",
        ...
        if self.mode == "source-git":
            self.backup_var.set("Checkout Git detectado: CorePulse no reemplazará archivos de tu rama. Para volver a una versión conocida usa ‘Versiones anteriores’.")
        elif self.mode == "source-portable":
            self.backup_var.set("Fuente portable: CorePulse crea una copia de seguridad automática antes de reemplazar archivos. Las versiones publicadas siguen disponibles en ‘Versiones anteriores’.")
        else:
        ...
        ctk.CTkButton(actions, text="Carpeta", width=100, height=36, fg_color="transparent", hover_color=_c("surface_2"), border_width=1, border_color=_c("border"), text_color=_c("text_2"), command=self.open_folder).grid(row=0, column=1, sticky="w", padx=(8, 0))
        ctk.CTkButton(actions, text="Versiones anteriores", width=150, height=36, fg_color="transparent", hover_color=_c("surface_2"), border_width=1, border_color=_c("border"), text_color=_c("text_2"), command=lambda: webbrowser.open(releases_web_url())).grid(row=0, column=2, sticky="w", padx=(8, 0))
    ...
    def _build_publish_view(self):
        ...
        self._publish_title_label = ctk.CTkLabel(
            header, text="Subir mi versión", font=(FONT, 23, "bold"),
            text_color=_c("text"), anchor="w",
        )
        ...
        ctk.CTkLabel(
            header,
            text="Sube sólo la versión actual al destino Git que elijas. FASE 1/2/3 y .github quedan fuera del commit.",
        ...
        ctk.CTkLabel(buttons, text="Enviar a", font=(FONT, 9, "bold"), text_color=_c("text_2")).pack(side="left", padx=(4, 6))
        self.publish_destination_choice_var = ctk.StringVar(value="Detectando ramas…")
        self.publish_destination_menu = ctk.CTkOptionMenu(
            buttons, variable=self.publish_destination_choice_var, values=["Detectando ramas…"],
            width=220, height=30, command=self._branch_choice_changed,
            fg_color=_c("surface_2"), button_color=_c("accent_2"), button_hover_color=_c("accent"),
            dropdown_fg_color=_c("surface"), dropdown_hover_color=_c("surface_2"), text_color=_c("text"),
        )
        self.publish_destination_menu.pack(side="left", padx=(0, 8))
    ...
    def _branch_label(self, branch):
        ...
        if branch.casefold() in {'main', 'master', 'trunk'}:
            return f'Estable · {branch}'
        if branch.casefold().endswith('corepulse-dev'):
            return f'Desarrollo · {branch}'
        return f'Otra rama · {branch}'

    def _configure_branch_choices(self, ctx):
        branches = list(ctx.get('remote_branches') or [])
        self._publish_branch_labels = {self._branch_label(b): b for b in branches}
        values = list(self._publish_branch_labels) or ['Sin ramas remotas']
        try:
            self.publish_destination_menu.configure(values=values, state='normal' if branches else 'disabled')
        except Exception:
            pass
        current = str(ctx.get('branch') or self.publish_branch_hint or '').strip()
        label = next((label for label, branch in self._publish_branch_labels.items() if branch == current), values[0])
        self.publish_destination_choice_var.set(label)
        if current:
            self.publish_branch_hint = current
            save_preferences(publish_branch=current)

    def _branch_choice_changed(self, value=None):
        label = str(value or self.publish_destination_choice_var.get() or '')
        branch = self._publish_branch_labels.get(label)
        if not branch:
            return
        self.publish_branch_hint = branch
        save_preferences(publish_branch=branch)
        # Cambio visual inmediato usando el contexto ya cacheado; la revisión
        # manual/publicación vuelve a validar origin antes de escribir.
        if isinstance(self.publish_context, dict) and self.publish_context.get('available'):
            try:
                ctx = retarget_publish_context(self.publish_context, branch)
                self._show_publish_context(ctx, getattr(self, '_publish_review_id', 0))
                return
            except Exception:
                pass
    ...
    def _show_publish_context(self, ctx, ticket):
        ...
        self.publish_protected_var.set('   ·   '.join(f"{name} {'✓ OK' if (ctx.get('protected') or {}).get(name) else '✕ Falta'}" for name in ('FASE 1', 'FASE 2', 'FASE 3', '.github')))
        ...
        if ctx.get('can_publish'):
            ...
            lines.append('FASE 1/2/3, .github, versiones anteriores y cambios ajenos quedan fuera del commit.')
    ...
    def publish_now(self):
        ...
        target_branch = str(reviewed_context.get('branch') or self.publish_branch_hint or '').strip()
        if str(reviewed_context.get('destination_kind') or '').lower() == 'stable':
            if not cp_ask_yes_no(
                self.app, 'Publicar versión estable',
                f'¿Subir {reviewed_context.get("publish_folder") or VERSION_LABEL} a {target_branch}?\n\n'
                'Este destino se considera estable. Sólo se incluirá la carpeta de esta versión; FASE 1/2/3 y .github no se tocarán.'
            ):
                return
        self._set_busy(True)
        self.do_publish_button.configure(state="disabled")
        ...
        def worker():
            try:
                result = publish_current_version(repo, message, target_branch=target_branch, progress=note, expected_context=reviewed_context)
    ...
    def _finish_publish(self, result):
        ...
        cp_info(
            ...
            f"FASE 1/2/3, .github y versiones anteriores no se modificaron.\n\nCommit: {result.get('commit')}"
        )
        self.publish_context = None
        self.do_publish_button.configure(state="disabled")
