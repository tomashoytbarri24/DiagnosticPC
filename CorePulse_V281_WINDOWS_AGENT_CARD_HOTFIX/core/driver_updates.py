...
def _installed_driver_candidates(limit: int = 20) -> list[dict[str, Any]]:
...
foreach($d in $drivers){
  ...
  $rows += [pscustomobject]@{
    ...
    hardware_ids=$ids; priority=$priority; is_signed=$d.IsSigned;
...
'''
    ...
    for row in rows:
        ...
        name_cf = str(row.get("device_name") or "").strip().casefold()
        ...
        if m:
            try:
                ...
                age_years = (_time.time() - int(m.group(1)) / 1000.0) / (365.25 * 86400)
            except Exception:
                age_years = None
        elif current_date:
            try:
                ...
                import time as _time
                age_years = (_time.time() - parsed.timestamp()) / (365.25 * 86400)
            except Exception:
                age_years = None
    ...
    # Prioriza primero hardware que ya tiene una señal local útil (paquete viejo
    # de terceros/no firmado), y luego la importancia del componente. Así la
    # búsqueda no gasta el cupo en dispositivos genéricos antes de GPU/red/audio.
    candidates.sort(key=lambda x: (int(x.get("attention_score") or 0), int(x.get("priority") or 0), str(x.get("device_name") or "").casefold()), reverse=True)
...
def scan_driver_updates(*, max_devices: int = 20) -> dict[str, Any]:
