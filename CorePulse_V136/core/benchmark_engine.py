...
class _SuiteReporter:
    """Puente de progreso + muestreo térmico durante la carga."""

    def __init__(self, callback: ProgressCallback = None, telemetry_sampler: TelemetrySampler = None):
    ...
    def emit(self, fraction: float, stage: str, detail: str = ''):
        _safe_progress(self.callback, fraction, stage, detail)
        now = time.monotonic()
        if callable(self.telemetry_sampler) and now - self._last_sample >= 0.45:
            self._last_sample = now
            try:
                sample = self.telemetry_sampler() or {}
            except Exception:
                sample = {}
            if isinstance(sample, dict):
                row = {
                    ...
                    'cpu_ghz': sample.get('cpu_ghz'),
                    'cpu_usage': sample.get('cpu_usage'),
                    'ram_usage': sample.get('ram_usage'),
    ...
    def summary(self) -> Dict[str, Any]:
        metrics: Dict[str, Any] = {'sample_count': len(self.samples)}
        for key in ('cpu_temp', 'cpu_ghz', 'cpu_usage', 'ram_usage', 'gpu_temp', 'gpu_usage'):
            vals = [_float(row.get(key)) for row in self.samples]
        ...
        if self.stop_reason:
            metrics['safety_stop'] = self.stop_reason
        return metrics
    ...
    try:
        ...
        if callable(stop_check) and stop_check():
            return _result(
                ...
                write_mbps=actual_mb / write_duration, read_mbps=None, size_mb=actual_mb,
...
def run_benchmark_suite(
    ...
    *,
    ...
    reporter = _SuiteReporter(progress_callback, telemetry_sampler)
    selected_labels = ', '.join(key.upper() for key in selected)
    reporter.emit(0.01, f"Preparando benchmark {profile_data['label'].lower()}", selected_labels)
    ...
    for key in selected:
        if reporter.should_stop:
            results[key] = _not_run(key.upper(), reporter.stop_reason or 'Detenido por seguridad')
            results[key]['status'] = 'SAFETY_STOP'
            continue
        ...
        if key == 'cpu':
            results[key] = benchmark_cpu(
                ...
                stop_check=lambda: reporter.should_stop,
            )
        elif key == 'ram':
    ...
    if reporter.should_stop:
        for key in selected:
            if str((results.get(key) or {}).get('status') or '').upper() == 'SKIPPED':
                results[key] = _not_run(key.upper(), reporter.stop_reason or 'Detenido por seguridad')
                results[key]['status'] = 'SAFETY_STOP'

    reporter.emit(1.0, 'Benchmark completado', 'Resultados listos para revisar')
