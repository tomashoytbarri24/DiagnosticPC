...
def select_active_gpu(gpus: Iterable[Dict[str, Any]]) -> Dict[str, Any]:
    """Selecciona la GPU representativa para la interfaz sin asumir fabricante.

    En equipos híbridos el adaptador que mueve el escritorio puede ser una iGPU,
    mientras que la GPU de alto rendimiento expone la telemetría más útil para el
    usuario (VRAM dedicada, potencia, ventilador, hotspot, etc.). Por ello la
    selección prioriza primero la *capacidad real demostrable* y luego la actividad
    medida. Si dos adaptadores tienen una capacidad equivalente, gana el de mayor
    uso real. Si ninguno aporta evidencia suficiente, se conserva el primero sólo
    como referencia visual; esto no se interpreta como una afirmación de que sea la
    GPU principal del sistema.
    """
    candidates = [gpu for gpu in (gpus or []) if isinstance(gpu, dict)]
    if not candidates:
        return {}

    def _os_vram_mb(gpu: Dict[str, Any]) -> Optional[float]:
        os_inv = gpu.get('os_inventory') if isinstance(gpu.get('os_inventory'), dict) else {}
        adapter_ram = number(os_inv.get('adapter_ram_bytes_os'))
        return None if adapter_ram is None or adapter_ram <= 0 else adapter_ram / (1024.0 * 1024.0)

    def _representative_vram_mb(gpu: Dict[str, Any]) -> Optional[float]:
        telemetry_vram = number(gpu.get('memory_total_mb'))
        if telemetry_vram is not None and telemetry_vram > 0:
            return telemetry_vram
        return _os_vram_mb(gpu)

    def _capability_signals(gpu: Dict[str, Any]) -> tuple[float, float, float]:
        vram_mb = _representative_vram_mb(gpu)
        power = number(gpu.get('power_w'))
        fan = number(gpu.get('fan_rpm'))
        hotspot = number(gpu.get('hotspot_c'))
        voltage = number(gpu.get('core_voltage_v'))
        dedicated_signals = sum(v is not None for v in (power, fan, hotspot, voltage))
        large_vram = 1.0 if vram_mb is not None and vram_mb >= 4096.0 else 0.0
        return large_vram, float(dedicated_signals), vram_mb if vram_mb is not None else -1.0
    ...
    def score(item: tuple[int, Dict[str, Any]]) -> tuple[float, ...]:
        ...
        power = number(gpu.get('power_w'))
        core_clock = number(gpu.get('core_clock_mhz'))
        memory_clock = number(gpu.get('memory_clock_mhz'))
        large_vram, dedicated_signals, vram_mb = _capability_signals(gpu)
        valid_metrics = sum(v is not None for v in (usage, memory_usage, temperature, power, core_clock, memory_clock))
        return (
            large_vram,
            dedicated_signals,
            vram_mb,
