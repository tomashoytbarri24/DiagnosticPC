"""Regresión V100: modo claro completo y sidebar balanceado."""
...
def main():
    ...
    results=[
        ...
        check('inner_surface_is_light', light_inner.lower() in {'#dde3e9','#e9edf2','#e8edf2','#f2f4f7'}),
        check('card2_surface_is_light', light_card2.lower() in {'#dde3e9','#e9edf2','#e8edf2','#f2f4f7'}),
        check('secondary_surface_is_light', light_alt.lower() in {'#dde3e9','#d9e3ec','#e8edf2','#e9edf2','#f2f4f7'}),
        check('chart_grid_is_light', grid.lower()==tm.get_theme_profiles()['snow']['border'].lower()),
        ...
        check('agent_card_compact', 'height=122' in layout and 'target_h = 112 if compact else 122 if standard else 126' in layout),
        check('agent_detail_not_clipped', "height=16 if compact else 18" in layout and "anchor='nw'" in layout),
