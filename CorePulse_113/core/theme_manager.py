...
10 temas. La preferencia se persiste y se aplica al reiniciar únicamente la
...
def _fallback_structural_role(value):
    """Clasifica sólo colores estructurales azules/neutros no catalogados.

    La clasificación se realiza sobre el color ORIGINAL y devuelve un rol, no
    un color mezclado. Los colores semánticos (rojo, verde, ámbar, violeta) se
    dejan intactos para conservar significado de estado.
    """
    try:
    ...
    if l <= 0.34:
        return 'border'
    if is_blue_family and l <= 0.58:
        return 'accent_2'
    return None


def get_theme_profiles():
    """Devuelve una copia liviana de las 10 paletas disponibles."""
