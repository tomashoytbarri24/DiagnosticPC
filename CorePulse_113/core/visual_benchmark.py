...
VISUAL_PROFILES = {
    'quick': {
        ...
        'texture_count': 2,
        'texture_passes': 10,
        'combined_texture_passes': 5,
        'shader_passes': 3,
        'combined_shader_passes': 1,
        'compute_buffer_mb': 4,
        'compute_passes': 1,
    ...
    'standard': {
        ...
        'texture_count': 4,
        'texture_passes': 18,
        'combined_texture_passes': 8,
        'shader_passes': 6,
        'combined_shader_passes': 2,
        'compute_buffer_mb': 8,
        'compute_passes': 1,
        'combined_compute_passes': 1,
...
def _run_windows_visual(profile_data, progress_callback: ProgressCallback, telemetry_sampler: TelemetrySampler):
...
'''
        ...
        phase_specs = tuple(
            ...
            if (item[0] != 'shaders' or shader_program is not None)
        ...
        phase_results = []
        for key, label in canonical_phase_specs:
        ...
        if status in ('CANCELLED', 'ERROR', 'SAFETY_STOP') and reason:
            ...
            final_detail = f'Carga combinada: {fps:.1f} FPS promedio · {measured_phase_count}/{len(canonical_phase_specs)} fases medidas'
...
def run_visual_benchmark(
    ...
    *,
    ...
    try:
        return _run_windows_visual(profile_data, progress_callback, telemetry_sampler)
    except Exception as exc:
        return {
