...
echo ================================================================
echo       COREPULSE V0.10.2.96w - BUILD EXE + SELF-TEST
echo ================================================================
echo.
...
powershell -NoProfile -ExecutionPolicy Bypass -Command "$j=Get-Content -Raw -LiteralPath '%PROBE%' | ConvertFrom-Json; if(-not $j.ok -or $j.version -ne '0.10.2.96w'){exit 1}"
if errorlevel 1 goto :FAIL_HELPER
...
:FAIL_RUNTIME
echo [ERROR] El .venv de release no usa Python 3.12 x64. Ejecuta instalar_dependencias.bat.
goto :ENDFAIL
:FAIL_BUILD
