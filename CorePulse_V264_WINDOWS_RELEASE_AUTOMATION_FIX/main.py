...
class App(ctk.CTk):
    ...
    def _schedule_storage_health_refresh(self):
        """Publica SMART/NVMe rápido y completa fallbacks sin bloquear la UI."""
        self._storage_health_refresh_after_id = None
        if not getattr(self, 'is_running', False):
            return
        ...
        if not isinstance(devices, list) or not devices:
            # El primer snapshot todavía no llegó: reintentar pronto, no en 5 minutos.
            try:
        ...
        def publish(result):
            ...
            for key, value in result.items():
                ...
                # Un refresco rápido no debe borrar uso/mounts reales ya cacheados.
                keep_previous_health = value.get('health') is None and previous.get('health') is not None
                health_fields = {'health', 'health_label', 'health_source', 'health_derived', 'wear_percent', 'wear_source', 'wear_quantitative_reliable'}
