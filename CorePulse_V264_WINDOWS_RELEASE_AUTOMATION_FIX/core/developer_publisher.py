...
V264 conserva la selección explícita de la rama remota de destino (desarrollo, main
...
def _workflow_support(repo: Path, branch: str) -> dict:
    ...
    if matched:
        matched_bodies = [(name, body) for name, body in texts if name in matched]
        legacy = [name for name, body in matched_bodies if "corepulse workflow schema: v2-long-folder" not in body]
        if legacy:
            return {
                "workflow_detected": False,
                ...
                "workflow_note": (
                    "Action detectada, pero usa el esquema antiguo de Releases y puede ignorar nombres "
                    "como CorePulse_V264_WINDOWS_.... Instala las plantillas V264 en .github/workflows."
                ),
            }
        return {
            ...
            "workflow_note": f"Action compatible V2 detectada: {', '.join(matched[:3])}",
        }
    return {
        "workflow_detected": False,
...
def retarget_publish_context(context: dict, target_branch: str) -> dict:
    """Reutiliza un análisis ya hecho al cambiar de rama en la UI.

    No consulta la red: ramas, repo, archivos planeados y protecciones ya fueron
    calculados por ``inspect_publish_context``. La publicación final vuelve a
    validar todo contra origin antes de crear/pushear el commit.
    """
    ctx = dict(context or {})
    branch = str(target_branch or '').strip()
    branches = [str(x).strip() for x in (ctx.get('remote_branches') or []) if str(x).strip()]
    blockers = [str(x) for x in (ctx.get('blockers') or []) if 'rama' not in str(x).casefold() or 'remota' not in str(x).casefold()]
    if not branch or branch not in branches:
        blockers.append(f"La rama remota '{branch or 'seleccionada'}' no está en la lista cacheada. Pulsa Analizar cambios para refrescar origin.")
    ctx['branch'] = branch
    profile = classify_destination_branch(branch)
    ctx['destination_kind'] = profile['kind']
    ctx['destination_label'] = profile['label']
    ctx['release_label'] = profile['release_label']
    ctx['release_kind'] = profile['release_kind']
    ctx['remote_branch_url'] = remote_branch_url(ctx.get('remote') or '', branch)
    root_text = str(ctx.get('repo_root') or '').strip()
    if root_text and branch:
        root = Path(root_text)
        workflow = _workflow_support(root, branch)
        ctx.update(workflow)
        local_branch = str(ctx.get('local_branch') or '').strip()
        pending = []
        ...
        ctx['pending_commits'] = pending
    ctx['blockers'] = blockers
    ctx['can_publish'] = bool(ctx.get('available', True)) and not blockers
    ctx['retargeted_from_cache'] = True
    return ctx
...
__all__ = [
    ...
    "discover_remote_branches", "classify_destination_branch", "retarget_publish_context",
