...
def test_v264_scroll_contracts_and_storage_alignment():
...
def test_v264_publish_ui_and_action_templates():
...
def test_v264_remote_branch_publish_preserves_local_checkout(tmp_path, monkeypatch):
    import subprocess
    ...
    source = tmp_path / 'CorePulse_V264_WINDOWS_DRIVER_CLARITY_FAST_GIT_TARGETS'
    (source / 'core').mkdir(parents=True)
    (source / 'main.py').write_text('print("V264")\n', encoding='utf-8')
    (source / 'core' / 'version.py').write_text('VERSION = "264"\n', encoding='utf-8')
    ...
    assert ctx['release_label'] == 'V264'

    result = pub.publish_current_version(
        repo, 'V264 test publish', target_branch='main', expected_context=ctx
    )
    ...
    # Main remoto contiene V264, pero conserva FASE/.github/versiones anteriores.
    git(repo, 'fetch', 'origin', 'main')
    tree = git(repo, 'ls-tree', '-r', '--name-only', 'origin/main').stdout.splitlines()
    assert 'CorePulse_V264_WINDOWS_DRIVER_CLARITY_FAST_GIT_TARGETS/main.py' in tree
    ...
    assert 'CorePulse_V264_WINDOWS_DRIVER_CLARITY_FAST_GIT_TARGETS/main.py' not in dev_tree



def test_v264_driver_counts_and_old_filter(monkeypatch):
...
def test_v264_retarget_is_local_and_instant(tmp_path, monkeypatch):
    import core.developer_publisher as pub
    repo = tmp_path / 'DiagnosticPC'
    (repo / '.github' / 'workflows').mkdir(parents=True)
    ...
    ctx = {
        ...
        'publish_folder': 'CorePulse_V264_WINDOWS_DRIVER_CLARITY_FAST_GIT_TARGETS',
    ...
    assert new['release_label'] == 'V264'
    assert new['workflow_detected'] is True
    assert not any(args and args[0] == 'ls-remote' for args in calls)


def test_v264_legacy_workflow_is_not_claimed_as_compatible(tmp_path):
    import core.developer_publisher as pub
    repo = tmp_path / 'DiagnosticPC'
...
def test_v264_long_folder_workflow_contract():
    import re
    ...
    assert rx.match('CorePulse_V264_WINDOWS_RELEASE_AUTOMATION_FIX').group(1) == '264'
