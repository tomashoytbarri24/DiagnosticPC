...
def _run_windows_visual(profile_data, progress_callback: ProgressCallback, telemetry_sampler: TelemetrySampler, components=None, cancel_check: CancelCheck = None, visible: bool = True):
    ...
    try:
        ...
        create_style = window_style | (WS_VISIBLE if visible else 0)
        hwnd = user32.CreateWindowExW(
            0, class_name, f'CorePulse GPU Technical Benchmark — {profile_label}',
            create_style,
            pos_x, pos_y, outer_w, outer_h, None, None, hinstance, None,
        )
        if not hwnd:
            raise RuntimeError('No se pudo crear la superficie OpenGL del benchmark')
        if visible:
...
def run_visual_benchmark(
    ...
    *,
    ...
    """Ejecuta el benchmark GPU por áreas. Puede operar visible u oculto.
    ...
    try:
        return _run_windows_visual(profile_data, progress_callback, telemetry_sampler, components=components, cancel_check=cancel_check, visible=visible)
    except Exception as exc:
        return {
