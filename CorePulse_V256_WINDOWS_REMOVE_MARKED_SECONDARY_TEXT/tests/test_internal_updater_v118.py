...
class InternalUpdaterV118Tests(unittest.TestCase):
    ...
    def test_version_stage(self):
        self.assertEqual(VERSION, "118")
        self.assertEqual(STAGE, "INTERNAL_RELEASE_UPDATER")
    ...
    def test_check_release_and_asset_selection(self):
        ...
        payload = [{
            "tag_name": "V119", "name": "CorePulse V119", "body": "Notas",
            ...
            "assets": [
                {"name": "CorePulse_119.zip", "size": 3, "browser_download_url": "https://example/zip", "url": "https://api.example/zip", "digest": f"sha256:{digest}", "content_type": "application/zip"},
                {"name": "CorePulse_Setup_V119.exe", "size": 10, "browser_download_url": "https://example/exe", "url": "https://api.example/exe", "digest": "sha256:" + "0" * 64, "content_type": "application/octet-stream"},
            ],
        }]
        ...
        self.assertEqual(um.choose_asset(release, frozen=False).name, "CorePulse_119.zip")
        self.assertEqual(um.choose_asset(release, frozen=True).name, "CorePulse_Setup_V119.exe")
    ...
    def test_source_tree_integration_markers(self):
        ...
        self.assertIn("Pruebas internas", dialog)
        self.assertIn("Modo fuente", dialog)
