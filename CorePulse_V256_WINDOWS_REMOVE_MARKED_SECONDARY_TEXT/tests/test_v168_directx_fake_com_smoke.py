...
def test_v168_render_frame_calls_valid_context_slots_without_accessing_old_v167_slots():
    calls = []
    U = dx.UINT
    F = dx.FLOAT
    VP = ctypes.c_void_p
    ...
    r.context=context; r.swap=swap
    r.rtv=VP(1); r.dsv=VP(2); r.raster=VP(3); r.layout=VP(4)
    r.vs=VP(5); r.ps=VP(6); r.cbuffer=VP(7); r.noise_srv=VP(8); r.sampler=VP(9); r.blend=VP(10)
    r.width=1280; r.height=720; r.visible=False; r._shown=False; r.hwnd=None; r.cancelled=False
    r.pump=lambda: None
    mesh=dx._Mesh(VP(11),VP(12),6)
    r.meshes={'sky':mesh,'water':mesh,'rock':mesh,'tree':mesh,'jet':mesh,'cloud':mesh}
    level=profile_levels('quick')[0]
    r.terrain_meshes={level.terrain_resolution:mesh}

    # keep references alive for the whole call
    r._test_keepalive=(keep_ctx,keep_swap)
    r.render_frame(level,0.25,0)

    required={'ClearRenderTargetView','ClearDepthStencilView','OMSetRenderTargets','RSSetViewports',
              'IASetInputLayout','VSSetShader','PSSetShader','VSSetConstantBuffers','PSSetConstantBuffers',
              'PSSetShaderResources','PSSetSamplers','IASetVertexBuffers','IASetIndexBuffer',
              'UpdateSubresource','DrawIndexed','DrawIndexedInstanced','Present'}
    assert required.issubset(set(calls))
