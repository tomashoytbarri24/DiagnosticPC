## V200 — Result Clarity + Aircraft Framing
- Benchmark GPU pasa a V20 por cambio de encuadre espacial de los jets.
- Jets más cerca y ligeramente más grandes; misma geometría, instancias y draw calls.
- Cada pestaña de resultados usa `StableScrollHost` propio con navegación fija.
- Se elimina el recorte de explicaciones/evidencia cuando el contenido supera la altura visible.
- Persistencia independiente: `benchmark_gpu_v20_ultimo_resultado.json`.
- Mantiene 49 s medidos, wall-clock, FPS/1% Low reales, GPU timestamps, REAL_OR_NA y seguridad térmica.

## V199 — Benchmark V19 Knowledge Clarity
- Conclusión CorePulse global al finalizar.
- Menos repetición de texto en Resumen.
- GPU reporta entrega irregular cuando el 1% Low se aleja fuertemente del promedio en alguna escena.
- Workload V19 intacto.

## V198 — Windows Mainline · Benchmark V19 · Knowledge Layer

- Mantiene byte-identical el workload GPU V19 de V197.
- Añade `benchmark_component_reading()` como interpretación determinista y auditable, basada sólo en evidencia real de la ejecución.
- Resumen: cada componente muestra `Lectura CorePulse` con estado y explicación corta.
- GPU 3D: explica FPS/1% Low y muestra el 1% Low como porcentaje del promedio para enseñar consistencia de frames.
- Sistema: CPU/RAM/SSD explican integridad, estabilidad, temperatura y alcance real de la prueba.
- SSD deja explícito que benchmark de E/S != SMART/desgaste.
- Evidencia térmica explica el criterio de parada con muestras reales, margen a TjMax y racha observada.
- No añade rankings externos, scores sintéticos ni umbrales de potencia por modelo.
- JSON vigente: `resultados/benchmark_gpu_v19_ultimo_resultado.json`.

## V197 — Windows Mainline · Benchmark V19 · UX Closure

- Mantiene byte-identical el workload GPU V19 de V196.
- Historial: 3 ejecuciones visibles inicialmente, 5 sesiones de precarga y resto en background.
- Textos técnicos traducidos a lenguaje más claro sin cambiar valores ni fórmulas.
- Acceso directo desde el estado térmico del Resumen hacia Evidencia.
- Evidencia térmica aclara el conteo de muestras por sobre umbral.
- JSON vigente: `resultados/benchmark_gpu_v19_ultimo_resultado.json`.

## V196 — Windows Mainline · Benchmark V19 · Visual/UX Finish

- Continúa exclusivamente desde V195 Windows; la preview Linux no forma parte de esta rama.
- GPU V19 por cambios visuales reales: formación tridimensional de jets, detalle de toberas, agua/costa menos periódica y variación morfológica de vegetación.
- Mantiene 7 draw calls por frame; jet 368 triángulos, árbol 328, agua 72.962.
- Historial: primera carga de 20 sesiones, precarga del resto en background y métricas por tarjetas.
- Evidencia: `Abrir carpeta` + `Copiar ruta`; ruta completa deja de dominar la pantalla.
- JSON vigente: `resultados/benchmark_gpu_v19_ultimo_resultado.json`.
- Tiempos estándar y políticas REAL_OR_NA/seguridad no cambian.

## V195 — Benchmark V18 · Professional Finish

- Corrige ghosting/corrupción visual al cambiar entre páginas del dashboard: las cuatro páginas se preconstruyen y se alternan sin reconstrucción completa.
- Elimina un bloque GPU legado duplicado que había quedado dentro del render de Resumen.
- Historial: lectura de hasta 100 sesiones en worker daemon y publicación en UI con datos precargados; 5 sesiones visibles inicialmente.
- Centraliza la identidad del benchmark en `core/benchmark_version.py`.
- GPU pasa a V18 por cambio visual material: jets en formación V y menor niebla aplicada a los jets para una silueta reconocible.
- HUD, ventana DirectX, provider, policies, presentación y JSON quedan alineados con V18.
- Resumen diferencia finalización normal de finalización con temperatura alta sin falsear el gatillo térmico.
- Resultado vigente: `resultados/benchmark_gpu_v18_ultimo_resultado.json`.

## V193 — Benchmark V16 · Clarity Finish

- Mantiene byte-identical el workload GPU V16 de V192.
- El dashboard final se construye antes de ocultar el progreso para evitar superficie vacía en la transición.
- GPU 3D usa divulgación progresiva: FPS + 1% Low primero; frametime/CV/spikes bajo demanda.
- Sistema muestra métricas principales primero; IOPS/CV/auxiliares pasan a detalle técnico opcional.
- Evidencia simplifica metodología y la deja expandible.
- Bordes y superficies del Benchmark se refuerzan con una jerarquía CorePulse más clara.
- Historial mantiene el estado de carga hasta publicar el panel construido.

## V192 — Benchmark V16 · Static Results

- Mantiene byte-identical el workload GPU V16.
- Dashboard final separado del scroll de configuración para eliminar ghosting de widgets CTk.
- Resumen sin duplicar las cuatro escenas GPU.
- Historial publicado atómicamente tras construirlo fuera de vista.
- Toolbar de filtros en segunda fila estable.
- Historial progresivo: 5 ejecuciones iniciales + 5 bajo demanda.

## V191 — Benchmark V16 · UX Refinement

- Mantiene byte-identical el workload GPU V16 de V190.
- Configuración inicial más compacta; elimina ayuda redundante para reducir scroll/ghosting en 1280x800.
- Historial renderiza primero 12 sesiones y ofrece carga progresiva de 12 en 12.
- Campo JSON readonly adopta la superficie CorePulse en vez del fondo heredado.
- Evidencia térmica se reorganiza en tarjetas CPU/GPU con estado de protección legible.
- No cambia FPS, 1% Low, escena, HLSL, geometría, wall-clock, timestamps GPU ni REAL_OR_NA.


## V190 — Benchmark V16 · CorePulse Finish

- Mantiene Benchmark GPU V16; no cambia escenas, HLSL, geometría, duraciones ni fórmulas de medición.
- La `WndProc` DirectX procesa `WM_SETCURSOR` y fuerza cursor nulo fuera de la región medida para eliminar el aro azul heredado de Windows.
- HUD y dashboard usan una escala tipográfica y de espaciado coherente con CorePulse.
- Tarjetas de resultados: acento discreto por componente, valor principal en blanco y estado en pill.
- Navegación superior renombrada a `Prueba actual / Historial` para separar navegación de acciones.
- Elimina el último borde magenta legado del bloque Estado/Resultados; Benchmark adopta azul/cian CorePulse.
- Aviso térmico instantáneo crítico se presenta como observación ámbar hasta confirmar persistencia; alertas sostenidas mantienen rojo.
- `directx_scene.py` y `benchmark_engine.py` permanecen byte-identical a V189; el HLSL V16 también conserva su hash.


## V189 — Benchmark V16 · Professional Integration

- Mantiene byte-identical el workload GPU V16 de V188.
- HUD externo nuevo, alineado con la identidad visual de CorePulse.
- FPS del HUD sólo provienen del callback real DirectX; warm-up/settle = N/A.
- Cursor DirectX oculto mediante guard de HWND/clase y restaurado al finalizar.
- Acentos del módulo Benchmark unificados de magenta a azul/cian CorePulse.
- Resultado y configuración usan títulos más claros y navegación `Resumen / GPU 3D / Sistema / Evidencia`.
- La página Benchmark dedicada omite el probe de batería y usa shell de carga indeterminado para mejorar la primera apertura.
- No cambia escena, shaders, agua, árboles, aviones, wall-clock, 49 s medidos, 1% Low, GPU timestamps, REAL_OR_NA ni seguridad térmica.


## V188 — Benchmark GPU V16 · UX/HUD Fix
- Mantiene intacto el workload DirectX 11 V16: escenas, shaders, temporización wall-clock y benchmark_engine no cambian.
- Restaura un HUD externo al swap-chain: escena en esquina superior izquierda y FPS real en esquina superior derecha; durante warm-up/settle muestra N/A en vez de inventar valores.
- Oculta el cursor únicamente durante la fase GPU visible y restaura exactamente el estado previo al salir.
- Tras finalizar, oculta por completo la selección y el bloque Ejecutar; el Resumen ofrece `Repetir prueba` y `Cambiar componentes`.
- Tarjetas de vistazo más explícitas: `GPU · Extreme`, `CPU · SHA-256`, `RAM · Copia`, `SSD · Lectura`.
- Compatibilidad de sensores pasa a ser bajo demanda dentro de Evidencia para reducir scroll y ghosting.
- Cada pestaña de resultados vuelve al inicio del viewport al abrirse.

## V187 — Benchmark GPU V16 UX Dashboard
- Benchmark GPU V16 preservado byte a byte: no cambia escenas, tiempos, shaders, FPS, telemetría ni seguridad.
- Primera entrada progresiva con feedback visible inmediato; construcción pesada diferida al siguiente frame.
- Historial lazy: deja de construirse en la primera entrada a Benchmark.
- Resultados reorganizados en Resumen / GPU / CPU · RAM · SSD / Evidencia.
- Configuración se colapsa al terminar y el viewport vuelve al resumen para lectura de un vistazo.
- Ruta JSON, sensores, metodología y evidencia térmica detallada pasan a la vista Evidencia.
- Reduce el scroll vertical largo para mitigar el ghosting todavía observado en V186.
- Versión central: V187.

## V186 — Benchmark GPU V16 Scroll Backend Fix
- Benchmark V16 sin cambios de workload ni medición.
- Página Benchmark: backend `place` dedicado para eliminar ghosting de widgets CTk durante scroll.
- El resto de StableScrollHost mantiene backend Canvas por compatibilidad.
- Restauración de scroll a través de la API del host.
- Versión central: V186.

## V185 — Benchmark GPU V16 UI Ghost Fix
- Corrige el último resto visual transitorio observado al hacer scroll en Windows.
- `StableScrollHost` conserva una solicitud de repaint fuerte hasta el próximo frame y la usa también con rueda del mouse.
- No cambia ningún archivo del workload GPU V16 ni su telemetría/seguridad.

## V184 — Benchmark GPU V16 Visual Cleanup
- Follaje menos planar sin aumentar conteo geométrico.
- Metadata GPU V16 coherente y JSON independiente.
- Alcance térmico UI aclarado.
- Result settle sin repaints tardíos que compitan con el primer scroll.

# V183 — Benchmark GPU V15 · visual polish

- Nuevo método `COREPULSE_GPU_DIRECTX11_SCENES_V15_WALLCLOCK_DETERMINISTIC`: no mezcla FPS con V13 porque cambió el workload visual.
- Agua: frecuencias no armónicas, UV/dominio rotado, domain warp, costa/espuma menos repetitiva y plano de 760 m con mezcla atmosférica en horizonte.
- Vegetación: cuatro tarjetas por cluster, silueta lobulada continua y borde dither determinista para reducir dientes sin alpha blending.
- Aviones: tomas de aire, derivas complementarias y panelado material para mejorar lectura a distancia.
- Se preservan duraciones, wall-clock, seguridad térmica V181, timestamps GPU y política REAL_OR_NA.
- Resultado independiente: `resultados/benchmark_gpu_v15_ultimo_resultado.json`.

# V181 — propagación SAFETY_STOP + settle UI V13

- Propaga el stop térmico del reporter al bucle DirectX.
- Añade evidencia exacta del gatillo CPU <=1.0 °C / 3 muestras.
- Fuerza geometría + repintado estable tras publicar resultados para evitar restos al primer scroll.
- Workload visual V13 sin cambios.

# V180 — seguridad CPU + UI atómica V13

- Protección CPU sostenida con margen real a TjMax <= 1,0 °C durante 3 muestras consecutivas.
- Publicación de resultados en staging sin destruir el Canvas completo.
- Workload DirectX V13 congelado sin cambios visuales ni de medición.

# V179 — cierre UI y auditoría térmica V13

- No cambia escenas, workload, tiempos, fórmulas FPS/1% Low ni timestamps GPU de V13.
- El cierre del benchmark deja de reconstruir la vista dos veces; elimina la transición vacía antes de resultados.
- El estado ya no puede mostrar LISTO mientras exista progreso parcial de una ejecución.
- El JSON V13 añade `telemetry.thermal_audit` con evidencia real: picos, muestras sobre referencias térmicas, tramo observado por timestamps, margen TjMax y límites GPU sólo cuando el sensor los reporta.
- Política térmica: REAL_OR_NA; conserva `0.0 °C` de margen TjMax como dato válido y usa un límite GPU sólo cuando el sensor real lo reporta; no interpola ni inventa límites.

# V178 — Benchmark GPU V13

- Agua: profundidad, espuma animada, detalle de luz en zonas someras y reflejo de cielo coherente con la fase.
- Terreno: estratos y contraste; vegetación: recortes de hojas, variación por instancia y normales corregidas.
- Avión: cola detrás de la cabina y material contrastado.
- Métodos V13 y JSON independiente; tiempos, mediciones y REAL_OR_NA conservados.
- Candidata pendiente de compilación HLSL y revisión visual en Windows.

# V177 — cierre y presentación del benchmark V12

- Estado final persistente al 100 %, callbacks tardíos descartados y ruta JSON copiable.
- Errores de guardado visibles y FPS destacados por escena.
- Motor V12 sin cambios. Ver NOTA_V177.md.

# V175 — Benchmark V12 · reloj de pared determinista

- Corrige la sensación de cámara lenta observada en V174: cámara, agua, viento, jets y duración de cada fase avanzan por `time.perf_counter()` (reloj real), no por la suma de `render + Present`.
- Valle/Bosque/Lago/Extreme conservan 10/12/12/15 s de medición de pared respectivamente.
- FPS, 1% Low, p95/p99 y frametime siguen cronometrando exclusivamente `render_frame + Present`; telemetría, query readback, message pump y UI quedan fuera de esa métrica.
- El progreso de UI se limita a 10 Hz para no introducir pausas visibles entre frames.
- Mantiene DirectX 11, geometría/materiales V11, timestamps GPU y política REAL_OR_NA.
- Cambia el identificador de metodología a V12 porque la semántica temporal sí cambió; no mezclar numéricamente historial V11 y V12.

# V174 — DirectX depth-state hotfix

- Corrige `CTX_OM_SET_DEPTH_STENCIL_STATE`: 34 -> 36.
- Elimina el `access violation` introducido por V173 al activar depth readonly para cielo/nubes.
- Agrega prueba de regresión del vtable COM.
- No cambia escenas, tiempos, workload ni métricas V11.

# V173 — Benchmark V11 · Realistic Polish + Audited Steady-State Timing

- Parte de V172 funcional; conserva Direct3D 11, 49 s efectivos medidos y timestamps GPU reales.
- Añade 8 prime frames por escena, fuera de estadísticas y fuera de queries, después del settle.
- Publica `frametime_cv` y `frametime_max_to_median_ratio` además de 1% Low/p95/p99/spikes.
- Nubes rehechas como soft-cards cruzadas con alpha procedural; dejan de usar lóbulos tipo roca.
- Nubes y sky usan depth-test sin escritura de profundidad; orden correcto: opacos → sky → transparencias.
- Agua V11: malla 192×192, cuatro ondas, choppiness tipo Gerstner, cuatro escalas de microdetalle, Fresnel y doble especular.
- Terreno V11: micro-normal procedural adicional sobre mezcla arena/pasto/roca.
- Árbol V11: canopy más irregular, más ramas y clusters pequeños; sigue siendo un único mesh instanciado.
- Corrige la UI de resultados para mostrar Valle/Bosque/Lago/Extreme en lugar de las claves antiguas Low/Medium/High.
- La tarjeta principal GPU ahora se etiqueta como `Extreme`, que es la escena primaria real.
- Nueva metodología: `COREPULSE_GPU_DIRECTX11_SCENES_V11_REALISTIC_POLISHED`.
- Política REAL_OR_NA preservada; no hay score sintético ni resultados por nombre de hardware.

# V172 — Benchmark V10 · Realistic Materials + Isolated Frametime

- Parte de la V171 funcional; no reemplaza DirectX por otro backend.
- Mantiene 49 s efectivos de medición GPU, pero agrega settle fuera de estadísticas por fase; perfil estándar nominal ~58 s.
- Message pump, `GetData` de timestamp queries, telemetría y callbacks se ejecutan fuera del frametime CPU/Present.
- La duración medida se acumula desde frames realmente renderizados/presentados; el overhead de instrumentación ya no acorta la muestra.
- Cámara/agua/animaciones usan un reloj determinista de fase durante la medición; el recorrido no depende de la velocidad del PC.
- Añade mediana de frametime, umbral de spike, cantidad de spikes y ratio de spikes por escena.
- Árbol V10: tronco/ramas + leaf cards cruzadas con alpha cutout procedural, variación determinista y viento.
- Agua V10: cuatro ondas, micro-normal de tres escalas, Fresnel, reflejo procedural del cielo, profundidad y espuma de costa.
- Terreno V10: material procedural multiescala con mezcla por altura y pendiente.
- Pool de timestamp queries ampliado a 192 slots; la cobertura sigue limitada a 0–100 %.
- Metodología GPU nueva: `COREPULSE_GPU_DIRECTX11_SCENES_V10_REALISTIC_ISOLATED`.
- Sin score sintético, lookup de modelo ni valores rellenados. REAL_OR_NA.

# V171 — Benchmark V9 · Realistic Valley + Audited Timing

- Perfil estándar GPU: 54 s totales, 49 s medidos.
- Fases: Valle 10 s, Bosque 12 s, Lago 12 s, Extreme 15 s; warm-up global 5 s fuera de medición.
- 4 frames de transición descartados por fase para aislar estadísticas.
- Corrección de cobertura GPU >100 %: las muestras parciales ya no se concatenan de nuevo al `drain`; hay guardrail y contador de overflow.
- Frametime CPU/Present medido sólo alrededor del render/present, excluyendo callbacks y telemetría.
- Sky dibujado al final del pass para evitar que la esfera escriba profundidad delante de la escena.
- Árbol procedural rehecho: tronco cónico, ramas, copa 3D de lóbulos, variación por instancia y viento leve.
- Agua rehecha: dos capas de micro-normal procedural, Fresnel, sol especular y espuma de costa basada en altura real del terreno.
- Material procedural 512×512 multicanal determinista para terreno/corteza/follaje/agua.
- Carga progresiva hasta ~4,58 M triángulos/frame en Extreme manteniendo 7 draw calls/frame.
- Nueva metodología: `COREPULSE_GPU_DIRECTX11_SCENES_V9_REALISTIC`.
- No se crea score sintético. Medidas reales o N/A.

# V170 — Benchmark V8 · estabilidad visual + GPU Timestamp D3D11

- Parte desde la V169 funcional; no vuelve al renderer OpenGL antiguo.
- Corrige el artefacto/polígono azul gigante: el sky sphere ahora es una esfera UV limpia y se centra en la cámara en cada frame.
- La dirección atmosférica del pixel shader se calcula respecto de la cámara, evitando facetas del cielo al recorrer la escena.
- Nubes rehechas como clusters 3D de cinco lóbulos, con deriva suave y alpha volumétrico.
- Jet procedural rehecho con fuselaje elíptico, ala delta, estabilizadores, quilla y canopy con material identificado de forma explícita.
- Agua con tres ondas combinadas y especular solar; mantiene carga determinista.
- Conserva timestamp queries D3D11 reales para GPU frametime; muestras inválidas/disjoint siguen como N/A.
- Nueva metodología `COREPULSE_GPU_DIRECTX11_SCENES_V8`, para no mezclar historial con V169/V7.
- Perfil estándar conserva 41,5 s totales: warm-up por escena fuera de medición + 8 s medidos por nivel.
- Se elimina de esta entrega los BAT de prueba V5/V6/V7 para evitar ejecutar accidentalmente un benchmark antiguo; usar `Probar_Benchmark_GPU_V8.bat`.


## V168 — DirectX 11 Runtime Fix
- Corrige access violation del benchmark V167 por slots incorrectos de `ID3D11DeviceContext`.
- Ventana GPU oculta hasta tener el primer frame 3D válido.
- Arranque DirectX autodiagnosticado por etapas y JSON con `failure_stage`.
- Seguridad térmica del benchmark deja de asumir CPU 96 °C como límite universal.
- Nuevo método GPU `COREPULSE_GPU_DIRECTX11_SCENES_V6`.
# V167 — Benchmark V5 DirectX 11

- Reemplaza la ruta GPU del benchmark por escenas Direct3D 11 progresivas Low / Medium / High / Extreme.
- La escena visible es el workload medido; warm-up excluido de la ventana de medición.
- VSync OFF, FPS real, 1% Low, frametime promedio/p95/p99 y workload auditable por escena.
- Sin fallback silencioso a OpenGL: fallo de DirectX => ERROR/N/A.
- El antiguo OpenGL queda únicamente como helper heredado del stress test.
- Añade `Probar_Benchmark_GPU_V5.bat` para validación física aislada en Windows.
- Benchmark CPU/RAM/SSD V4 y Diagnóstico Total 6.0 permanecen integrados.

# V166 — Benchmark V4 por áreas

- Reemplaza el Benchmark Diagnóstico V3 como metodología activa por `COREPULSE_BENCHMARK_4_AREAS`.
- CPU: SHA-256 mono/multihilo, throughput, compresión, descompresión y pipeline real de procesamiento de imagen con 3 muestras e integridad.
- CPU ALU entero puro, FP puro, SIMD y latencia de caché quedan `N/A` mientras no exista backend nativo universal; no se estiman por modelo ni por clocks.
- RAM: escritura nativa (`memset`) y copia nativa (`memmove`) con warm-up fuera del tiempo medido, 3 muestras y sentinelas de integridad.
- RAM lectura pura y latencia quedan `N/A` hasta un backend nativo específico.
- SSD: secuencial R/W + Random 4K QD1 R/W; latencia QD1 se deriva exclusivamente de IOPS medidos. Direct I/O de Windows se conserva.
- GPU: geometría, fill/blending, texture sampling, fragment shader, compute y mixed graphics. El FPS principal corresponde a la fase Mixed Graphics cuando está disponible.
- GPU buffer-copy/VRAM, ray tracing, mesh shaders y AI/matrix aparecen como `N/A` hasta contar con una prueba real; no se asignan ceros ni resultados por modelo.
- La variación alta no invalida una medición íntegra: se conserva `status=OK` y se marca `measurement_quality=VARIABLE` + `quality_warning`.
- No existe ranking/promedio de equipos ni score derivado de nombre de hardware. Política: `REAL_OR_NA_BENCHMARK_V4_AREAS_NO_REFERENCE_RANKING`.

# V165 — Benchmark Diagnóstico V3

- Reemplaza el benchmark por defecto por `COREPULSE_BENCHMARK_3_DIAGNOSTIC`.
- GPU: benchmark técnico oculto por áreas (Geometry/Vertex, Pixel Fill/Blending, Texture Sampling, Fragment Shader/ALU, Compute/GPGPU y Mixed Graphics). No abre la antigua ventana 3D durante el diagnóstico.
- CPU: 3 muestras SHA-256 + compresión/descompresión zlib con verificación de integridad.
- RAM: 3 muestras de copia real con verificación byte a byte.
- SSD: 3 muestras secuenciales; en Windows añade Random 4K QD1 con NO_BUFFERING/WRITE_THROUGH y verificación post-test.
- Se guardan mediana y CV. Las mediciones con variación excesiva se marcan `VARIABLE/PARTIAL` en vez de presentarse como estables.
- No hay rankings externos ni valores inventados: REAL_OR_NA.

# V164 — Diagnóstico Total 6.0 (2026-09-17)

- `Iniciar diagnóstico` inicia siempre una sesión nueva del Diagnóstico Total.
- Flujo único de 11 fases: telemetría inicial → hardware/batería/SSD → Windows → DISM/SFC/CHKDSK diagnóstico → benchmark → estrés → ventiladores → audio → limpieza segura → correlación → IA/vigencia.
- Benchmark y estrés permanecen separados: el benchmark mide rendimiento; el estrés comprueba estabilidad y no publica score de rendimiento.
- Limpieza automática restringida a la allowlist de temporales/cachés recreables, con revalidación antes de borrar. No borra documentos, juegos, Descargas ni registro.
- DISM/SFC/CHKDSK se ejecutan en modo de diagnóstico; no se aplican reparaciones automáticas dentro del flujo.
- Audio ejecuta reproducción técnica WASAPI cuando no existe una prueba guiada reciente. La audición física sólo se considera verificada con confirmación humana.
- La IA se ejecuta al final del diagnóstico, utiliza únicamente evidencia real disponible y evalúa vigencia en el año de ejecución. Si falta evidencia/proveedor, queda N/A/UNAVAILABLE.
- La pestaña muestra la interpretación final y la vigencia antes de generar el PDF.
- `Generar PDF` es opcional y reutiliza la evidencia y el análisis IA congelados del diagnóstico; incluye la identidad visual de CorePulse.
- Política permanente `REAL_OR_NA`: no se fabrican sensores, resultados, rankings ni métricas ausentes.

# V163 — Diagnóstico Integral 5.0

- Unifica el botón Diagnóstico en un flujo completo: telemetría → componentes → Windows/integridad → benchmark → estrés → ventiladores → audio → resultado/PDF.
- Añade DISM CheckHealth/ScanHealth + SFC VerifyOnly y CHKDSK /scan como comprobaciones no reparadoras.
- Ejecuta benchmark y estrés como fases separadas.
- Observa ventiladores sólo desde RPM/control reales; sin sensor queda N/A.
- Audio requiere confirmación humana mediante la prueba guiada existente.
- Elimina el énfasis visual en promedios de sesión y muestra picos observados en el panel diagnóstico.
- No ejecuta reparaciones automáticamente.
...
## V153 — Diagnóstico accionable por componente
...
## V176
- El resultado real final del benchmark GPU DirectX 11 V12 se guarda automáticamente en `resultados/benchmark_gpu_v12_ultimo_resultado.json`.
- La carpeta se crea al primer resultado GPU correcto y el archivo se reemplaza atómicamente en cada ejecución correcta.
- La interfaz muestra la ruta absoluta guardada. En Python se usa la raíz del proyecto; en PyInstaller, la carpeta del ejecutable.
- Se mantiene REAL_OR_NA: `null` conserva métricas no disponibles y cualquier valor no JSON/no finito se registra como `N/A`.
