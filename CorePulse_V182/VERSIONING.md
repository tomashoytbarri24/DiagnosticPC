...
## V182 — DIAGNOSTIC_LIFECYCLE_GPU_COMPLETION_STORAGE_SCOPE

- Corrige la cancelación aparente del diagnóstico al entrar en Audio: `RUNNING_AUDIO` ya forma parte del ciclo de vida válido.
- Linux OpenGL sólo interpreta `WM_DELETE_WINDOW`/DestroyNotify como cierre real; otros ClientMessage del compositor ya no cancelan la escena.
- Benchmark manual ejecuta CPU/RAM/SSD primero y deja GPU visual para el tramo final, de modo que la ventana 3D se cierra al terminar la suite y no a mitad del progreso global.
- Linux identifica sistemas de archivos no montados y explica por qué usado/libre no puede medirse sin montar el volumen; no inventa capacidad utilizada.
- Windows conserva su benchmark visual y rutas de almacenamiento; los cambios comunes de lifecycle/orden también se aplican.

## V181 — CROSS_PLATFORM_DIAGNOSTIC_AUDIO_GPU

- Evidencia sólo se presenta para WARNING/CRITICAL con datos reales.
- Tarjetas de diagnóstico alineadas por fila en Windows y Linux.
- Audio técnico automático dentro del diagnóstico en ambas plataformas.
- Linux ejecuta una ventana OpenGL/GLX visible para medir GPU.
- Windows conserva el benchmark visual multifase existente.
- GPU vuelve a estar disponible en el módulo Benchmark de Linux.

La autoridad de versión sigue siendo `core/version.py`.

## V180 — LINUX_PHYSICAL_STORAGE_AND_NVME_HEALTH
- Linux excluye zram, loop, ramdisk y dispositivos virtuales del resumen de almacenamiento.
- Las tarjetas del Resumen representan discos físicos; particiones/subvolúmenes quedan para la vista de detalles.
- Discos no montados siguen mostrando capacidad física y estado de montaje sin inventar espacio usado.
- La salud NVMe en Linux intenta primero UDisks2 NVMe SMART (D-Bus) y deriva vida restante desde Percentage Used.
- smartctl queda como fallback; Windows mantiene su regla de no convertir Wear=0 ambiguo en 100%.
- El instalador Linux incluye UDisks2/gdbus entre dependencias de sistema best-effort.

## V179 — Almacenamiento físico multiplataforma + diagnóstico compacto
- Linux enumera todos los discos físicos con `lsblk`, incluso unidades no montadas como un segundo disco de Windows en dual boot.
- El uso de espacio sólo se calcula cuando existe un sistema de archivos montado; las unidades inaccesibles conservan N/A en uso sin desaparecer.
- Resumen, detalle y salud de almacenamiento comparten el mismo inventario físico; Windows conserva LHM/Win32 y Linux usa lsblk + smartctl cuando está disponible.
- `Abrir carpeta de informes` usa el explorador nativo en Windows, `xdg-open` en Linux y `open` en macOS.
- Las tarjetas finales del diagnóstico dejan de estirarse verticalmente cuando sobra altura.

V173 continúa V172: incorpora Ajustes Linux seguros, elimina gutters visuales en Linux y renueva la experiencia de Temas sin alterar la autoridad de color.
...
## V163 — FINAL_OPTIMIZATION_POWER_PLAN_IDEMPOTENCE
- Continúa la optimización aplicada sobre V162 sin cambiar Benchmark 2.0 ni Overlay.
- Red avanzada carga Ookla/servidores sólo al abrir Conectividad y evita repintar la subvista oculta.
- Gaming y Centro de salud no refrescan páginas cacheadas mientras están ocultas.
- Test de Audio reduce escrituras repetidas y detiene/cancela su polling de forma robusta.
- Gestión de energía explicita reutilización/idempotencia: un mismo modo conserva su GUID y no crea copias repetidas.
- Regresión de 100 reinicios/aplicaciones confirma una sola copia CorePulse Game.
- Limpieza de wrappers visuales redundantes y exclusión de cachés Python del paquete final.

## V164 — GIT_PUBLICATION_PROFILES_ROOT_SCOPE
- La publicación visible migra a perfiles persistentes por usuario/equipo: nombre, raíz del clon, origin, rama e identidad Git local.
- La autenticación sigue fuera de CorePulse mediante Git Credential Manager/SSH; no se almacenan tokens ni contraseñas.
- El ámbito de publicación pasa a ser la raíz completa del clon elegido. Git detecta altas, modificaciones y eliminaciones y respeta `.gitignore`.
- FASE 1/2/3 dejan de ser excepciones del nuevo flujo: sólo se incluyen cuando realmente cambian.
- `main`, `master`, `trunk`, detached HEAD, conflictos y operaciones Git incompletas siguen bloqueados.
- No existe force-push. Antes de publicar se consulta/fetch la rama remota y sólo se permite avance seguro.
- El publicador histórico V163 se conserva como compatibilidad interna; la interfaz V164 usa `inspect_profile_publish_context` / `publish_profile_root`.

## V167 — AUTOMATIC_DEVELOPMENT_RELEASES
- Cada push a `*/corepulse-dev` crea o actualiza automáticamente la prerelease de desarrollo de la versión detectada.
- La release contiene ZIP fuente y SHA-256 y alimenta el canal Desarrollo del actualizador.

## V168 — SIDE_BY_SIDE_AUTO_RESTART_UPDATES
- Las actualizaciones fuente dejan de usar una copia de prueba aislada en AppData como destino final.
- La nueva versión se instala junto a la actual en la misma carpeta padre, por ejemplo `CorePulse_V167` → `CorePulse_V168`.
- Un helper espera el cierre de la versión actual y lanza la nueva automáticamente.
- No hay sobrescritura de la carpeta actual ni rollback manual en la interfaz.
- Destinos preexistentes ajenos al actualizador se protegen y nunca se reemplazan automáticamente.

## V169 — QUALITY_GATE_AND_VERSION_CONSISTENCY
- `core/version.py` queda como autoridad única de VERSION/STAGE para runtime y packaging.
- `pyproject.toml` obtiene la versión dinámicamente desde `core.version.VERSION`; build EXE e instalador dejan de tener fallbacks numéricos antiguos.
- Los tests script-style dejan de ejecutar `SystemExit` durante collection.
- Las assertions que dependían de la versión viva dejan de fijarse a números históricos.
- Snapshots `test_vNNN_*` anteriores a V162 quedan conservados con marca `legacy_snapshot` y fuera del Quality Gate actual.
- `quality_gate.py` / `Validar_CorePulse.bat` unifican metadatos, compileall y pytest en una sola validación reproducible.
- No se modifican los flujos funcionales de Publicar, Actualizaciones, Gaming, Benchmark, Diagnóstico, Overlay, planes de energía ni Tweaks.
## V175 — LINUX_DIAGNOSTIC_BATTERY_AND_BROWSER_FOCUS
- Diagnóstico completo en Linux deja de ejecutar/mostrar la fase Windows y excluye el benchmark GPU 3D no soportado.
- La telemetría GPU de escritorio sigue formando parte del diagnóstico Linux sin penalizar la evaluación por ausencia del benchmark 3D.
- Battery Health en Linux lee `/sys/class/power_supply` (energía/carga, salud, desgaste, ciclos, voltaje, corriente y potencia cuando el kernel los expone).
- La vista de batería usa mAh como fallback cuando el firmware no expone mWh.
- El botón Releases usa apertura integrada de escritorio y solicita foco al navegador en Linux mediante KDE/xdg-open + activación best-effort.

## V176 — LINUX_CLICK_TO_LAUNCH
- Doble clic mediante `CorePulse_Linux.desktop` o el ejecutable `CorePulse`.
- Primer inicio prepara automáticamente el entorno Linux sin exigir comandos al usuario.
- El instalador registra un acceso de menú y un puntero estable `~/.local/share/CorePulse/current`.
- El helper de actualización Linux relanza por el nuevo launcher y mantiene ese puntero en la versión activa.
