...
def _raw_meta(sensor, unit=None, derived=False, contributors=None):
    if sensor is None and (not contributors):
        return {'unit': unit, 'source': None, 'sensor': None, 'sensor_timestamp': None, 'derived_from_real': bool(derived), 'contributors': []}
    ...
    sources = [str(s.get('source') or '').strip() for s in sensors if str(s.get('source') or '').strip()]
    source = sources[0] if sources and all(item == sources[0] for item in sources) else ('multiple real sources' if sources else None)
    return {'unit': unit, 'source': source, 'sensor': _sensor_label(sensor) if sensor else 'multiple real sensors', 'sensor_timestamp': min(timestamps) if timestamps else None, 'derived_from_real': bool(derived), 'contributors': [{'sensor': _sensor_label(s), 'identifier': s.get('identifier'), 'sensor_timestamp': _sensor_timestamp(s), 'source': s.get('source')} for s in sensors]}
...
def _cert_cpu_details(sensors):
    ...
    for index, sensor in enumerate(cpu):
        ...
        sensor_rows.append({
            ...
            'source': str(sensor.get('source') or 'Native sensor provider'),
            'quality': 'VALID',
    ...
    result = {
        ...
        'source': str(cpu[0].get('source') or 'Native sensor provider') if cpu else None,
...
def _cert_battery_details(sensors):
    ...
    result = {'designed_capacity_mwh': round(designed['value'], 1) if designed else None, 'full_charge_capacity_mwh': round(full['value'], 1) if full else None, 'remaining_capacity_mwh': round(remaining['value'], 1) if remaining else None, 'degradation_percent': round(degradation['value'], 2) if degradation else None, 'charge_percent': round(charge['value'], 2) if charge else None, 'voltage_v': round(volt['value'], 3) if volt else None, 'current_ma': round(curr['value'] * 1000.0, 2) if curr else None, 'charge_discharge_rate_w': round(rate['value'], 2) if rate else None, 'source': str(battery[0].get('source') or 'Native sensor provider') if battery else None}
    ...
    if curr:
        result['_metrics']['current_ma']['source_unit'] = 'A'
        result['_metrics']['current_ma']['normalization'] = 'A_TO_mA'
...
def _linux_storage_inventory():
    """Enumera discos físicos locales en Linux mediante lsblk.

    Incluye también unidades que no estén montadas (por ejemplo otro disco con
    Windows en dual boot). El uso/free sólo se calcula sobre sistemas de archivos
    montados; nunca se inventa espacio utilizado para volúmenes inaccesibles.
    """
    ...
    commands = [
        ['lsblk', '--json', '--bytes', '--output',
         'NAME,KNAME,PATH,TYPE,SIZE,MODEL,SERIAL,TRAN,FSTYPE,MOUNTPOINTS,RM,ROTA'],
        ['lsblk', '--json', '--bytes', '--output',
         'NAME,KNAME,PATH,TYPE,SIZE,MODEL,SERIAL,TRAN,FSTYPE,MOUNTPOINT,RM,ROTA'],
    ]
    data = None
    for command in commands:
        try:
            cp = subprocess.run(command, capture_output=True, text=True, errors='replace', timeout=5)
    ...
    def mountpoints_of(node):
        values = node.get('mountpoints')
        if isinstance(values, list):
            return [str(x).strip() for x in values if x not in (None, '') and str(x).strip()]
    ...
    for node in data.get('blockdevices') or []:
        if not isinstance(node, dict) or str(node.get('type') or '').casefold() != 'disk':
            continue
        linux_name = str(node.get('name') or '').strip().casefold()
        # zram/loop/ram/dm pueden aparecer como TYPE=disk en lsblk, pero no son
        # unidades físicas que deban mostrarse como almacenamiento del equipo.
        if linux_name.startswith(('zram', 'loop', 'ram', 'fd', 'sr', 'dm-')):
            continue
        if str(node.get('rm') or '').strip().casefold() in {'1', 'true', 'yes'}:
            # Dispositivos extraíbles se consideran sólo si el kernel los presenta
            # como almacenamiento fijo. Evita pendrives efímeros en el resumen.
            continue
        device_id = str(node.get('path') or '').strip() or ('/dev/' + str(node.get('name') or '').strip())
        model = str(node.get('model') or '').strip() or str(node.get('name') or '').strip() or device_id
        serial = str(node.get('serial') or '').strip() or None
        size = _finite(node.get('size'))
        rota = str(node.get('rota') or '').strip().casefold()
        media = 'HDD' if rota in {'1', 'true', 'yes'} else 'SSD' if rota in {'0', 'false', 'no'} else None
        descendants = children_of(node)
        mountpoints = []
        filesystems = []
        ...
        for child in descendants + [node]:
            child_path = str(child.get('path') or '').strip() or str(child.get('name') or '').strip()
            fs_type = str(child.get('fstype') or '').strip()
            points = mountpoints_of(child)
            if fs_type and fs_type not in filesystems:
                filesystems.append(fs_type)
            if fs_type and not points and fs_type not in unmounted_filesystems:
                unmounted_filesystems.append(fs_type)
            for mountpoint in points:
                if mountpoint not in mountpoints:
                    mountpoints.append(mountpoint)
            if not points or not child_path or child_path in fs_seen:
                continue
            # Un mismo dispositivo puede aparecer con varios mountpoints (binds /
            # subvolúmenes). Se mide una sola vez para no duplicar bytes.
            measured = False
            for mountpoint in points:
                try:
                    ...
                    continue
                mounted_total += int(usage.total)
                mounted_used += int(usage.used)
                mounted_free += int(usage.free)
                measured = True
                break
            if measured:
                fs_seen.add(child_path)

        used_percent = (mounted_used / mounted_total * 100.0) if mounted_total > 0 else None
        result.append({
            'name': model,
            'model': model,
            'normalized_name': _norm_hw_name(model),
            'serial_number': serial,
            'firmware_revision': None,
            'pnp_device_id': None,
            'device_id': device_id or None,
            'disk_index': disk_number,
            'interface_type': str(node.get('tran') or '').strip() or None,
            'media_type_os': media,
            'size_bytes_os': int(size) if size is not None and size >= 0 else None,
            'os_status': 'OK',
            'inventory_source': 'lsblk',
            'mount_points': ', '.join(mountpoints) if mountpoints else None,
            'system_disk': '/' in mountpoints,
            'mounted_total_bytes': mounted_total if mounted_total > 0 else None,
            'mounted_used_bytes': mounted_used if mounted_total > 0 else None,
            'mounted_free_bytes': mounted_free if mounted_total > 0 else None,
            'used_space_percent': round(used_percent, 1) if used_percent is not None else None,
            'usage_scope': 'mounted_filesystems' if mounted_total > 0 else 'unmounted_or_unreadable',
            'filesystems': list(filesystems),
            'unmounted_filesystems': list(unmounted_filesystems),
            'usage_reason': (None if mounted_total > 0 else
                             ('Sistema(s) de archivos sin montar: ' + ', '.join(unmounted_filesystems)
                              if unmounted_filesystems else 'Sin sistema de archivos montado para medir uso/libre')),
        })
        disk_number += 1
    return result
...
def _enum_cached():
    ...
    if platform.system() == 'Windows':
        ...
        # GPU ya se obtiene de DRM/nvidia-smi en el proveedor nativo; lsblk se
        # usa aquí para enumerar todos los discos físicos, incluso no montados.
        _ENUM_CACHE['gpu'] = []
        _ENUM_CACHE['storage'] = _linux_storage_inventory()
    else:
        _ENUM_CACHE['gpu'] = []
        _ENUM_CACHE['storage'] = []
    _ENUM_CACHE['ts'] = now
...
def _attach_os_storage(entry, os_disk):
    ...
    source = str(os_disk.get('inventory_source') or ('Win32_DiskDrive' if platform.system() == 'Windows' else 'OS inventory')).strip()
    if source and source not in entry['inventory_sources']:
        entry['inventory_sources'].append(source)
    # Datos de capacidad/uso de OS completan huecos de la telemetría sin
    # sobrescribir sensores reales.
    if entry.get('mount_points') in (None, '', 'N/A') and os_disk.get('mount_points'):
        entry['mount_points'] = os_disk.get('mount_points')
    if entry.get('used_space_percent') is None and os_disk.get('used_space_percent') is not None:
        entry['used_space_percent'] = os_disk.get('used_space_percent')
    if entry.get('total_space_gb') is None and os_disk.get('size_bytes_os') is not None:
        try:
            entry['total_space_gb'] = float(os_disk.get('size_bytes_os')) / (1024 ** 3)
        except Exception:
            pass
    if entry.get('free_space_gb') is None and os_disk.get('mounted_free_bytes') is not None:
        try:
            entry['free_space_gb'] = float(os_disk.get('mounted_free_bytes')) / (1024 ** 3)
        except Exception:
            pass
...
def _merge_storage_inventory(lhm_disks, os_disks):
    ...
    for idx, os_disk in enumerate(os_disks):
        ...
        source = str(os_disk.get('inventory_source') or ('Win32_DiskDrive' if platform.system() == 'Windows' else 'OS inventory')).strip()
        size_bytes = os_disk.get('size_bytes_os')
        try:
            total_gb = float(size_bytes) / (1024 ** 3) if size_bytes is not None else None
        except Exception:
            total_gb = None
        free_bytes = os_disk.get('mounted_free_bytes')
        try:
            free_gb = float(free_bytes) / (1024 ** 3) if free_bytes is not None else None
        except Exception:
            free_gb = None
        merged.append({
            ...
            'size_bytes_os': size_bytes,
            'mount_points': os_disk.get('mount_points'),
            'system_disk': bool(os_disk.get('system_disk')),
            'used_space_percent': os_disk.get('used_space_percent'),
            'total_space_gb': total_gb,
            'free_space_gb': free_gb,
            'usage_scope': os_disk.get('usage_scope'),
            'telemetry_available': False,
            'inventory_sources': [source] if source else [],
            'os_inventory': copy.deepcopy(os_disk),
            'identity_ambiguous': False,
            'os_inventory_candidates_count': 1,
            'source': source or 'OS inventory',
...
def _certify_snapshot(snapshot):
    ...
    gpu_sources = sorted({src for row in s['_gpu_inventory'] for src in (row.get('inventory_sources') or []) if src})
    storage_sources = sorted({src for row in s['_storage_inventory'] for src in (row.get('inventory_sources') or []) if src})
    s['_hardware_enumeration'] = {'version': VERSION, 'policy': 'REAL_OR_NA_ONLY', 'gpu_count': len(s['_gpu_inventory']), 'storage_count': len(s['_storage_inventory']), 'gpu_sources': gpu_sources, 'storage_sources': storage_sources, 'inventory_only_devices_keep_sensor_values_na': True, 'deduplication_uses_real_identity_fields': True, 'active_gpu_not_assumed': True}
...
def _lhm_runtime_capability(snapshot):
    ...
    provider_name = ((snapshot.get('_sensor_summary') or {}).get('provider') or 'Native sensor provider')
    return {'provider': provider_name, 'available': provider_available, 'status': status, 'metric_counts': counts, 'freshness_contract': True, 'real_or_na': True}
