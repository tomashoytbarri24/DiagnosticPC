...
Los campos ausentes permanecen N/A. ``Wear`` y ``PercentageUsed`` no se presentan
como sensores directos de salud: cuando existen, se puede mostrar una vida restante
derivada y claramente etiquetada como tal.
"""
...
def _device(index: int, telemetry: Dict[str, Any]) -> Dict[str, Any]:
    if not isinstance(telemetry, dict):
        return {}
    inventory = telemetry.get("_storage_inventory")
    devices = inventory if isinstance(inventory, list) and inventory else telemetry.get("_storage_devices")
...
def build_storage_detail_snapshot(
    ...
    # Un 0% proveniente de una capa de telemetría puede ser un sentinel de sensor
    # no soportado. El SMART NVMe directo se resuelve más abajo como vida derivada.
    candidate_life = _number(_first(device.get("life_percent"), fast.get("health")))
    windows_status_hint = _text(_first(reliability.get("health_status"), reliability.get("operational_status"))).casefold()
    if candidate_life == 0.0 and windows_status_hint in {"healthy", "ok", "normal"}:
        life = None
    else:
        life = candidate_life
    health_source = _metric_source(device, "life_percent") if life is not None else ""
    ...
    if temp is None:
        temp = _valid_temperature(fast.get("temperature_c"))
    if temp is None:
        temp = _valid_temperature(reliability.get("temperature"))
    ...
    nvme_source_text = _text(nvme.get("source"))
    wear_source = (
        "smartctl / NVMe SMART"
        if nvme_used is not None and "smartctl" in nvme_source_text.casefold()
        else "SMART NVMe"
    ...
    remaining_life = None
    remaining_life_source = 'N/A'
    # Percentage Used=0 del SMART NVMe es válido (unidad nueva/sin desgaste).
    # Wear=0 de StorageReliabilityCounter puede ser sentinel/no soportado, por lo
    # que sólo se deriva desde Windows cuando el desgaste es positivo.
    if nvme_used is not None and nvme_used >= 0:
        remaining_life = max(0.0, min(100.0, 100.0 - float(nvme_used)))
        remaining_life_source = (
            'smartctl / NVMe SMART · 100 - Percentage Used'
            if 'smartctl' in nvme_source_text.casefold()
            else 'SMART NVMe · 100 - Percentage Used'
        )
    elif _number(reliability.get('health')) is not None:
        remaining_life = max(0.0, min(100.0, float(_number(reliability.get('health')))))
        remaining_life_source = _text(reliability.get('health_source')) or 'Windows Reliability · vida restante corroborada'
    elif windows_wear is not None and windows_wear > 0:
        remaining_life = max(0.0, min(100.0, 100.0 - float(windows_wear)))
        remaining_life_source = 'Windows Reliability · 100 - desgaste reportado'
    ...
    return {
        ...
        "remaining_life_source": remaining_life_source,
        ...
        "temperature_max_c": _valid_temperature(reliability.get("temperature_max")),
        "telemetry_available": bool(device.get("telemetry_available", True) if device else False),
        "reliability_available": bool(reliability),
        "nvme_smart_available": bool(nvme) and 'smartctl' not in nvme_source_text.casefold(),
        "smartctl_available": bool(nvme) and 'smartctl' in nvme_source_text.casefold(),
        "identity_ambiguous": bool(device.get("identity_ambiguous")),
        "sources": sources,
        "policy": "REAL_OR_NA",
        "derived_health_from_wear": remaining_life is not None,
    }
