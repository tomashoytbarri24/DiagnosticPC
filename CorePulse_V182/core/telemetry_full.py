...
def _cpu_details(sensors):
    ...
    for sensor in cpu:
        ...
        sensor_rows.append({
            ...
            'source': sensor.get('source') or 'LibreHardwareMonitorLib',
            'quality': 'VALID',
    ...
    return {
        ...
        'source': (cpu[0].get('source') or 'LibreHardwareMonitorLib') if cpu else None,
...
def get_system_telemetry():
    ...
    if cpu['package_temp_c'] is not None:
        ...
        telemetry['_metrics']['cpu_temp'] = _metric(cpu['package_temp_c'], '°C', cpu.get('source') or provider_name, 'VALID', sensor=f"{cpu['hardware']} / CPU Package")
    if cpu['clock_avg_ghz'] is not None:
        telemetry['cpu_ghz'] = cpu['clock_avg_ghz']
        telemetry['_metrics']['cpu_ghz'] = _metric(cpu['clock_avg_ghz'], 'GHz', cpu.get('source') or provider_name, 'VALID', sensor=f"{cpu['hardware']} / CPU core clocks")
    if gpus:
        ...
        if telemetry.get('gpu_temp') is None and primary['temperature_c'] is not None:
            ...
            telemetry['_metrics']['gpu_temp'] = _metric(primary['temperature_c'], '°C', primary.get('source') or provider_name, 'VALID', sensor=f"{primary['name']} / GPU Core")
        if telemetry.get('gpu_usage') is None and primary['usage_percent'] is not None:
            telemetry['gpu_usage'] = primary['usage_percent']
            telemetry['_metrics']['gpu_usage'] = _metric(primary['usage_percent'], '%', primary.get('source') or provider_name, 'VALID', sensor=f"{primary['name']} / GPU Core")
    telemetry['_sensor_summary'] = {'provider': provider_name, 'provider_available': provider.available, 'provider_error': provider.error, 'sensor_count': len(sensors), 'cpu_detected': bool(cpu['hardware']), 'gpu_count': len(gpus), 'storage_count': len(storage), 'battery_detected': battery is not None, 'timestamp': time.time()}
    return telemetry
