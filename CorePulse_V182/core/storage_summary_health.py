...
def collect_storage_summary_health(telemetry: Dict[str, Any]) -> Dict[int, Dict[str, Any]]:
    ...
    inventory = tele.get('_storage_inventory') if isinstance(tele.get('_storage_inventory'), list) else []
    sensor_devices = tele.get('_storage_devices') if isinstance(tele.get('_storage_devices'), list) else []
    ...
    for index, device in enumerate(devices):
        ...
        if platform.system() == 'Windows':
            ...
            # En Linux get_storage_health() ya usa smartctl sobre /dev/* y
            # devuelve el registro emparejado. Nunca se consulta \\.\PhysicalDriveN.
            nvme_native = {}
            smartctl = {}
        ...
        reported_wear = _num(record.get('wear'))
        reported_health = _clamp_pct(record.get('health'))
        reported_health_source = str(record.get('health_source') or '').strip()
        ...
        if nvme_remaining is not None:
            ...
            derived = False
        elif reported_health is not None:
            health = reported_health
            smart_record = platform.system() != 'Windows' or 'smart' in str(record.get('source') or '').casefold()
            health_label = (f'Salud SMART  {health:.0f}%' if smart_record else f'Salud  {health:.0f}%')
            health_source = reported_health_source or str(record.get('source') or 'Salud cuantitativa')
            derived = bool(record.get('health_derived', True))
        elif platform.system() != 'Windows' and reported_wear is not None and reported_wear >= 0.0:
            health = _remaining_from_wear(reported_wear)
            health_label = f'Salud SMART  {health:.0f}%'
            provider = str(record.get('source') or '').strip() or 'SMART cuantitativo'
            health_source = f'{provider} · vida restante 100 - desgaste'
            derived = True
        elif platform.system() == 'Windows' and reported_wear is not None and reported_wear > 0.0:
            # Mantiene la regla histórica de Windows: Wear=0 puede ser un sentinel
            # de miniport y no se convierte en 100% sin corroboración adicional.
            health = _remaining_from_wear(reported_wear)
        ...
        mounts = str(record.get('mount_points') or device.get('mount_points') or os_inv.get('mount_points') or '').strip() or None
        out[index] = {
            'index': index,
            ...
            'wear_percent': nvme_used if nvme_used is not None else reported_wear,
            'wear_source': ('NVMe SMART Percentage Used' if nvme_used is not None else str(record.get('source') or 'SMART') if reported_wear is not None else None),
            'wear_quantitative_reliable': bool(nvme_used is not None or reported_wear is not None or reported_health is not None),
            'temperature_c': temp,
