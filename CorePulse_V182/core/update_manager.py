    ...
    if os.name != "nt":
        while time.time() < end:
            try:
                ...
                # El proceso existe aunque no podamos inspeccionarlo.
                pass
            time.sleep(0.25)
...
def relaunch(plan, root):
    # Linux usa el lanzador autocurativo de la versión nueva. Así una
    # actualización side-by-side no depende del .venv de la versión anterior
    # y el acceso del menú queda apuntando a la versión recién instalada.
    if os.name != "nt":
        ...
        if launcher.is_file():
            ...
            try:
                ...
                cp_home = data_home / "CorePulse"
                cp_home.mkdir(parents=True, exist_ok=True)
                current = cp_home / "current"
                if current.is_symlink() or not current.exists():
                    current.unlink(missing_ok=True)
                    current.symlink_to(root, target_is_directory=True)
            except Exception:
                pass
            subprocess.Popen(["bash", str(launcher), "--after-update"], cwd=str(root), close_fds=True, start_new_session=True)
            return
