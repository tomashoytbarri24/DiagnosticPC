...
def get_system_telemetry():
    ...
    telemetry['_sensor_provider'] = {'name': provider_name, 'available': provider.available, 'error': provider.error}
    ...
    if cpu is not None:
        ...
        if old.get('quality') != 'VALID':
            _set_metric(telemetry, 'cpu_temp', None, '°C', provider_name, 'UNAVAILABLE', error=provider.error or 'No se encontró un sensor CPU real.')
