...
def _collect_snapshot():
    ...
    provider_name = getattr(provider, 'name', None) or 'Native sensors'
    ...
    snapshot = {'_telemetry_version': '0.8.4.2', '_background_worker': True, '_worker_refresh_duration_ms': duration_ms, '_snapshot_timestamp': time.time(), 'cpu_name': cpu_name, 'cpu_usage': cpu_usage, 'cpu_ghz': cpu_ghz, 'cpu_temp': cpu_temp, 'ram_usage': ram_usage, 'ram_total_gb': ram_total_gb, 'ram_used_gb': ram_used_gb, 'ram_available_gb': ram_available_gb, 'gpu_name': gpu_name, 'gpu_usage': gpu_usage, 'gpu_temp': gpu_temp, 'gpu_vram_gb': gpu_vram_gb, '_cpu': cpu, '_gpus': gpus, '_storage_devices': storage, '_battery': battery, '_metrics': {'cpu_usage': _metric(cpu_usage, '%', 'psutil.cpu_percent'), 'cpu_ghz': _metric(cpu_ghz, 'GHz', cpu.get('source') or provider_name, sensor=f'{cpu_name} / clocks'), 'cpu_temp': _metric(cpu_temp, '°C', cpu.get('source') or provider_name, sensor=f'{cpu_name} / CPU Package'), 'ram_usage': _metric(ram_usage, '%', 'psutil.virtual_memory'), 'gpu_usage': _metric(gpu_usage, '%', primary_gpu.get('source') or provider_name, sensor=f'{gpu_name} / GPU Core'), 'gpu_temp': _metric(gpu_temp, '°C', primary_gpu.get('source') or provider_name, sensor=f'{gpu_name} / GPU Core'), 'gpu_vram_gb': _metric(gpu_vram_gb, 'GB', primary_gpu.get('source') or provider_name, sensor=f'{gpu_name} / GPU Memory Total')}, '_sensor_summary': {'provider': provider_name, 'provider_available': provider.available, 'provider_error': provider.error, 'sensor_count': len(sensors), 'cpu_detected': bool(cpu.get('hardware')), 'gpu_count': len(gpus), 'storage_count': len(storage), 'battery_detected': battery is not None, 'timestamp': time.time(), 'background_worker': True}}
    return snapshot
