...
import time
import psutil
from core.windows_commands import run_hidden
try:
...
def _run_command(command, timeout=2):
    args = command if isinstance(command, (list, tuple)) else [str(command)]
    result = run_hidden(args, timeout=timeout, category='TELEMETRY_COMMAND', log_failures=False)
...
def _linux_cpu_name():
    try:
        ...
        pass
    return platform.processor() or 'Procesador'

def _linux_gpu_inventory():
    rows = []
    # NVIDIA: nombre y VRAM certificados por nvidia-smi cuando está disponible.
    output = _run_command(['nvidia-smi', '--query-gpu=name,memory.total', '--format=csv,noheader,nounits'], timeout=2)
    if output:
        for line in output.splitlines():
            ...
            if len(parts) > 1:
                try:
                    ...
                    pass
            rows.append((parts[0] or 'NVIDIA GPU', vram))
        if rows:
            return rows
    # Fallback de inventario DRM. No se infiere VRAM si sysfs no la expone.
    ...
    if drm.is_dir():
        for card in sorted(drm.glob('card[0-9]*')):
            ...
            if not dev.exists():
                continue
            try:
                vendor = (dev / 'vendor').read_text().strip().casefold()
            except Exception:
                vendor = ''
            vendor_name = {'0x1002': 'AMD', '0x8086': 'Intel', '0x10de': 'NVIDIA'}.get(vendor, 'GPU')
            driver = ''
            try:
                link = (dev / 'driver').resolve()
                driver = link.name
            except Exception:
                pass
            vram = None
            try:
                ...
                if raw > 0:
                    vram = round(raw / 1024**3, 2)
            except Exception:
                pass
            label = ' '.join(x for x in (vendor_name, driver.upper() if driver else None, card.name) if x)
            rows.append((label, vram))
    return rows
...
def get_system_chassis_and_bios():
    ...
    if IS_WINDOWS and wmi is not None:
        ...
        bios_vendor = _linux_dmi_text('bios_vendor')
        bios_version = _linux_dmi_text('bios_version')
        board_vendor = _linux_dmi_text('board_vendor')
        board_name = _linux_dmi_text('board_name')
        if bios_vendor or bios_version:
            bios_info = ' '.join(x for x in (bios_vendor, bios_version) if x)
        if board_vendor or board_name:
            board_info = ' '.join(x for x in (board_vendor, board_name) if x)
        chassis_type = _linux_dmi_text('chassis_type')
        try:
            chassis_code = int(chassis_type) if chassis_type is not None else None
        except Exception:
            chassis_code = None
        laptop_types = {8, 9, 10, 11, 12, 14, 18, 21, 30, 31, 32}
        is_laptop = chassis_code in laptop_types if chassis_code is not None else False
    try:
...
def get_system_telemetry():
    ...
    metrics['gpu_vram_gb'] = _metric(vram_gb, 'GB', gpu.get('vram_source') or (('Win32_VideoController' if IS_WINDOWS else 'Linux DRM/nvidia-smi') if detected_vram is not None else 'No reliable GPU VRAM provider'), 'VALID' if vram_gb is not None else 'UNAVAILABLE')
