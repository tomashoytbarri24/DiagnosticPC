...
# Nivel del tono de prueba generado por CorePulse. 0.32 ~= -9.9 dBFS pico:
# claramente audible, con margen amplio frente a clipping y sin modificar
# el volumen maestro/por-aplicación de Windows.
...
def tone(spec, channel):
    ...
    for i in range(count):
        ...
        value = TEST_TONE_AMPLITUDE * envelope * math.sin(2 * math.pi * 440 * i / rate)
    ...
    def play(self, channel, expected, cancel, recording=None):
        ...
        with endpoint(0) as (client, fmt, info):
            ...
            if recording is None:
                ...
                gain = min(1., .25 / peak) if peak else 1.
                values = []
    ...
    def record(self, expected, cancel, on_level, duration_s=5.0):
        ...
        with endpoint(1) as (client, fmt, info):
            ...
            try:
                ...
                limit = int(duration_s * info['rate'])
                deadline = time.monotonic() + duration_s + 2.5
                while len(samples) < limit and not cancel.is_set():
                    if time.monotonic() > deadline:
                        raise AudioError('No se recibieron muestras de audio dentro del tiempo esperado. Revisa micrófono y permisos.')
                    packet = U()
