    ...
    if key == 'gpu':
        ...
        if status == 'UNAVAILABLE' or (value is None and status != 'ERROR'):
            return {
                ...
                'secondary': reason or 'El sistema no expuso un contexto OpenGL acelerado.',
        ...
        if unit.upper() == 'FPS' or r.get('benchmark_method') == 'COREPULSE_GPU_VISUAL_MULTIPHASE_V2':
            ...
            secondary = ' · '.join(secondary_parts) or f'Duración: {duration_text}'
            linux_visible = r.get('benchmark_method') == 'COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1'
            return {
                'title': 'GPU',
                'subtitle': 'Benchmark OpenGL visible' if linux_visible else 'Benchmark visual multifase',
                'status': 'Prueba GPU completada' if linux_visible else 'Prueba 3D completada',
                'short_status': 'Medida', 'tone': 'purple',
                'headline': headline, 'secondary': secondary, 'display_headline': headline, 'display_secondary': secondary,
                'verdict': 'Carga gráfica OpenGL completada' if linux_visible else 'Carga gráfica multifase completada',
                'meaning': ('CorePulse renderiza una ventana OpenGL real y visible en Linux para medir el renderer activo.' if linux_visible else 'CorePulse mide el mismo workload visual multifase usado por Diagnóstico: geometría, fill, texturas, shaders/compute cuando están disponibles y carga combinada.'),
