...
def _benchmark_gpu_opengl_linux(
    seconds: float,
    progress_callback: ProgressCallback = None,
    stop_check: Optional[Callable[[], bool]] = None,
):
    """Benchmark GPU OpenGL visible para Linux usando X11/GLX vía ctypes.

    No requiere paquetes Python adicionales. En sesiones Wayland se apoya en
    XWayland cuando DISPLAY está disponible. La ventana se muestra de verdad:
    no es un vídeo ni una animación simulada, sino el workload que se mide.
    """
    from ctypes.util import find_library

    if not os.environ.get('DISPLAY'):
        return _result(
            'GPU', None, 'FPS', 'CorePulse OpenGL visible', 0,
            status='UNAVAILABLE', reason='No hay DISPLAY/XWayland disponible para crear la ventana OpenGL.',
            benchmark_method='COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1',
        )
    try:
        x11 = ctypes.CDLL(find_library('X11') or 'libX11.so.6')
        gl = ctypes.CDLL(find_library('GL') or 'libGL.so.1')
    except Exception as exc:
        return _result(
            'GPU', None, 'FPS', 'CorePulse OpenGL visible', 0,
            status='UNAVAILABLE', reason=f'OpenGL/X11 no disponible: {exc}',
            benchmark_method='COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1',
        )
    ...
    class XVisualInfo(ctypes.Structure):
        _fields_ = [
            ...
            ('depth', ctypes.c_int), ('class_', ctypes.c_int), ('red_mask', ctypes.c_ulong),
    ...
    class XDestroyWindowEvent(ctypes.Structure):
        _fields_ = [
            ('type', ctypes.c_int), ('serial', ctypes.c_ulong), ('send_event', ctypes.c_int),
            ('display', ctypes.c_void_p), ('event', ctypes.c_ulong), ('window', ctypes.c_ulong),
        ]
    ...
    x11.XRootWindow.argtypes = [ctypes.c_void_p, ctypes.c_int]; x11.XRootWindow.restype = ctypes.c_ulong
    x11.XCreateColormap.argtypes = [ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p, ctypes.c_int]; x11.XCreateColormap.restype = ctypes.c_ulong
    x11.XCreateWindow.argtypes = [ctypes.c_void_p, ctypes.c_ulong, ctypes.c_int, ctypes.c_int, ctypes.c_uint, ctypes.c_uint, ctypes.c_uint, ctypes.c_int, ctypes.c_uint, ctypes.c_void_p, ctypes.c_ulong, ctypes.POINTER(XSetWindowAttributes)]; x11.XCreateWindow.restype = ctypes.c_ulong
    ...
    x11.XPending.argtypes = [ctypes.c_void_p]; x11.XPending.restype = ctypes.c_int
    ...
    x11.XSetWMProtocols.argtypes = [ctypes.c_void_p, ctypes.c_ulong, ctypes.POINTER(ctypes.c_ulong), ctypes.c_int]
    x11.XDestroyWindow.argtypes = [ctypes.c_void_p, ctypes.c_ulong]
    x11.XFreeColormap.argtypes = [ctypes.c_void_p, ctypes.c_ulong]
    x11.XFree.argtypes = [ctypes.c_void_p]
    x11.XCloseDisplay.argtypes = [ctypes.c_void_p]

    gl.glXChooseVisual.argtypes = [ctypes.c_void_p, ctypes.c_int, ctypes.POINTER(ctypes.c_int)]; gl.glXChooseVisual.restype = ctypes.POINTER(XVisualInfo)
    gl.glXCreateContext.argtypes = [ctypes.c_void_p, ctypes.POINTER(XVisualInfo), ctypes.c_void_p, ctypes.c_int]; gl.glXCreateContext.restype = ctypes.c_void_p
    gl.glXMakeCurrent.argtypes = [ctypes.c_void_p, ctypes.c_ulong, ctypes.c_void_p]; gl.glXMakeCurrent.restype = ctypes.c_int
    gl.glXSwapBuffers.argtypes = [ctypes.c_void_p, ctypes.c_ulong]
    gl.glXDestroyContext.argtypes = [ctypes.c_void_p, ctypes.c_void_p]
    gl.glGetString.argtypes = [ctypes.c_uint]; gl.glGetString.restype = ctypes.c_char_p
    ...
    gl.glVertexPointer.argtypes = [ctypes.c_int, ctypes.c_uint, ctypes.c_int, ctypes.c_void_p]
    ...
    gl.glRotatef.argtypes = [ctypes.c_float, ctypes.c_float, ctypes.c_float, ctypes.c_float]

    GLX_RGBA, GLX_DOUBLEBUFFER, GLX_DEPTH_SIZE = 4, 5, 12
    attrs = (ctypes.c_int * 7)(GLX_RGBA, GLX_DOUBLEBUFFER, GLX_DEPTH_SIZE, 24, 0, 0, 0)
    display = x11.XOpenDisplay(None)
    if not display:
        return _result('GPU', None, 'FPS', 'CorePulse OpenGL visible', 0, status='UNAVAILABLE',
                       reason='XOpenDisplay falló; DISPLAY existe pero la sesión gráfica no es accesible.',
                       benchmark_method='COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1')

    vi = None; ctx = None; window = 0; cmap = 0
    try:
        screen = x11.XDefaultScreen(display)
        root = x11.XRootWindow(display, screen)
        vi = gl.glXChooseVisual(display, screen, attrs)
        if not vi:
            raise RuntimeError('GLX no encontró un visual RGBA con doble buffer')
        cmap = x11.XCreateColormap(display, root, vi.contents.visual, 0)
        swa = XSetWindowAttributes()
        swa.colormap = cmap
        swa.event_mask = (1 << 0) | (1 << 15) | (1 << 17)  # KeyPress | Exposure | StructureNotify
        CWEventMask, CWColormap = (1 << 11), (1 << 13)
        width, height = 960, 540
        window = x11.XCreateWindow(
            display, root, 120, 120, width, height, 0, vi.contents.depth, 1,
            vi.contents.visual, CWColormap | CWEventMask, ctypes.byref(swa)
        )
        if not window:
            raise RuntimeError('X11 no pudo crear la ventana del benchmark')
        x11.XStoreName(display, window, b'CorePulse GPU Benchmark - OpenGL')
        wm_delete = x11.XInternAtom(display, b'WM_DELETE_WINDOW', 0)
        if wm_delete:
            atom = ctypes.c_ulong(wm_delete)
            x11.XSetWMProtocols(display, window, ctypes.byref(atom), 1)
        x11.XMapWindow(display, window)
        x11.XFlush(display)

        ctx = gl.glXCreateContext(display, vi, None, 1)
        if not ctx or not gl.glXMakeCurrent(display, window, ctx):
            raise RuntimeError('No se pudo activar el contexto OpenGL/GLX')

        GL_RENDERER, GL_VENDOR, GL_VERSION = 0x1F01, 0x1F00, 0x1F02
        renderer = (gl.glGetString(GL_RENDERER) or b'').decode('utf-8', 'replace')
        vendor = (gl.glGetString(GL_VENDOR) or b'').decode('utf-8', 'replace')
        gl_version = (gl.glGetString(GL_VERSION) or b'').decode('utf-8', 'replace')
        if not renderer or 'llvmpipe' in renderer.casefold() or 'softpipe' in renderer.casefold():
            return _result('GPU', None, 'FPS', 'CorePulse OpenGL visible', 0, status='UNAVAILABLE',
                           reason=f'Renderizado por software detectado: {renderer or "N/A"}', renderer=renderer or 'N/A', vendor=vendor or 'N/A',
                           benchmark_method='COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1')

        # Intento best-effort de desactivar VSync. Si el driver/compositor no lo
        # permite, la prueba sigue siendo válida para comparar ejecuciones locales.
        try:
            get_proc = getattr(gl, 'glXGetProcAddressARB')
            get_proc.argtypes = [ctypes.c_char_p]; get_proc.restype = ctypes.c_void_p
            ptr = get_proc(b'glXSwapIntervalEXT')
            if ptr:
                ctypes.CFUNCTYPE(None, ctypes.c_void_p, ctypes.c_ulong, ctypes.c_int)(ptr)(display, window, 0)
        except Exception:
            pass
        ...
        gl.glClearColor(ctypes.c_float(0.015), ctypes.c_float(0.035), ctypes.c_float(0.065), ctypes.c_float(1.0))
        gl.glEnableClientState(GL_VERTEX_ARRAY)

        triangles_per_draw = 9000
        draws_per_frame = 10
        vertex_count = triangles_per_draw * 3
        vertices = (ctypes.c_float * (vertex_count * 2))()
        for tri in range(triangles_per_draw):
            cell_x = tri % 150
            cell_y = (tri // 150) % 60
            x = -0.96 + (cell_x / 150.0) * 1.92
            y = -0.96 + (cell_y / 60.0) * 1.92
            sx, sy = 0.009, 0.014
            base = tri * 6
            vertices[base:base + 6] = (x, y, x + sx, y, x, y + sy)
        ptr_vertices = ctypes.cast(vertices, ctypes.c_void_p)

        seconds = max(3.0, min(30.0, float(seconds)))
        start = time.perf_counter(); deadline = start + seconds
        frames = 0; frame_times = []; last_frame = start; next_emit = start
        user_closed = False
        while time.perf_counter() < deadline:
            if callable(stop_check) and stop_check():
                break
            while x11.XPending(display):
                event = XEvent(); x11.XNextEvent(display, ctypes.byref(event))
                event_type = int(event.type)
                if event_type == 17:  # DestroyNotify real de la ventana.
                    user_closed = True
                elif event_type == 33 and wm_delete:  # ClientMessage: sólo WM_DELETE_WINDOW cancela.
                    try:
                        user_closed = int(event.xclient.data.l[0]) == int(wm_delete)
                    except Exception:
                        user_closed = False
            if user_closed:
                break
            ...
            now = time.perf_counter()
            angle = ctypes.c_float(((now - start) * 32.0) % 360.0)
            gl.glClear(GL_COLOR_BUFFER_BIT)
            gl.glMatrixMode(GL_MODELVIEW); gl.glLoadIdentity(); gl.glRotatef(angle, 0.0, 0.0, 1.0)
            gl.glVertexPointer(2, GL_FLOAT, 0, ptr_vertices)
            for draw in range(draws_per_frame):
                phase = (draw + frames * 0.02) % 3.0
                gl.glColor3f(ctypes.c_float(0.15 + 0.18 * (phase == 0)), ctypes.c_float(0.55 + 0.16 * (phase == 1)), ctypes.c_float(0.85 + 0.12 * (phase == 2)))
                gl.glDrawArrays(GL_TRIANGLES, 0, vertex_count)
            gl.glFinish(); gl.glXSwapBuffers(display, window)
            frames += 1
            frame_end = time.perf_counter()
            frame_times.append(max(1e-6, frame_end - last_frame)); last_frame = frame_end
            if frame_end >= next_emit:
                frac = min(1.0, (frame_end - start) / seconds)
                _safe_progress(progress_callback, frac, 'GPU · OpenGL visible', f'{renderer} · {frames} frames')
                next_emit = frame_end + 0.30

        duration = max(1e-6, time.perf_counter() - start)
        if user_closed:
            status, reason = 'CANCELLED', 'El usuario cerró la ventana del benchmark GPU'
        elif callable(stop_check) and stop_check():
            status, reason = 'SAFETY_STOP', 'Benchmark GPU detenido por seguridad o cancelación'
        else:
            status, reason = 'OK', None
        fps = frames / duration if frames else None
        triangles = frames * triangles_per_draw * draws_per_frame
        sorted_slow = sorted(frame_times, reverse=True)
        low_count = max(1, math.ceil(len(sorted_slow) * 0.01)) if sorted_slow else 0
        one_low = (1.0 / (sum(sorted_slow[:low_count]) / low_count)) if low_count else None
        result = _result(
            'GPU', fps, 'FPS', 'CorePulse OpenGL visible', duration,
            benchmark_method='COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1', status=status, reason=reason,
            renderer=renderer, vendor=vendor, gl_version=gl_version, frames=frames,
            frames_per_s=fps, fps_1pct_low=one_low, triangles_per_s=(triangles / duration if frames else None),
            triangles_per_frame=triangles_per_draw * draws_per_frame,
            resolution=f'{width}x{height} visible', requested_resolution=f'{width}x{height}',
            phase_results=[{
                'key': 'combined', 'label': 'OpenGL visible', 'status': 'OK' if status == 'OK' else status,
                'frames_per_s': fps, 'one_percent_low_fps': one_low,
                'frametime_avg_ms': (1000.0 / fps) if fps else None,
                'workload': {'triangles_per_frame': triangles_per_draw * draws_per_frame, 'visible_window': True},
            }],
        )
        result['phases'] = list(result.get('phase_results') or [])
        result['phase_count'] = len(result['phases'])
        result['measured_phase_count'] = sum(1 for row in result['phases'] if row.get('status') == 'OK')
        result['one_percent_low_fps'] = one_low
        result['frametime_avg_ms'] = (1000.0 / fps) if fps else None
        _safe_progress(progress_callback, 1.0, 'GPU completado', f'{renderer} · {fps:.1f} FPS' if fps else renderer)
        return result
    except Exception as exc:
        return _result(
            'GPU', None, 'FPS', 'CorePulse OpenGL visible', 0,
            benchmark_method='COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1', status='ERROR', reason=f'{type(exc).__name__}: {exc}',
        )
    finally:
        try:
            if display and ctx:
        ...
        try:
            if display and cmap:
                x11.XFreeColormap(display, cmap)
        except Exception:
            pass
        try:
            if vi:
...
def benchmark_gpu(
    ...
    *,
    ...
    """Benchmark gráfico real OpenGL para Windows y Linux."""
    duration = max(2.0, min(float(seconds if seconds is not None else min(timeout, 12)), 30.0))
    start = time.perf_counter()
    try:
        if platform.system() == 'Linux':
            return _benchmark_gpu_opengl_linux(duration, progress_callback=progress_callback, stop_check=stop_check)
        if platform.system() == 'Windows':
            return _benchmark_gpu_opengl(duration, progress_callback=progress_callback, stop_check=stop_check)
        return _result('GPU', None, 'FPS', 'N/A', 0, status='UNAVAILABLE', reason='Plataforma gráfica no soportada')
    except Exception as exc:
        return _result(
...
def run_benchmark_suite(
    ...
    *,
    ...
    for key in selected:
        ...
        try:
            if key == 'cpu':
                ...
                if platform.system() == 'Linux':
                    results[key] = _benchmark_gpu_opengl_linux(
                        profile_data['gpu_seconds'], progress_callback=callback, stop_check=should_stop,
                    )
                else:
    ...
    return {
        ...
        'component_methods': {'cpu': CPU_METHOD_ID, 'ram': RAM_METHOD_ID, 'ssd': SSD_METHOD_ID, 'gpu': ('COREPULSE_GPU_OPENGL_VISIBLE_LINUX_V1' if platform.system() == 'Linux' else GPU_METHOD_ID)},
