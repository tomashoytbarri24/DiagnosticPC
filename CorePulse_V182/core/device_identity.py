...
def _linux_sysfs_value(name: str) -> Optional[str]:
    try:
        return _clean((Path('/sys/class/dmi/id') / name).read_text(encoding='utf-8', errors='replace'))
    except Exception:
        return None

def _linux_device_identity() -> Dict[str, Any]:
    manufacturer = _linux_sysfs_value('sys_vendor')
    model = _linux_sysfs_value('product_name')
    product_version = _linux_sysfs_value('product_version')
    product_uuid = _linux_sysfs_value('product_uuid')
    board_manufacturer = _linux_sysfs_value('board_vendor')
    board_model = _linux_sysfs_value('board_name')
    board_version = _linux_sysfs_value('board_version')
    board_serial = _linux_sysfs_value('board_serial')
    bios_vendor = _linux_sysfs_value('bios_vendor')
    bios_version = _linux_sysfs_value('bios_version')
    bios_date = _linux_sysfs_value('bios_date')
    chassis_raw = _linux_sysfs_value('chassis_type')
    try:
        chassis_code = int(chassis_raw) if chassis_raw else None
    except Exception:
        chassis_code = None
    laptop_codes = {8, 9, 10, 11, 12, 14, 18, 21, 30, 31, 32}
    desktop_codes = {3, 4, 5, 6, 7, 13, 15, 16, 23, 24, 35, 36}
    if chassis_code in laptop_codes:
        form_factor = 'LAPTOP'
    elif chassis_code in desktop_codes:
        form_factor = 'DESKTOP'
    else:
        try:
            form_factor = 'LAPTOP' if psutil.sensors_battery() is not None else 'UNKNOWN'
        except Exception:
            form_factor = 'UNKNOWN'
    ...
    if form_factor == 'DESKTOP':
        ...
        display_model = system_label or model or manufacturer or 'Laptop / Notebook'
        support_target = system_label or board_label
    else:
        display_model = system_label or board_label or platform.node() or 'Equipo Linux'
        support_target = system_label or board_label
    return {
        'schema': 3, 'version': VERSION, 'captured_at': time.time(), 'platform': platform.system(),
        'form_factor': form_factor, 'chassis_codes': [chassis_code] if chassis_code is not None else [],
        'manufacturer': manufacturer, 'model': model, 'display_model': display_model,
        'support_target': support_target, 'system_type': platform.machine() or None,
        'product': {'vendor': manufacturer, 'name': model, 'version': product_version, 'uuid': product_uuid, 'identifying_number': None},
        'motherboard': {'manufacturer': board_manufacturer, 'model': board_model, 'version': board_version, 'serial': board_serial},
        'bios': {'manufacturer': bios_vendor, 'version': bios_version, 'release_date': bios_date, 'serial': None},
        'sources': {'system': 'Linux DMI sysfs', 'product': 'Linux DMI sysfs', 'motherboard': 'Linux DMI sysfs', 'bios': 'Linux DMI sysfs', 'form_factor': 'Linux DMI sysfs/psutil'},
        'policy': 'REAL_OR_NA_NO_INFERENCE',
    }

def _linux_cpu_identity() -> Dict[str, Any]:
    name = None
    vendor = None
    family = None
    stepping = None
    ...
    try:
        for line in Path('/proc/cpuinfo').read_text(encoding='utf-8', errors='replace').splitlines():
            ...
            low = key.casefold()
            if name is None and low in {'model name', 'hardware'}:
                name = _clean(value)
            elif vendor is None and low in {'vendor_id', 'cpu implementer'}:
                vendor = _clean(value)
            elif family is None and low == 'cpu family':
                try: family = int(value)
                except Exception: pass
            elif stepping is None and low == 'stepping':
                try: stepping = int(value)
                except Exception: pass
            elif low in {'flags', 'features'}:
    ...
    try:
        f = psutil.cpu_freq()
        if f and getattr(f, 'max', None):
            freq = round(float(f.max) / 1000.0, 3)
    except Exception:
        pass
    return {
        'name': name or platform.processor() or None, 'manufacturer': vendor,
        'description': platform.machine() or None, 'socket': None, 'max_clock_ghz': freq,
        'current_clock_ghz_wmi': None, 'cores': psutil.cpu_count(logical=False), 'threads': psutil.cpu_count(logical=True),
        'processor_id': None, 'architecture': platform.machine() or None, 'architecture_code': None,
        'address_width_bits': 64 if platform.architecture()[0].startswith('64') else 32, 'data_width_bits': None,
        'l2_cache_kb': None, 'l3_cache_kb': None,
        'virtualization_firmware_enabled': True if {'vmx','svm'} & flags else None,
        'slat_supported': None, 'vm_monitor_mode_extensions': True if {'vmx','svm'} & flags else None,
        'family': family, 'stepping': stepping, 'revision': None,
        'source': '/proc/cpuinfo + psutil', 'quality': 'VALID' if name else 'UNAVAILABLE',
    }

def _linux_gpu_inventory() -> List[Dict[str, Any]]:
    out = []
    ...
    for card in sorted(drm.glob('card[0-9]*')):
        ...
        if not dev.exists():
            continue
        vendor_raw = _clean(_read_text_file(dev / 'vendor'))
        vendor = {'0x10de': 'NVIDIA', '0x1002': 'AMD', '0x8086': 'Intel'}.get((vendor_raw or '').lower(), vendor_raw)
        try:
            driver = (dev / 'driver').resolve().name
        except Exception:
            driver = None
        device_id = _clean(_read_text_file(dev / 'device'))
        name = ' '.join(x for x in (vendor, driver.upper() if driver else None, device_id) if x).strip() or card.name
        key = name.casefold()
        if key in seen:
            continue
        seen.add(key)
        vram = None
        try:
            raw = float((dev / 'mem_info_vram_total').read_text().strip())
            if raw > 0: vram = round(raw / 1024**3, 2)
        except Exception:
            pass
        out.append({'name': name, 'vram_gb_wmi': vram, 'driver_version': None, 'pnp_device_id': None,
                    'vendor': vendor, 'video_processor': device_id, 'hardware_type': 'LINUX_DRM_ADAPTER',
                    'source': 'Linux DRM sysfs', 'quality': 'VALID'})
    return out
...
def _linux_storage_inventory() -> List[Dict[str, Any]]:
    try:
        cp = subprocess.run(['lsblk', '-J', '-b', '-o', 'NAME,MODEL,SERIAL,SIZE,TYPE,TRAN'], capture_output=True, text=True, timeout=4, errors='replace')
        ...
        return []
    out = []
    for row in payload.get('blockdevices') or []:
        if not isinstance(row, dict) or row.get('type') != 'disk':
            continue
        linux_name = str(row.get('name') or '').strip().casefold()
        if linux_name.startswith(('zram', 'loop', 'ram', 'fd', 'sr', 'dm-')):
            continue
        size = _num(row.get('size'))
        model = _clean(row.get('model')) or _clean(row.get('name')) or 'Unidad de almacenamiento'
        out.append({'index': len(out), 'name': model, 'model': model, 'manufacturer': None,
                    'serial': _clean(row.get('serial')), 'total_space_gb': round(size / 1024**3, 2) if size else None,
                    'interface': _clean(row.get('tran')), 'pnp_device_id': None, 'temperature_c': None, 'life_percent': None,
                    'source': 'lsblk', 'quality': 'IDENTITY_ONLY'})
    return out

def collect_device_identity() -> Dict[str, Any]:
    if platform.system() == 'Linux':
        return _linux_device_identity()
...
def collect_ram_identity() -> Dict[str, Any]:
    """Devuelve inventario RAM real sin inferir SPD ausente.
    ...
    if platform.system() == 'Linux':
        try:
            ...
            total = None
        return {'installed_capacity_gb': total, 'module_count': None, 'slots_total': None,
                'slots_available': None, 'modules': [], 'memory_types': [],
                'source': 'psutil.virtual_memory', 'quality': 'VALID' if total else 'UNAVAILABLE',
                'available_slots_derivation': None, 'policy': 'REAL_OR_NA_NO_CHANNEL_OR_TIMING_INFERENCE'}
...
def collect_cpu_identity() -> Dict[str, Any]:
    """Devuelve especificaciones CPU reales según la plataforma."""
    if platform.system() == 'Linux':
        return _linux_cpu_identity()
    return _cpu_wmi()
...
def _storage_wmi() -> List[Dict[str, Any]]:
    if platform.system() == 'Linux':
        return _linux_storage_inventory()
...
def collect_hardware_inventory(telemetry: Optional[dict]=None, disks: Optional[list]=None) -> Dict[str, Any]:
    ...
    cpu_source = 'core.telemetry_background/native sensors' if _clean(t.get('cpu_name')) else cpu_wmi.get('source')
    ram_source = 'core.telemetry_background/psutil.virtual_memory' if _num(t.get('ram_total_gb')) is not None else ('Win32_PhysicalMemory' if platform.system() == 'Windows' else 'psutil.virtual_memory')
    storage_source = 'core.telemetry_background/_storage_devices' if telemetry_storage else 'core.telemetry_background/get_all_disks_data' if d else ('Win32_DiskDrive' if platform.system() == 'Windows' else 'lsblk')
