...
- Corregida la tarjeta Evidencia vacía/N/A del diagnóstico.
- Alturas homogéneas para tarjetas de una misma fila.
- Prueba técnica automática de salida estéreo + captura de micrófono.
- Benchmark GPU visible en Linux mediante OpenGL/GLX sin dependencias Python nuevas.
- Diagnóstico Linux incluye GPU en la suite estándar; Windows mantiene su flujo GPU actual.
- Presentación de benchmark adaptada al proveedor OpenGL visible de Linux.

## V180
- Almacenamiento Linux: zram/loop/ram/dm dejan de aparecer como discos físicos.
- Resumen: las tarjetas muestran el disco físico y su capacidad, no la lista de particiones/subvolúmenes.
- Discos no montados: se informa capacidad física y estado "No montado en este sistema".
- Salud NVMe Linux: nueva lectura UDisks2 NVMe SMART sin ejecutar CorePulse como root, con smartctl como fallback.
- Vida restante: 100 - Percentage Used cuando el controlador NVMe expone ese contador estándar.
- Compatibilidad Windows preservada, incluido el tratamiento seguro de Wear=0 ambiguo.
# CorePulse V180

- Diagnóstico: se restauran las tres facetas visibles; los N/A reales ya no se ocultan.
- Diagnóstico: almacenamiento consolida telemetría física y uso real del volumen para evitar N/A falsos en espacio/salud cuando la fuente existe.
- Evidencia: la UI sólo muestra trazabilidad cuando existe un hallazgo WARNING/CRITICAL; los componentes normales ya no duplican métricas como evidencia.
- Windows y Linux conservan sus módulos y reglas de plataforma existentes.

# Changelog

## V177 — Diagnóstico sin N/A visuales y evidencia contextual
- El Diagnóstico conserva REAL_OR_NA internamente, pero oculta facetas sin medición en la presentación.
- Los componentes parcialmente medidos muestran PARCIAL; si no existe ningún dato útil muestran SIN DATOS en lugar de N/A.
- El bloque y botón Ver evidencia sólo aparecen cuando existe evidencia real para mostrar.
- Las filas N/A se filtran del detalle de evidencia sin alterar la fuente de datos ni las reglas del diagnóstico.
- Se mantiene intacto el benchmark GPU actual; se documenta como siguiente paso su sustitución por una carga GPU OpenGL multiplataforma.

# CorePulse V176 — Inicio Linux sin comandos

- Diagnóstico Linux específico de plataforma: sin bloque Windows ni GPU 3D incompatible.
- Batería Linux desde sysfs power_supply con capacidades, salud/desgaste, ciclos y lecturas eléctricas reales cuando estén disponibles.
- Releases intenta llevar el navegador al frente en Linux sin cambiar la seguridad del compositor.
- Windows conserva su flujo de diagnóstico y benchmark GPU existente.

# CorePulse V169 — Quality Gate y consistencia de versión

- Autoridad única de versión en `core/version.py`; metadatos/build/installer sincronizados.
- `pyproject.toml` usa versión dinámica; se eliminan fallbacks numéricos obsoletos de build e Inno Setup.
- Collection de pytest deja de abortar por tests script-style con `SystemExit` en import.
- Tests históricos anteriores a V162 se conservan como `legacy_snapshot` y no bloquean la suite actual.
- Nuevo `quality_gate.py`, `Validar_CorePulse.bat` y `QUALITY_GATE.md`.
- Sin cambios funcionales intencionales en los módulos estabilizados de V168.

# CorePulse V167 — Releases automáticas de Desarrollo

- Cada push a una rama `*/corepulse-dev` puede crear o actualizar automáticamente una prerelease de Desarrollo mediante GitHub Actions.
- El workflow se instala en la raíz del clon desde el publicador por perfiles; no se hardcodean usuarios, rutas ni credenciales.
- Se mantiene una prerelease por versión (`V167-dev`), actualizando sus assets en pushes posteriores de la misma versión.
- Se genera `CorePulse_V167.zip` limpio y su SHA-256; el canal Desarrollo del actualizador puede consumirlo directamente.
- El canal Estable continúa ignorando prereleases y Publicar conserva perfiles, bloqueo de ramas principales y push sin force.
- Ver `ACTUALIZACIONES_V167.md` y `VALIDACION_V167.md`.

# CorePulse V164 — Perfiles de publicación Git

- Nuevo sistema de perfiles locales de publicación: carpeta raíz, repositorio/origin, rama e identidad Git por usuario.
- Las credenciales no se almacenan en CorePulse; Git Credential Manager/SSH continúa siendo la autoridad de autenticación.
- La publicación deja de depender de ejecutar una carpeta `CorePulse_Vxxx` dentro del clon. El ámbito pasa a ser la raíz del repositorio elegida.
- Git detecta altas, modificaciones y eliminaciones en toda la raíz, respeta `.gitignore` y evita commits vacíos/push innecesarios.
- FASE 1/2/3 dejan de ser una excepción del nuevo flujo: sólo se publican si realmente cambiaron.
- Selector/gestor de perfiles con Nuevo, Guardar, Eliminar, elección de carpeta, rama e identidad; botón explícito para activar la rama.
- `main`, `master` y `trunk` siguen bloqueadas; no existe force-push. Antes de publicar se consulta/fetch la rama remota y se exige fast-forward seguro.
- El publicador histórico V163 se conserva para compatibilidad interna y pruebas; la UI V164 usa el nuevo publicador por perfiles.
...
## V165
- Publicador por perfiles con editor contraído por defecto.
- Creación/edición explícita de perfiles y autodetección desde un clon Git local.
- Consola de publicación más visible en modo ventana y seguimiento del progreso del push.
- Conserva el publicador seguro de raíz completa introducido en V164.


## V166
- Centro de actualizaciones: corrige la superposición entre la tarjeta de Seguridad/Recuperación y la barra inferior de acciones.
- El bloque de estado deja de estirarse artificialmente para ocupar casi toda la ventana cuando no hay una release disponible.
- Publicar: el botón deja de decir “X cambios” y pasa a “X archivos” para reflejar lo que realmente cuenta Git.
- La consola aclara que el total corresponde a rutas de archivo pendientes y muestra nuevos/modificados/eliminados por separado.
- Se conserva sin cambios la arquitectura de perfiles y publicación segura de V165.

## V167
- Publicación de desarrollo automatizada con GitHub Actions en cada push a `*/corepulse-dev`.
- Generación automática de `CorePulse_VN.zip` y SHA-256.
- Creación/actualización de prerelease `VN-dev` compatible con el canal Desarrollo.

## V168
- El actualizador en modo fuente instala la nueva versión como carpeta hermana (`CorePulse_VN`) junto a la versión actual.
- La versión en ejecución no se sobrescribe ni se modifica durante la actualización.
- Tras verificar SHA-256, CorePulse prepara la nueva carpeta, cierra la versión actual y abre automáticamente la nueva.
- Se elimina el botón y flujo manual de Rollback del Centro de actualizaciones.
- Se elimina la etapa visible de “copia de prueba”; el flujo queda en Buscar → Descargar/verificar → Actualizar/reiniciar.
- Si ya existe una carpeta destino no creada por el actualizador, CorePulse no la sobrescribe.
- Publicar, perfiles Git y GitHub Actions permanecen intactos.

## V170
- Test de Audio: tono L/R/Ambos reforzado de 0.08 a 0.32 de amplitud pico (~4×) para mejorar audibilidad.
- Mantiene envolvente suave y margen frente a clipping; no toca el volumen maestro ni el volumen por aplicación de Windows.
- Se conserva aislamiento real de canales y el flujo de confirmación/evidencia existente.
- Resto de módulos sin cambios funcionales respecto de V169.


## V171
- Primera compatibilidad Linux portable/fuente sin degradar el comportamiento Windows.
- Proveedor de sensores Linux: psutil + hwmon/sysfs, DRM y `nvidia-smi` cuando están disponibles.
- Identidad de equipo/CPU/GPU/almacenamiento adaptada a DMI sysfs, `/proc/cpuinfo`, DRM y `lsblk`.
- Salud SMART Linux mediante `smartctl`, sin convertir PASS/Healthy en porcentajes inventados.
- Test de Audio Linux mediante ALSA (`aplay`/`arecord`) con el mismo tono 0.32 de V170 y sin tocar el volumen maestro.
- Arranque y readiness aceptan Linux; dependencias Win32 dejan de ser obligatorias fuera de Windows.
- Tweaks Windows 11 y Gaming/RTSS/PresentMon/Game Boost quedan explícitamente N/A en Linux V171.
- Actualizador side-by-side espera correctamente el cierre del proceso también en Linux antes de relanzar la nueva versión.
- Nuevos scripts `Instalar_CorePulse_Linux.sh` y `Ejecutar_CorePulse_Linux.sh`, más `requirements-linux.txt` y guía `LINUX.md`.


## V172
- Linux deja de mostrar módulos exclusivos de Windows como N/A: se ocultan de la navegación y del Centro de salud.
- Tweaks Windows 11 no aparece en el sidebar Linux.
- Centro de salud Linux omite Windows, Reparación DISM/SFC y Rendimiento/Gaming; Recuperación conserva sólo la recuperación de sesión compatible.
- Benchmark Linux ofrece únicamente CPU, RAM y SSD mientras la escena GPU 3D dependa de Win32.
- Autodiagnóstico Linux omite PresentMon, RTSS y PowerShell; N/A queda reservado a capacidades compatibles pero ausentes.
- Windows conserva exactamente sus módulos y navegación existentes.


## V173
- Linux reemplaza el acceso de `Tweaks Windows 11` por `Ajustes Linux`; nunca vuelve a mostrar texto o diálogo de Tweaks de Windows.
- Nuevo panel Linux seguro: perfil energético vía `powerprofilesctl` cuando existe, lectura del governor CPU, swappiness, zram y detección de GameMode. No aplica sysctl ni cambios de kernel automáticamente.
- Se corrigen gutters derecho/inferior de la ventana Linux que podían verse como líneas negras.
- Galería de Temas rediseñada: tarjetas con paleta, filtros Todos/Oscuros/Claro, indicador de tema actual/seleccionado y vista previa más completa.
- Windows conserva Tweaks Windows 11 sin cambios funcionales.
## V174
- Ajustes Linux deja de duplicar Perfil de energía, CPU, Memoria y GameMode en cada refresco; todas las tarjetas se reconstruyen dentro de un único contenedor limpio.
- El cambio Ahorro/Equilibrado/Rendimiento se refleja visualmente de inmediato y luego se verifica contra power-profiles-daemon sin bloquear la selección visible.
- Temas amplía el catálogo de 10 a 15 paletas: 9 oscuras y 6 claras. Se añaden Cielo, Menta, Arena, Lavanda y Perla.
- El filtro `Claros` selecciona automáticamente una vista previa compatible y evita mostrar un tema oscuro oculto por el filtro.
- Sin cambios funcionales en Windows, Publicar, Actualizaciones, Benchmark, Diagnóstico, Audio u Overlay.

## V176
- Linux puede iniciar CorePulse con doble clic mediante `CorePulse_Linux.desktop` o el lanzador `CorePulse`, sin escribir comandos.
- El primer inicio prepara el entorno automáticamente; si requiere paquetes del sistema, abre una terminal de instalación de forma automática.
- La preparación registra CorePulse en el menú de aplicaciones e instala su icono por usuario.
- Se crea un puntero estable `~/.local/share/CorePulse/current` para que el acceso del menú siga apuntando a la versión activa.
- Las actualizaciones side-by-side de Linux relanzan la versión nueva mediante el lanzador autocurativo y actualizan el puntero estable.
- Windows no cambia funcionalmente.
## V179
- Diagnóstico: las tarjetas de la última fila usan altura natural y dejan de caer/estirarse innecesariamente.
- Reportes: la carpeta de informes se abre también en Linux/macOS con el explorador del sistema.
- Almacenamiento Linux: inventario físico mediante lsblk, incluyendo discos no montados y dual boot; uso de espacio sólo cuando hay volúmenes montados.
- Salud/detalle de discos: Linux deja de intentar consultas `PhysicalDriveN` exclusivas de Windows y reutiliza smartctl nativo.
- Windows conserva su inventario Win32/LHM y las rutas de SMART existentes.


## V182
- Diagnóstico: corrige la transición `RUNNING_AUDIO` que podía abortar la sesión al comenzar la prueba automática de audio.
- Benchmark Linux: los mensajes normales del gestor de ventanas ya no se interpretan como cierre del benchmark; sólo un cierre real del usuario termina la ventana OpenGL.
- Benchmark manual: CPU/RAM/SSD se ejecutan antes de GPU para que la ventana visual sea el tramo final y cierre junto al 100% global.
- Almacenamiento dual boot: un volumen NTFS no montado se identifica explícitamente; usado/disponible sólo se publica cuando el sistema de archivos está montado y puede medirse de forma real.
- Cambios comunes mantienen compatibilidad Windows/Linux; la corrección X11 y la explicación de montaje son específicas de Linux.
