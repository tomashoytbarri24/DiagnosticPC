...
def _style_storage(app, disks_data=None):
    ...
    for idx, widgets in getattr(app, 'disk_widgets', {}).items():
        ...
        # Tarjeta física: las particiones/mountpoints se muestran sólo en detalle.
        ...
        if not parts and total is not None:
            parts.append(f'Capacidad física  {total:.2f} GB')
            if 'unmounted' in str(d.get('usage_scope') or '').casefold():
                fs = ', '.join(str(x) for x in (d.get('unmounted_filesystems') or d.get('filesystems') or []) if str(x).strip())
                parts.append((f'{fs} sin montar' if fs else 'No montado en este sistema'))
                parts.append('Usado/disponible requiere montar el volumen')
