...
import threading, types, time, platform
...
NAV = {'_btn_summary': 'Resumen', 'btn_benchmark': 'Benchmark', 'btn_diagnostic': 'Iniciar diagnóstico', 'btn_health_center': 'Centro de salud', 'btn_cleanup': 'Limpieza de sistema', 'btn_tweaks': ('Tweaks Windows 11' if platform.system() == 'Windows' else 'Ajustes Linux'), 'btn_network': 'Red avanzada', 'btn_smart_alerts': 'Alertas técnicas', 'btn_session_trends': 'Tendencias', 'btn_alert_history': 'Historial de alertas'}
...
def _build_agent_card(app):
    ...
    badge = ctk.CTkLabel(title_row, text='Tiempo real', height=17, font=(FONT, 7, 'bold'), text_color=CYAN, fg_color=role_color('accent_2'), corner_radius=8, padx=7, pady=1)
...
def _storage_height_for(app, mode):
    ...
    if count <= 2:
        cap = {'compact': 126, 'standard': 154, 'large': 164}.get(mode, 154)
        return max(68, min(content, cap))
    cap = {'compact': 132, 'standard': 160, 'large': 174}.get(mode, 160)
...
def _style_storage_widgets(app):
    ...
    for widgets in getattr(app, 'disk_widgets', {}).values():
        ...
        try:
            ...
            pass
        _cfg(card, fg_color=SURFACE_2, border_color=BORDER, border_width=1, corner_radius=11)
        try:
...
def _style_header_mode(app, mode):
    ...
    try:
        ...
        _cfg(header, height=58 if compact else 68 if mode == 'standard' else 72)
    except Exception:
        pass
...
def _style_status_mode(app, mode):
    ...
    compact = mode == 'compact'
    standard = mode == 'standard'
    height = 82 if compact else 100 if standard else 106
...
def _chart_target_height_for(app, mode):
    ...
    ratio = 0.29 if mode == 'compact' else 0.37 if mode == 'standard' else 0.40
    target = int(height * ratio)
    minimum = 230 if mode == 'compact' else 326 if mode == 'standard' else 352
...
def _style_charts(app):
    _cfg(app.frame_charts, fg_color=SURFACE_2, border_color=BORDER, border_width=1, corner_radius=12)
    try:
        max_points = max(2, int(getattr(app, 'max_points', 25) or 25))
        ticks = _chart_tick_positions(max_points)
        labels = ['-60s', '-45s', '-30s', '-15s', 'Ahora']
        app.fig.set_facecolor(SURFACE_2)
...
def _apply_layout(app, *, force=False):
    ...
    meter_h = 100 if mode == 'compact' else 118 if mode == 'standard' else 126
