...
from core.platform_open import open_url_foreground
from core.update_manager import (
    ...
    launch_source_update_helper, load_preferences, open_updates_folder,
    prepare_side_by_side_source_update, releases_web_url, save_preferences,
...
class UpdatePanel:
    ...
    def __init__(self, app, parent):
        ...
        self.asset = None
        self.download = None
        self.staged = None  # compatibilidad histórica; V168 ya no usa copia de prueba
        self.busy = False
        ...
        self.backup_var = ctk.StringVar(value="La versión actual se conservará sin cambios.")
    ...
    def _build(self):
        ...
        mode_text = {
            ...
            "source-git": "Desarrollo Git · la nueva versión se instala junto a la actual",
            "source-portable": "Fuente portable · actualización paralela sin reemplazar la versión actual",
        ...
        self._flow_label = ctk.CTkLabel(
            controls,
            text="1 · Buscar  →  2 · Descargar y verificar SHA-256  →  3 · Actualizar y reiniciar",
        ...
        ctk.CTkLabel(safety, text="SEGURIDAD DE ACTUALIZACIÓN", font=(FONT, 8, "bold"), text_color=_c("muted"), anchor="w").pack(fill="x", padx=14, pady=(8, 2))
        ...
        if self.mode in {"source-portable", "source-git"}:
            try:
                from core.runtime_paths import source_root
                parent = source_root().resolve().parent
                self.backup_var.set(
                    f"La nueva versión se instalará junto a {VERSION_LABEL} en {parent}. "
                    "La carpeta actual no se sobrescribe."
                )
            except Exception:
                self.backup_var.set("La nueva versión se instalará junto a la actual sin sobrescribirla.")
        else:
            self.backup_var.set("Modo instalado: CorePulse verificará el instalador antes de abrirlo.")
        ...
        ctk.CTkButton(actions, text="Descargas", width=100, height=36, fg_color="transparent", hover_color=_c("surface_2"), border_width=1, border_color=_c("border"), text_color=_c("text_2"), command=self.open_folder).grid(row=0, column=1, sticky="w", padx=(8, 0))
        ctk.CTkButton(actions, text="Releases", width=100, height=36, fg_color="transparent", hover_color=_c("surface_2"), border_width=1, border_color=_c("border"), text_color=_c("text_2"), command=lambda: open_url_foreground(releases_web_url())).grid(row=0, column=2, sticky="w", padx=(8, 0))
        ...
        ctk.CTkButton(actions, text="Volver al resumen", width=130, height=36, fg_color="transparent", hover_color=_c("surface_2"), text_color=_c("text_2"), command=self.close).grid(row=0, column=5, sticky="e")

        previous = read_last_update_status()
        if isinstance(previous, dict):
            if previous.get('ok'):
                self.backup_var.set('Última actualización iniciada correctamente. Versión en ejecución: ' + VERSION_LABEL)
            else:
                self.backup_var.set('Última actualización no completada: ' + str(previous.get('error') or 'La versión anterior permanece disponible en su carpeta.'))
    ...
    def _channel_changed(self, _value=None):
        ...
        self.result = self.asset = self.download = None
        self.staged = None
    ...
    def _finish_download(self, downloaded):
        ...
        if is_frozen():
            ...
            self.status_var.set("Paquete verificado. La nueva versión se instalará junto a la actual y CorePulse se reiniciará.")
    ...
    def apply_source_update(self):
        ...
        self._set_busy(True)
        self.primary.configure(state="disabled")
        self.status_var.set("Preparando la nueva versión junto a la actual…")

        def worker():
            try:
                plan = prepare_side_by_side_source_update(self.download)
    ...
    def _finish_apply_plan(self, plan):
        self._set_busy(False)
        target = plan.get("target_root") or ""
        self.backup_var.set(f"Nueva versión preparada en {target}. La versión actual permanece intacta.")
        self.status_var.set("CorePulse se cerrará y abrirá automáticamente la nueva versión.")
        try:
            launch_source_update_helper(plan)
            self.frame.after(350, self._close_app_for_update)
        except Exception as exc:
            self._fail("No se pudo iniciar la nueva versión", exc)
