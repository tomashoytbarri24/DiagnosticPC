...
def _driver_status_label(value):
    raw = str(value or 'N/A').strip().upper()
    return {'DEVICE_PROBLEM':'Problema del dispositivo','UNSIGNED':'No firmado','OLD':'Antiguo · revisar actualización','OK':'Correcto','N/A':'N/A'}.get(raw, str(value or 'N/A'))
...
class HealthCenterPanel:
    ...
    WINDOWS_TABS = (('windows', 'Windows'), ('repair', 'Reparación'))

    def __init__(self, app, host, *, performance_only=False, external_scroll=None, benchmark_only=False, initial_tab=None, initial_windows_section=None, initial_performance_section=None):
        ...
        if self._performance_only:
            ...
            allowed_tabs = {'summary', 'battery', 'history', 'recovery', 'corepulse', 'audio'}
            if platform.system() == 'Windows':
                allowed_tabs.update({'windows', 'repair'})
            self._tab = requested_tab if requested_tab in allowed_tabs else 'summary'
        ...
        self._benchmark_component_flags = {'gpu': platform.system() in {'Windows', 'Linux'}, 'cpu': True, 'ram': True, 'ssd': True}
        ...
        section = str(initial_performance_section or 'home').strip().lower()
        self._performance_section = ('benchmark' if self._benchmark_only else
                                     section if section in ('home', 'library', 'stability', 'boost') else 'home')
        ...
        self._render_pending_direct = False
        self._reset_scroll_on_render = False
        self._dirty_tabs = set()
        self._gaming_metric_labels = {}
    ...
    def _health_module_card(self, parent, row, column, title, description, status, color, command, button_text='Abrir'):
        """Tarjeta compacta del Centro de salud.

        V141 reduce frames/pills puramente decorativos: conserva identidad, estado,
        descripción y acción con menos widgets y menos altura por módulo.
        """
        ...
        top.pack(fill='x', padx=13, pady=(11, 5))

        ctk.CTkLabel(
            top, text=icon_text, font=(FONT, 22, 'bold'), text_color=accent_color,
            anchor='center', justify='center', width=29
        ).pack(side='left', padx=(0, 8))

        text_col = ctk.CTkFrame(top, fg_color='transparent')
        text_col.pack(side='left', fill='x', expand=True)
        ctk.CTkLabel(
            text_col, text=chip_text, font=(FONT, 9, 'bold'), text_color=accent_color,
            anchor='w', justify='left'
        ).pack(fill='x')
        ctk.CTkLabel(
            text_col, text=title, font=(FONT, 14, 'bold'), text_color=TEXT,
            anchor='w', justify='left'
        ).pack(fill='x', pady=(1, 0))

        # Estado visible sin una caja anidada completa: menos widgets y menos alto.
        ctk.CTkLabel(
            top, text=str(status), font=(FONT, 9, 'bold'), text_color=color,
            anchor='e', justify='right', wraplength=155
        ).pack(side='right', padx=(8, 0))

        ctk.CTkLabel(
            card, text=description, font=(FONT, 10), text_color=TEXT2,
            anchor='w', justify='left', wraplength=330
        ).pack(fill='x', padx=13, pady=(0, 9))

        self._button(card, button_text, command, variant='secondary', height=30).pack(
    ...
    def _request_render(self, delay=0):
        ...
        if not self._alive or self._tab == 'audio':
            return
        self._dirty_tabs.add(self._tab)
        ...
        try:
            if self.scroll.is_scrolling():
                requested_tab = self._tab
                self.scroll.defer_until_idle(
                    lambda: self._request_render() if self._tab == requested_tab else None,
                    key='health-render')
                return
        except Exception:
            pass
        ...
        def run():
            self._render_after_id = None
            if (self._alive and self._visible and self._tab == requested_tab
                    and self._tab != 'audio' and self._tab in self._dirty_tabs):
                self._render()
            elif self._alive and self._visible and self._tab in self._dirty_tabs:
                # Una petición de la nueva pestaña pudo quedar agrupada detrás
                # de este callback. No perderla al descartar el destino antiguo.
                self._request_render(1)
    ...
    def _finalize_render(self):
        ...
        self._rendering = False
        if self._render_pending_direct:
            self._render_pending_direct = False
            self._render_pending = False
            # Navegación explícita pendiente, incluso hacia Audio. No pasar por
            # el filtro que impide reconstrucciones incidentales de esa vista.
            self._render()
    ...
    def _render(self):
        if not self._alive:
            return
        if self._rendering:
            self._render_pending_direct = True
            return
        ...
        self._dirty_tabs.discard(self._tab)
        self._restore_scroll_fraction = (0.0 if self._reset_scroll_on_render
                                         else self._capture_scroll_fraction())
        self._reset_scroll_on_render = False
        try:
            self._clear()
            renderer = {
        ...
        module_specs.extend((
            ...
            (
                ...
                ('Puntos de restauración y protección previa a cambios en Windows.'
                 ...
                 'Recuperación de sesión y registro de cierres no limpios.'),
    ...
    def _render_corepulse_diagnostics(self):
        self._title(
            'Estado de CorePulse',
            'Comprueba qué puede usar CorePulse en este equipo. Las funciones propias de otro sistema operativo se ocultan.'
        )
        ...
        ctk.CTkLabel(
            hero, text='Las capacidades compatibles pero ausentes se informan sin penalizar la salud del equipo.',
    ...
    def _render_battery(self):
        ...
        def capacity_text(energy_key, charge_key):
            energy = _num(b.get(energy_key))
            if energy is not None:
                return _fmt(energy, ' mWh', 0)
            charge_mah = _num(b.get(charge_key))
            return _fmt(charge_mah, ' mAh', 0) if charge_mah is not None else 'N/A'

        first_row = (
            ('Capacidad de diseño', capacity_text('designed_capacity_mwh', 'designed_capacity_mah'), CYAN, ''),
            ('Carga completa', capacity_text('full_charge_capacity_mwh', 'full_charge_capacity_mah'), GREEN, ''),
            ('Capacidad restante', capacity_text('remaining_capacity_mwh', 'remaining_capacity_mah'), CYAN, ''),
        )
        second_row = (
    ...
    def _gaming_metric_values(self, live):
    ...
    def _refresh_gaming_home_metrics(self):
        if (not self._alive or not self._visible or self._rendering
                or self._tab != 'performance' or self._performance_section != 'home'):
            return
        for name, text, color in self._gaming_metric_values(self._gaming_live_snapshot()):
            widget = self._gaming_metric_labels.get(name)
            if widget is not None:
                configure_changed(widget, text=text, text_color=color)
    ...
    def _render_gaming_comfort_home(self):
        ...
        for col, (label, value, accent) in enumerate(self._gaming_metric_values(live)):
            ...
            metric_label = ctk.CTkLabel(cell, text=value, font=(FONT, 11, 'bold'), text_color=accent, anchor='w')
            metric_label.pack(fill='x', pady=(2, 0))
            self._gaming_metric_labels[label] = metric_label
    ...
    def _select_performance_section(self, key):
        ...
        if key not in ('home', 'library', 'stability', 'boost'):
            key = 'home'
        if key == self._performance_section:
            if self._tab in self._dirty_tabs:
                self._request_render(1)
            elif key == 'home':
                self._refresh_gaming_home_metrics()
            return
    ...
    @staticmethod
    def _benchmark_available_components():
        return ('gpu', 'cpu', 'ram', 'ssd') if platform.system() in {'Windows', 'Linux'} else ('cpu', 'ram', 'ssd')

    def _benchmark_selected_components(self):
        return [
            key for key in self._benchmark_available_components()
    ...
    def _set_all_benchmark_components(self, enabled):
        if 'visual_benchmark' in self._jobs:
            return
        value = bool(enabled)
        for key in self._benchmark_available_components():
    ...
    def _render_gaming_benchmark_section(self):
        ...
        self._benchmark_component_vars = {}
        for index, key in enumerate(self._benchmark_available_components()):
        ...
        ctk.CTkLabel(
            note,
            text=(
                'El benchmark mide CPU, RAM y SSD; la GPU abre una ventana OpenGL visible. No es una prueba de estrés dedicada. CorePulse registra mediciones reales durante la ejecución.'
                if platform.system() == 'Linux' else
                'El benchmark mide CPU, RAM y SSD; la GPU abre una ventana 3D independiente. No es una prueba de estrés dedicada. CorePulse registra temperatura, uso, frecuencia y resultados reales. Si Windows asigna la escena 3D a otra GPU, CorePulse lo indica en el resultado en vez de ocultarlo.'
            ),
            font=(FONT, 8), text_color=MUTED, anchor='w', justify='left', wraplength=1020,
    ...
    def _render_visual_benchmark_card(self, parent):
        ...
        if platform.system() == 'Windows' and 'gpu' in selected and visual:
        ...
        if 'ssd' in selected:
            metrics.extend([
                ...
                ('SSD · Volumen probado', str(ssd.get('path_root') or 'N/A'), AMBER),
            ])
        ...
        phases = visual.get('phases') if isinstance(visual.get('phases'), list) else (visual.get('phase_results') if isinstance(visual.get('phase_results'), list) else [])
        # La cuadrícula es exclusivamente GPU 3D. Filtramos de forma defensiva
        # para que placeholders CPU/RAM antiguos nunca ocupen una tarjeta real.
        gpu_phase_keys = {'geometry', 'fill', 'textures', 'shaders', 'compute', 'combined'}
        phases = [p for p in phases if isinstance(p, dict) and str(p.get('key') or '').lower() in gpu_phase_keys]
        if phases:
            ctk.CTkLabel(shell, text='GPU · fases 3D medidas', font=(FONT, 9, 'bold'), text_color=TEXT2, anchor='w').pack(fill='x', padx=12, pady=(1, 4))
    ...
    def _run_visual_benchmark(self):
        ...
        def work():
            ...
            # V182: CPU/RAM/SSD se ejecutan primero y la ventana GPU queda para
            # el tramo final. Así la escena visual no desaparece cuando aún
            # resta media suite: al cerrarse, el benchmark completo ya terminó.
            classic_span = (classic_weight / total_weight) if classic_selected else 0.0
            if classic_selected:
                def classic_progress(local, stage, detail=''):
                    push_progress(max(0.0, min(1.0, float(local))) * classic_span, stage, detail)
                ...
                suite_status = str((suite_result or {}).get('status') or '').upper()
                if suite_status == 'SAFETY_STOP':
                    return None, suite_result, time.perf_counter() - started
                if gpu_selected:
                    push_progress(classic_span, 'CPU/RAM/SSD completados', 'Continúa: GPU 3D visible')

            if gpu_selected:
                gpu_base = classic_span
                gpu_span_final = max(0.0, 1.0 - gpu_base)
                def gpu_progress(local, stage, detail=''):
                    push_progress(gpu_base + max(0.0, min(1.0, float(local))) * gpu_span_final,
                                  'GPU · ' + str(stage), detail)
                if platform.system() == 'Linux':
                    gpu_suite = run_benchmark_suite(
                        profile_key, ['gpu'], progress_callback=gpu_progress, telemetry_sampler=telemetry_sample,
                    )
                    gpu_result = (gpu_suite or {}).get('gpu') if isinstance(gpu_suite, dict) else None
                else:
    ...
    def _apply_performance_mode(self, mode):
        ...
        self._async('profile_change', lambda: manager.set_mode(mode), done,
                    on_started=lambda: self._request_render(1))
    ...
    def _render_analyzer(self, title, data, kind):
        ...
        if kind == 'startup':
            ...
            self._kv(card, 'Antiguos de terceros (>5 años) · revisar', data.get('older_than_5y', 0), AMBER if data.get('older_than_5y') else TEXT)
            ctk.CTkLabel(
                card,
                text='“Antiguo” describe la edad del paquete detectado. No significa que CorePulse haya confirmado una actualización disponible en Internet.',
                font=(FONT, 9), text_color=MUTED, anchor='w', justify='left', wraplength=1040,
            ...
            rows = [
                [
                    ...
                    (f"{float(x.get('age_years')):.1f} años" if x.get('age_years') is not None else 'N/A'),
            ...
            self._render_compact_table(
                ...
                columns=[
                    {'title': '#', 'weight': 1, 'wrap': 24},
                    {'title': 'Dispositivo', 'weight': 6, 'wrap': 250},
                    {'title': 'Proveedor', 'weight': 4, 'wrap': 170},
                    {'title': 'Versión', 'weight': 3, 'wrap': 125},
                    {'title': 'Edad aprox.', 'weight': 2, 'wrap': 95},
                    {'title': 'Estado', 'weight': 4, 'wrap': 200},
                ],
                rows=rows,
    ...
    def _render_recovery(self):
        self._title(
            'Restauración y rollback' if platform.system() == 'Windows' else 'Recuperación de sesión',
            ('Protección antes de cambios delicados. CorePulse no habilita Restaurar sistema sin tu autorización.'
             if platform.system() == 'Windows' else
             'Revisa cierres no limpios y acciones de recuperación de la sesión anterior.')
        )
    ...
    def _async(self,name,fn,on_done,on_started=None):
        ...
        def worker():
            ...
            def done():
                ...
                self._dirty_tabs.add(job_tab)
                if self._tab == job_tab:
                    self._request_render(1)
    ...
    def set_active(self, active):
        ...
        if not self._visible and self._render_after_id is not None:
            try:
                self.app.after_cancel(self._render_after_id)
            except Exception:
                pass
            self._render_after_id = None
        elif self._visible and self._tab in self._dirty_tabs:
            self._request_render(1)
