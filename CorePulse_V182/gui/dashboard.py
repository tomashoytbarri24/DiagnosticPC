...
SIDEBAR_LABELS = {
    ...
    'btn_tweaks': ('Tweaks Windows 11' if platform.system() == 'Windows' else 'Ajustes Linux'),
...
def _rebuild_sidebar(app):
    ...
    _safe_config(app.btn_tweaks, text=('Tweaks Windows 11' if platform.system() == 'Windows' else 'Ajustes Linux'))
    ...
    def wrapped(self, disks_data):
        ...
        for idx, widgets in getattr(self, 'disk_widgets', {}).items():
            ...
            if isinstance(total_raw, (int, float)) and float(total_raw) > 0:
                title += f'   ·   {float(total_raw):.2f} GB'
            # El resumen representa discos físicos, no particiones ni la lista de
            # subvolúmenes/mountpoints. Esa información queda en Ver detalles.
            _safe_config(
            ...
            try:
                ...
                elif isinstance(total_raw, (int, float)) and float(total_raw) > 0:
                    scope = str(d.get('usage_scope') or '').casefold()
                    capacity = f'Capacidad física {float(total_raw):.2f} GB'
                    if 'unmounted' in scope:
                        capacity += '  ·  No montado en este sistema'
                else:
