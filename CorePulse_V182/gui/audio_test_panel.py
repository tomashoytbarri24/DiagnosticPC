...
class AudioTestPanel(ctk.CTkFrame):
    def __init__(self, master, service=None):
        ...
        self.label('Prueba manual local · Tono reforzado sin modificar el volumen maestro de Windows.', 12)
    ...
    def poll(self):
        ...
        latest_level = None
        try:
            # Procesar una ráfaga acotada y pintar sólo su nivel más reciente.
            # La evidencia de picos se calcula en el servicio, no en esta barra.
            for _ in range(128):
                ...
                if kind == 'level':
                    ...
                    continue
                self.busy = False
                configure_changed(self.status, text=('Error · ' + value) if kind == 'error' else
        ...
        if latest_level is not None:
            set_changed(self.level, min(1., latest_level))
            configure_changed(self.level_text, text=f'Amplitud real: {latest_level:.4f} (0–1)')
    ...
    def update_state(self):
        ...
        configure_changed(self.output, text='SALIDA · ' + (output.get('name') or 'N/A') +
                              ('\n' + output.get('error', 'No disponible') if not output.get('available') else ''))
        configure_changed(self.input, text='MICRÓFONO · ' + (source.get('name') or 'N/A') +
                             ('\n' + source.get('error', 'No disponible') if not source.get('available') else ''))
        configure_changed(self.stereo, text='Canales L/R disponibles según Windows.' if output.get('stereo') else 'Prueba estéreo no disponible · N/A')
        for widget in self.actions:
            configure_changed(widget, state='normal')
        for button in self.actions[:2]:
            configure_changed(button, state='normal' if output.get('available') and output.get('stereo') else 'disabled')
        configure_changed(self.actions[2], state='normal' if output.get('available') else 'disabled')
        configure_changed(self.record_button, state='normal' if source.get('available') else 'disabled')
        configure_changed(self.replay, state='normal' if self.service.recording and output.get('available') else 'disabled')
        pending = self.service.pending
        configure_changed(self.question, text=('¿La grabación se escucha correctamente?' if pending == 'microphone' else
                                      '¿Escuchaste correctamente este canal?') if pending else '')
        for button in self.answers:
            configure_changed(button, state='normal' if pending else 'disabled')
        from core.audio_test import summary
        rows = '\n'.join(f'{key}: {value}' for key, value in evidence_rows(self.service.result))
        configure_changed(self.summary, text='RESUMEN · ' + summary(self.service.result) + '\n' + rows)
