...
def _phase_labels():
    return ('Escritorio', 'Windows', 'Benchmark', 'Bajo carga') if IS_WINDOWS else ('Escritorio', 'Hardware', 'Benchmark', 'Bajo carga')


def _phase_statuses(result):
    complete = result.get('complete_diagnostic') if isinstance(result.get('complete_diagnostic'), dict) else {}
    phases = complete.get('phases') or {}
    keys = ('desktop', 'windows', 'benchmark', 'load_observation') if IS_WINDOWS else ('desktop', 'hardware', 'benchmark', 'load_observation')
    return [(label, str((phases.get(key) or {}).get('status') or 'N/A')) for key, label in zip(keys, _phase_labels())]


def _running_phase_values(state):
    """Actividad real del worker; no muestra una fase Windows en Linux."""
...
def _complete_interpretation(result):
    ...
    if status == 'NORMAL':
        return ('Escritorio, Windows y benchmark finalizaron sin anomalías observadas entre las áreas evaluables. El PDF es opcional.' if IS_WINDOWS else 'Escritorio, hardware y benchmark finalizaron sin anomalías observadas entre las áreas evaluables. El PDF es opcional.')
...
class DiagnosticExperiencePanel(ctk.CTkFrame):
    ...
    def _apply_responsive_layout(self, force=False):
        ...
        try:
            ...
            # V181: no volver a mapear la superficie Evidencia durante un cambio de layout.
            # grid_configure() puede hacer reaparecer un widget previamente grid_remove() en Tk.
            if self._evidence_open_key:
                self.evidence_card.grid_configure(columnspan=columns)
            else:
                self.evidence_card.grid_remove()
            for idx, key in enumerate((('cpu', 'gpu', 'ram', 'storage', 'battery', 'windows', 'audio') if IS_WINDOWS else ('cpu', 'gpu', 'ram', 'storage', 'battery', 'audio'))):
            ...
            # Todas las tarjetas de una misma fila ocupan la misma altura visual.
            used_rows = {2 + (idx // columns) for idx in range(len((('cpu', 'gpu', 'ram', 'storage', 'battery', 'windows', 'audio') if IS_WINDOWS else ('cpu', 'gpu', 'ram', 'storage', 'battery', 'audio'))))}
            for row_index in used_rows:
                self.component_report.grid_rowconfigure(row_index, weight=0)
        except Exception:
            pass
    ...
    def _build_body(self):
        ...
        for idx, key in enumerate((('cpu', 'gpu', 'ram', 'storage', 'battery', 'windows', 'audio') if IS_WINDOWS else ('cpu', 'gpu', 'ram', 'storage', 'battery', 'audio'))):
            ...
            card.grid(row=row, column=col, sticky='new', padx=(0 if col == 0 else 4, 4 if col == 0 else 0), pady=4)
            card.grid_columnconfigure(0, weight=1)
            ...
            self._component_cards[key] = {
                ...
                'facets_frame': facets, 'facets': facet_refs, 'evidence': evidence,
        ...
        self.btn_repeat = ctk.CTkButton(self.result_actions, text='Diagnosticar de nuevo', width=146, height=34, corner_radius=8, fg_color=theme_color('#17263a'), hover_color=theme_color('#203650'), border_width=1, border_color=BORDER, text_color=TEXT, font=('Segoe UI', 8, 'bold'), command=self._repeat)
    ...
    def _show_component_evidence(self, key, result):
        ...
        def _meaningful_section(section):
            rows = [row for row in (section.get('rows') or []) if isinstance(row, dict)]
            empty = {'', 'N/A', 'NA', 'NONE', 'NO EVALUABLE', 'SIN DATOS', 'UNKNOWN', '—', '-'}
            return any(str(row.get('value') or '').strip().upper() not in empty for row in rows)

        sections = [x for x in (detail.get('sections') or []) if isinstance(x, dict) and _meaningful_section(x)]
        status = str(detail.get('status') or 'NO_EVALUABLE').upper()
        if status not in {'WARNING', 'CRITICAL'} or not sections:
            # Evidencia es trazabilidad de un hallazgo real. Nunca se muestra
            # una tarjeta vacía/N/A ni para componentes normales o informativos.
    ...
    def _show_component_report(self, result):
        ...
        try:
            ...
            pass
        try:
            self.lbl_details_title.configure(text=('RESULTADO INTEGRAL · ESCRITORIO + BENCHMARK + TELEMETRÍA BAJO CARGA + WINDOWS' if IS_WINDOWS else 'RESULTADO INTEGRAL · ESCRITORIO + HARDWARE + BENCHMARK + TELEMETRÍA'))
            self.component_report.grid(row=1, column=0, columnspan=2, sticky='ew', pady=(2, 4))
        ...
        if priority:
            ...
            self.lbl_priority_text.configure(
                text='Sin acciones prioritarias basadas en la evidencia disponible. Las mediciones parciales se muestran sólo cuando aportan datos útiles.',
                text_color=DIM,
            )
        ...
        if isinstance(comparison, dict) and comparison.get('available'):
            ...
            if changes:
                ...
                self.lbl_comparison.configure(text=('Comparación anterior: sin cambios de estado en CPU/GPU/RAM/almacenamiento/batería/Windows.' if IS_WINDOWS else 'Comparación anterior: sin cambios de estado en CPU/GPU/RAM/almacenamiento/batería.'), text_color=MUTED)
        else:
        ...
        for report in reports:
            ...
            display_facets = list(report.get('display_facets') or [])
            evidence = list(report.get('evidence') or [])
            has_partial_data = bool(display_facets or evidence)
            if status == 'NO_EVALUABLE':
                badge_text = 'PARCIAL' if has_partial_data else 'SIN DATOS'
                color = CYAN if has_partial_data else DIM
            else:
                badge_text = badge_labels.get(status, status)
                color = STATUS_COLORS.get(status, DIM)
            refs['title'].configure(text=str(report.get('title') or key).upper())
            refs['badge'].configure(text=badge_text, text_color=color)
            ...
            # V178: las tres facetas permanecen visibles. Si un dato no puede
            # obtenerse de ninguna fuente real se conserva N/A; nunca se oculta.
            for idx, pair in enumerate(refs.get('facets') or []):
                title_label, value_label = pair
                cell = title_label.master
                item = display_facets[idx] if idx < len(display_facets) else ('—', 'N/A', 'NO_EVALUABLE')
                label = str(item[0] if len(item) > 0 else '—')
                value = str(item[1] if len(item) > 1 else 'N/A')
                facet_status = str(item[2] if len(item) > 2 else 'INFO').upper()
                cell.grid()
                title_label.configure(text=label.upper())
                value_label.configure(text=value, text_color=facet_color.get(facet_status, DIM))
            refs['facets_frame'].grid()

            # Evidencia = trazabilidad de un hallazgo. Para componentes
            # normales/informativos sólo duplicaba las métricas visibles.
            finding_visible = status in {'WARNING', 'CRITICAL'}
            if finding_visible and evidence:
                ...
                refs['evidence'].configure(text='')
                refs['evidence'].grid_remove()
            ...
            try:
                ...
                for section in (detail.get('sections') or []):
                    if not isinstance(section, dict):
                        continue
                    if any(str(row.get('value') or '').strip().upper() not in empty
                           for row in (section.get('rows') or []) if isinstance(row, dict)):
                        meaningful = True
                        break
                has_evidence_detail = bool(finding_visible and meaningful)
            except Exception:
                has_evidence_detail = False
            if has_evidence_detail:
                refs['evidence_action'].configure(
                    text='Ver evidencia',
                    command=lambda k=key, r=result: self._show_component_evidence(k, r),
                    state='normal'
                )
                refs['evidence_action'].grid()
            else:
                refs['evidence_action'].grid_remove()
            refs['action'].configure(
                text=str(report.get('action_text') or 'Ver detalle'),
                command=action, state='normal' if callable(action) else 'disabled'
            )
            if has_evidence_detail:
                refs['action'].grid_configure(column=1, columnspan=1, padx=(3, 0))
            else:
                refs['action'].grid_configure(column=0, columnspan=2, padx=0)
    ...
    def update_complete_progress(self, info):
        ...
        self.lbl_quality.configure(text=('Escritorio, Windows, rendimiento y telemetría durante benchmark conservan su evidencia.' if IS_WINDOWS else 'Escritorio, hardware, rendimiento y telemetría conservan su evidencia.'))
        ...
        if self._observed_title_labels:
            for label, name in zip(self._observed_title_labels, _phase_labels()):
                label.configure(text=name)
        phase_values = _running_phase_values(getattr(getattr(self.app, '_diagnostic_run', None), 'state', None))
        self._set_observed_values(list(zip(_phase_labels(), phase_values)))
