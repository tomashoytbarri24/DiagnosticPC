...
from gui.internal_navigation import activate_internal_page, commit_internal_page, attach_internal_page_panel, abort_internal_page, show_dashboard
...
class StorageDetailPanel:
    ...
    def _build(self):
        ...
        hero = build_title_block(
            titles,
            eyebrow='Almacenamiento · SMART y confiabilidad',
            title='Detalles del almacenamiento',
            subtitle='Cargando identidad real de la unidad…',
            accent=CYAN,
            badges=(('SMART', GREEN), ('NVMe/SATA', CYAN), ('REAL_OR_NA', AMBER)),
            title_size=19,
        )
    ...
    def refresh_reliability(self):
        ...
        self._job = job
        self.lbl_loading.configure(text=('Consultando SMART / confiabilidad de Windows…' if platform.system() == 'Windows' else 'Consultando SMART / salud de almacenamiento…'))
        ...
        def worker():
            try:
                ...
                if platform.system() == 'Windows' and physical_index is not None:
                    native = query_nvme_health_log(physical_index)
                    smartctl = query_smartctl_health(physical_index)
                    job['nvme_smart'] = merge_health_sources(native, smartctl)
                # Linux obtiene SMART desde get_storage_health(), que consulta
                # directamente /dev/* con smartctl. No se usa PhysicalDriveN.
    ...
    def _poll_job(self, telemetry, disks, generation):
        ...
        elif snapshot.get('smartctl_available'):
            self.lbl_loading.configure(text='Salud NVMe leída mediante smartctl / smartmontools.')
    ...
    def _render(self, data):
        ...
        remaining_life = _number(data.get('remaining_life_percent'))
        ...
        elif remaining_life is not None:
            health_text = f'{remaining_life:.0f}%'
            health_color = _health_color(remaining_life)
            health_detail = f"Vida restante derivada · Fuente: {_display_status(data.get('remaining_life_source'))}"
...
def show_storage_details(app, disk_index):
    """Abre almacenamiento publicando primero un shell liviano."""
    host, reused = activate_internal_page(app, 'storage_details')
    if reused:
        panel = getattr(app, 'storage_detail_panel', None)
        if panel is not None:
            defer = getattr(app, '_defer_ui_call', None)
            if callable(defer):
                defer(lambda: panel.select_disk(disk_index), 1)
            else:
                try:
                    app.after(1, lambda: panel.select_disk(disk_index))
                except Exception:
    ...
    shell = ctk.CTkFrame(host, fg_color=BG, corner_radius=0)
    shell.place(relx=0, rely=0, relwidth=1, relheight=1)
    box = ctk.CTkFrame(shell, fg_color=CARD, border_width=1, border_color=BORDER, corner_radius=12)
    box.place(relx=0.5, rely=0.46, anchor='center')
    ctk.CTkLabel(
        box, text='Cargando información de almacenamiento…',
        font=(FONT, 15, 'bold'), text_color=TEXT,
    ).pack(padx=34, pady=(22, 5))
    ctk.CTkLabel(
        box, text='Preparando capacidad, identidad y estado SMART',
        font=(FONT, 9), text_color=MUTED,
    ).pack(padx=34, pady=(0, 22))

    if not commit_internal_page(app, 'storage_details', host, panel=None):
        return None
    ...
    def build_panel():
        ...
        try:
            ...
            if not attach_internal_page_panel(app, 'storage_details', host, panel):
                panel.destroy()
                return
            try:
                ...
                if shell.winfo_exists():
                    for child in shell.winfo_children():
                        try:
                            ...
                            pass
                    ctk.CTkLabel(
                        shell, text='No se pudo preparar esta vista.',
                        font=(FONT, 13, 'bold'), text_color=RED,
                    ).place(relx=0.5, rely=0.46, anchor='center')
            except Exception:
                pass
            log = getattr(app, '_log_throttled_exception', None)
            if callable(log):
                try:
                    log('storage_detail_build', 'No se pudo construir la vista avanzada de almacenamiento')
                except Exception:
                    pass

    defer = getattr(app, '_defer_ui_call', None)
    if callable(defer):
        defer(build_panel, 16)
    else:
        try:
            app.after(16, build_panel)
        except Exception:
            build_panel()
    return None
