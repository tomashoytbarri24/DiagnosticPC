...
def test_inventory_states():
    assert _inventory_states_ok()


def test_history_reader():
    assert _history_reader_ok()
...
def main():
    ...
    checks = [
        ...
        check('inventory_state_machine', _inventory_states_ok()),
        check('history_order_and_labels', _history_reader_ok()),
    ]
    ok = all(checks)
