...
def main():
    ...
    checks=[
        ...
        check('tweak_row_hover', '_bind_row_hover' in tweaks and 'paint(hover)' in tweaks and 'paint(normal)' in tweaks),
        check('dashboard_single_visible_redraw', 'canvas.draw_idle()' in nav and '            canvas.draw()' not in nav.split('def _redraw_dashboard',1)[1].split('def show_dashboard',1)[0]),
    ]
    ok=all(checks)
