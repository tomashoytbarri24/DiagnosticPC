...
class UpdaterTests(unittest.TestCase):
    ...
    def test_stable_release_on_second_page_is_found(self):
        calls = []
        current = int(VERSION)
        preview_tag = f'V{current + 2}'
        stable_tag = f'V{current + 1}'
        def opener(req, **_):
            calls.append(req.full_url)
            first = len(calls) == 1
            response = Response(json.dumps([{'tag_name': preview_tag if first else stable_tag,
        ...
        self.assertEqual(result['latest'].tag, stable_tag)
...
@unittest.skipUnless(importlib.util.find_spec("customtkinter") is not None, "customtkinter no instalado")
