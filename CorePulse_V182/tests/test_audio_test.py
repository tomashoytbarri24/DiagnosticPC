...
class AudioTests(unittest.TestCase):
    ...
    def test_pipeline_delegates_automatic_audio_probe(self):
        ...
        self.assertIn('automatic_audio_probe', names)
        self.assertNotIn('AudioTest', names)
        self.assertNotIn('WindowsAudio', names)
        # El orquestador no abre streams por su cuenta; el backend queda aislado
        # detrás de automatic_audio_probe.
        for node in ast.walk(tree):
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute):
                self.assertNotIn(node.func.attr, ('record', 'play'))

    def test_complete_pipeline_uses_automatic_audio_result(self):
        ...
        bench = {
            'status': 'OK',
            'cpu': {'status': 'OK', 'telemetry': {'during_sample_count': 1, 'cpu_usage': {'samples': 1}}},
            'ram': {'status': 'OK', 'telemetry': {'during_sample_count': 1, 'ram_usage': {'samples': 1}}},
            'ssd': {'status': 'OK', 'telemetry': {}},
            'gpu': {'status': 'OK', 'telemetry': {'during_sample_count': 1, 'gpu_usage': {'samples': 1}}},
        }
        with Rig() as rig, patch('core.complete_diagnostic.run_benchmark_suite', return_value=bench), \
                patch('core.complete_diagnostic.automatic_audio_probe', return_value=auto):
            result = rig.run()
        self.assertEqual(result['audio_test']['status'], 'AUDIO TÉCNICO OK')
        self.assertEqual(result['complete_diagnostic']['audio_test']['mode'], 'AUTOMATIC_TECHNICAL')
...
class NativeFormatTests(unittest.TestCase):
    ...
    def test_left_and_right_samples_are_truly_isolated(self):
        ...
        for channel, silent in [('left', 1), ('right', 0)]:
            ...
            self.assertGreater(max(abs(x) for x in values[1 - silent::2]), .30)
            self.assertLessEqual(max(abs(x) for x in values), .321)
