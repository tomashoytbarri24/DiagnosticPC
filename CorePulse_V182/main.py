...
class App(ctk.CTk):
    ...
    def show_last_pdf_report_folder(self):
        """Abre la carpeta autoritativa de informes en Windows, Linux o macOS."""
        try:
    ...
    def __init__(self):
        ...
        self.btn_tweaks = ctk.CTkButton(self.sidebar, text=('Tweaks Windows 11' if IS_WINDOWS else 'Ajustes Linux'), fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#f4f7fb'), font=('Segoe UI', 11, 'bold'), height=34, corner_radius=8, command=self.open_platform_tweaks)
        ...
        # V173: en Linux se eliminan los gutters derecho/inferior que Tk/CTk
        # dejaba visibles como líneas negras. Conservamos aire a la izquierda/arriba.
        if IS_LINUX:
            self.main_content.grid(row=0, column=1, sticky='nsew', padx=(15, 0), pady=(15, 0))
        else:
    ...
    def _background_services_worker(self):
        ...
        try:
            ...
            if IS_WINDOWS:
                ...
                payload['game_detector'] = None
                payload['performance_manager'] = None
        except Exception as exc:
    ...
    def open_platform_tweaks(self):
        """Abre la optimización nativa del sistema operativo actual."""
        if IS_WINDOWS:
            return self.open_windows_tweaks()
        if not self.is_running:
            return
        host = None
        try:
            ...
            if reused and self.windows_tweaks_panel is not None:
                refresh = getattr(self.windows_tweaks_panel, 'refresh', None)
                if callable(refresh):
                    self._defer_ui_call(refresh)
                return
            if host is None:
                return
            from gui.linux_tuning_panel import LinuxTuningPanel
            panel = LinuxTuningPanel(self, host)
            # Se conserva la referencia histórica para no romper la caché/navegación.
            ...
            cp_error(self, 'Ajustes Linux', f'No se pudo abrir la vista de Ajustes Linux:\n\n{exc}')
    ...
    def open_windows_tweaks(self):
        ...
        if not self.is_running:
            return
        if not IS_WINDOWS:
            return self.open_platform_tweaks()
        host = None
        try:
            host, reused = activate_internal_page(self, 'tweaks')
            if reused and self.windows_tweaks_panel is not None:
    ...
    def open_gaming(self, tab='performance'):
        ...
        if not self.is_running:
            return
        if not IS_WINDOWS:
            cp_info(self, 'Gaming', 'Gaming/Overlay sólo está disponible en Windows en esta versión.')
            return
        if not self._services_ready:
    ...
    # V161: un solo botón coordina observación, Windows y benchmark con telemetría.
    ...
    def _finish_diagnostic_session(self):
        """Congela la fase pasiva y continúa con el diagnóstico completo V142."""
        ...
        self._complete_diagnostic_stage = 'Hardware y Windows' if IS_WINDOWS else 'Hardware y batería'
        ...
        try:
            if panel is not None and panel.winfo_exists():
                panel.update_complete_progress({'progress': 0.22, 'stage': 'Estado en escritorio completado', 'detail': ('Continúa: hardware → Windows → benchmark y telemetría.' if IS_WINDOWS else 'Continúa: hardware → benchmark y telemetría.')})
        except Exception:
            pass
    ...
    def _complete_diagnostic_finished(self, result, error=None, state=None, run_token=None):
        ...
        self.btn_pdf.configure(state='normal', text='Generar PDF · opcional')
        refresh_navigation_state(self)
        panel = getattr(self, 'diagnostic_experience_panel', None)
        panel_visible = False
        try:
        ...
        if not panel_visible:
            ...
            message = (
                ...
                f'Fases: escritorio + hardware + Windows + benchmark y telemetría\n\n'
    ...
    def _refresh_trend_axis_labels(self, visible_count):
        ...
        try:
            ...
            while len(deduped) < 5:
                deduped.append(last)
            axes = (self.ax_cpu, self.ax_ram, self.ax_gpu)
            signature = (axes, tuple(deduped[:5]), tuple(labels))
            if signature == getattr(self, '_trend_axis_signature', None):
                return
            for ax in axes:
                if ax is None:
                    continue
            ...
            self._trend_axis_signature = signature
            # Las etiquetas forman parte del fondo del blit: regenerarlo sólo
            # cuando cambia la escala temporal, no en cada tick de interfaz.
            self.background = None
        except Exception:
            pass
    ...
    def update_charts_fast(self):
        ...
        if not self._charts_ready or self.canvas is None:
            return
        try:
            # La adquisición y el historial continúan aunque el gráfico no sea
            # visible. Sólo se omite su trabajo de presentación.
            visible = not getattr(self, '_active_internal_page', None) and self.winfo_viewable()
            if visible and not self.is_resizing:
                with self.telemetry_lock:
                    revision = getattr(self, '_chart_history_revision', 0)
                    changed = revision != getattr(self, '_chart_rendered_revision', None)
                    if changed:
                ...
                if changed:
                    ...
                    self._chart_rendered_revision = revision
            ...
            self.background = None
            self._chart_rendered_revision = None
        if self.is_running:
            self.chart_after_id = self.after(200, self.update_charts_fast)
    ...
    def _build_fast_disk_snapshot(self, telemetry):
        ...
        result = []
        # La UI debe representar todos los discos físicos conocidos. En Windows
        # esto conserva LHM + Win32; en Linux añade lsblk, incluyendo discos no
        # montados (p. ej. el disco de Windows en un dual boot).
        inventory = telemetry.get('_storage_inventory') if isinstance(telemetry.get('_storage_inventory'), list) else []
        sensor_devices = telemetry.get('_storage_devices') if isinstance(telemetry.get('_storage_devices'), list) else []
        devices = inventory or sensor_devices
        mount_points = []
        try:
        ...
        for idx, device in enumerate(devices):
            try:
                ...
                if free is None:
                    free_bytes = os_inventory.get('mounted_free_bytes')
                    try:
                        free = float(free_bytes) / (1024 ** 3) if free_bytes is not None else None
                    except Exception:
                        free = None
                used_percent = device.get('used_space_percent')
                if used_percent is None:
                    used_percent = os_inventory.get('used_space_percent')
                ...
                mounts_text = (
                    enrichment.get('mount_points')
                    or device.get('mount_points')
                    or os_inventory.get('mount_points')
                    or (', '.join(mount_points) if len(devices) == 1 and mount_points else 'N/A')
                )
                ...
                result.append({
                    ...
                    'temperature_c': temp,
                    'system_disk': bool(device.get('system_disk') or os_inventory.get('system_disk')),
                    'device_id': device.get('device_id') or os_inventory.get('device_id'),
                    'usage_scope': device.get('usage_scope') or os_inventory.get('usage_scope'),
                    'usage_reason': device.get('usage_reason') or os_inventory.get('usage_reason'),
                    'filesystems': device.get('filesystems') or os_inventory.get('filesystems') or [],
                    'unmounted_filesystems': device.get('unmounted_filesystems') or os_inventory.get('unmounted_filesystems') or [],
            ...
            while self.is_running:
                ...
                try:
                    ...
                    with self.telemetry_lock:
                        ...
                        self._chart_history_revision = getattr(self, '_chart_history_revision', 0) + 1
