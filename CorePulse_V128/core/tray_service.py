...
def _icon():
    ...
    if Image is not None:
        try:
            path = resource_path('assets', 'CorePulseSymbol.png')
            if path.exists():
...
class CorePulseTray:
    ...
    def start(self):
        ...
        def clean_ram(*_):
            # Acción directa: no abre ni restaura la ventana principal.
            self._dispatch('clean_ram_from_tray')
        ...
        self.icon = pystray.Icon(
            'CorePulse', icon_image, self._last_title,
            pystray.Menu(
                pystray.MenuItem('Abrir CorePulse', show, default=True),
                pystray.Menu.SEPARATOR,
                pystray.MenuItem('Limpiar RAM ahora', clean_ram),
                pystray.MenuItem('Configurar Overlay', overlay),
                pystray.MenuItem('Alertas y diagnóstico', alerts),
                pystray.MenuItem('Tendencias', trends),
                pystray.MenuItem('Historial de alertas', history),
                pystray.Menu.SEPARATOR,
                pystray.MenuItem('Salir de CorePulse', quit_app),
            ),
        )
    ...
    def notify_game_boost(self, title, message):
        """Muestra una notificación sólo con acciones que CorePulse confirmó."""
        if not self.icon:
            return False
        try:
            self.icon.notify(str(message or ''), str(title or 'CorePulse Game Boost'))
            return True
        except Exception:
            return False

    def notify_action(self, title, message):
        """Notifica el resultado de una acción rápida sin abrir la interfaz."""
        if not self.icon:
            return False
        try:
            self.icon.notify(str(message or ''), str(title or 'CorePulse'))
            return True
        except Exception:
            return False
