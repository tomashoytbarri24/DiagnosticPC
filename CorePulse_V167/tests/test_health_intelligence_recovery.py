"""Regresión V100 — Health Intelligence & Recovery."""
...
def main():
    results=[]
    results.append(check('version',VERSION == '103'))
    results.append(check('stage',STAGE=='STARTUP_RESPONSIVENESS_SINGLE_LAYOUT_AUTHORITY'))
    ...
    results += [
        ...
        check('all_requested_sections',all(x in panel for x in ('Salud de batería','Rendimiento y throttling térmico','benchmark rápido','Analizar inicio','Analizar servicios','Estabilidad de Windows','Revisar controladores','Cambios de hardware','Antes vs Después','Restauración y rollback','Red avanzada'))),
