...
AUTO_MAX_W = 1720
AUTO_MAX_H = 960
PRESETS = {'Automático': None, 'Compacto': (1280, 720), 'Recomendado': (1560, 860), 'Amplio': (1680, 920)}
    ...
    try:
        ...
        if path.exists():
            ...
            if isinstance(raw, dict):
                defaults.update(raw)
                # V145 migra el antiguo preset Recomendado 1420x780. Esa
                # geometría dejaba el Dashboard demasiado apretado en 1080p.
                # No se toca un tamaño Personalizado elegido por el usuario.
                try:
                    if str(raw.get('preset') or '') == 'Recomendado':
                        rw = int(raw.get('width', 0) or 0)
                        rh = int(raw.get('height', 0) or 0)
                        if rw <= 1420 and rh <= 800:
...
def show_window_size_dialog(app):
    ...
    def apply_selected():
        ...
        close_dialog()
    ctk.CTkButton(actions, text='Cancelar', height=34, fg_color=theme_color('#16263a'), hover_color=theme_color('#203650'), border_width=1, border_color=theme_color('#29435f'), text_color=TEXT, command=close_dialog).grid(row=0, column=0, sticky='ew', padx=(0, 5))
