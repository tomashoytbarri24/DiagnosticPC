...
class CPUDetailPanel:
    ...
    def _build(self):
        ...
        ctk.CTkButton(
            header,
            text='Volver al monitoreo',
            width=145,
            height=31,
            fg_color='transparent',
            hover_color=theme_color('#102840'),
            border_width=1,
            border_color=theme_color('#214765'),
            text_color=TEXT_2,
            font=(FONT, 9, 'bold'),
            corner_radius=8,
        ...
        hero = build_title_block(
            titles,
            eyebrow='Procesador · Vista avanzada',
            title='Detalles avanzados de CPU',
            subtitle='Identidad de Windows + sensores reales en tiempo real',
            accent=CYAN,
            badges=(('Windows', CYAN), ('Sensores reales', GREEN), ('REAL_OR_NA', AMBER)),
            title_size=19,
        )
    ...
    def _update_sensor_rows(self, sensors):
        ...
        for index, sensor in enumerate(valid):
            ...
            kind = str(sensor.get('type') or '').lower()
            configure_changed(kind_lbl, text=TYPE_LABELS.get(kind, kind.upper() or 'SENSOR'), text_color=TYPE_COLORS.get(kind, CYAN))
            ...
            configure_changed(source_lbl, text=source)
        ...
        if not valid and not self.lbl_no_sensors.winfo_manager():
            self.lbl_no_sensors.pack(fill='x', padx=8, pady=12)
        configure_changed(self.lbl_sensor_status, 
    ...
    def refresh(self):
        ...
        try:
            ...
            if cpu_name and not (self._identity and self._identity.get('name')):
                configure_changed(self.lbl_title, text=cpu_name)
            ...
            configure_changed(self.usage_value, text=_fmt(usage, 1, '%'), text_color=CYAN if _num(usage) is not None else MUTED)
            configure_changed(self.usage_detail, text='Carga total del sistema · psutil')
            configure_changed(self.temp_value, text=_fmt(temp, 1, ' °C'), text_color=_thermal_color(temp))
            configure_changed(self.temp_detail, text='CPU Package · LibreHardwareMonitor' if cpu.get('source') else 'Sensor térmico disponible')
            configure_changed(self.clock_value, text=_fmt(clock, 3, ' GHz'), text_color=CYAN if _num(clock) is not None else MUTED)
            max_clock = cpu.get('clock_max_ghz')
            configure_changed(self.clock_detail, text=f'Máximo observado: {_fmt(max_clock, 3, " GHz")}' if _num(max_clock) is not None else 'Frecuencia actual disponible')
            configure_changed(self.power_value, text=_fmt(power, 2, ' W'), text_color=PURPLE if _num(power) is not None else MUTED)
            configure_changed(self.power_detail, text='CPU Package · sensor real' if _num(power) is not None else 'Sensor de potencia no disponible')
            ...
            if not scrolling:
                configure_changed(self._advanced_labels['lhm_load'], text=_fmt(cpu.get('total_load_percent'), 1, '%'))
                configure_changed(self._advanced_labels['core_max'], text=_fmt(cpu.get('core_max_temp_c'), 1, ' °C'))
                configure_changed(self._advanced_labels['core_avg'], text=_fmt(cpu.get('core_average_temp_c'), 1, ' °C'))
                configure_changed(self._advanced_labels['tjmax'], text=_fmt(cpu.get('distance_to_tjmax_min_c'), 1, ' °C'), text_color=_distance_to_tjmax_color(cpu.get('distance_to_tjmax_min_c')))
                configure_changed(self._advanced_labels['clock_max'], text=_fmt(cpu.get('clock_max_ghz'), 3, ' GHz'))
                configure_changed(self._advanced_labels['bus_clock'], text=_fmt(cpu.get('bus_clock_mhz'), 2, ' MHz'))
                configure_changed(self._advanced_labels['voltage'], text=_fmt(cpu.get('core_voltage_v'), 4, ' V'))
