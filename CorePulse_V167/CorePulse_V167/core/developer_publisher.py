...
El publicador por perfiles prepara la raíz completa elegida y respeta .gitignore.
En ramas */corepulse-dev instala además el workflow administrado que mantiene la
prerelease automática de Desarrollo. No guarda credenciales: usa Git local.
"""
...
from core.development_release_automation import (
    WORKFLOW_RELATIVE_PATH, inspect_managed_workflow, materialize_managed_workflow,
    restore_managed_workflow,
)
...
def inspect_profile_publish_context(profile: dict | None) -> dict:
    ...
    managed_workflow = inspect_managed_workflow(root, expected_branch or branch)
    if managed_workflow.blocker:
        blockers.append(managed_workflow.blocker)
    elif managed_workflow.enabled and managed_workflow.needed:
        workflow_rel = WORKFLOW_RELATIVE_PATH.as_posix()
        plan = sorted(set(plan + [workflow_rel]), key=str.casefold)
        if managed_workflow.exists:
            modified = sorted(set(modified + [workflow_rel]), key=str.casefold)
        else:
            new_files = sorted(set(new_files + [workflow_rel]), key=str.casefold)
    ...
    return {
        ...
        'development_release_automation': bool(managed_workflow.enabled),
        'development_release_workflow_needed': bool(managed_workflow.enabled and managed_workflow.needed),
    }
...
def publish_profile_root(profile: dict, commit_message: str, *, progress=None, expected_context=None):
    ...
    try:
        try:
            managed_workflow_change = materialize_managed_workflow(repo, branch)
        except RuntimeError as exc:
            raise PublishError(str(exc)) from exc
        if managed_workflow_change.get('changed'):
            note('Activando GitHub Actions para prereleases automáticas de Desarrollo…')
        expected_worktree_hash = hashlib.sha256(
            _worktree_status_blob(repo).encode('utf-8', 'surrogatepass')
        ).hexdigest()
        ...
        staged = _paths(repo, 'diff', '--cached', '--name-only', '--no-renames', env=env)
        if hashlib.sha256(_worktree_status_blob(repo).encode('utf-8', 'surrogatepass')).hexdigest() != expected_worktree_hash:
        ...
        if temporary is not None:
            temporary.unlink(missing_ok=True)
        if managed_workflow_change and managed_workflow_change.get('changed') and not commit_created:
            restore_managed_workflow(managed_workflow_change)
