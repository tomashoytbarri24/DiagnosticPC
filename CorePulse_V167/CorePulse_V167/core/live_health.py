...
def _agent_instant(state):
    """Devuelve la condición instantánea REAL publicada por el agente, si existe.

    No calcula sensores ni estima valores: sólo reutiliza la muestra real ya
    clasificada por `core.agent_reaction.instant_health_from_sample`.
    """
    if not isinstance(state, dict):
        return {}
    item = state.get('instant')
    return item if isinstance(item, dict) else {}


def _agent_instant_level(state):
    item = _agent_instant(state)
    return str(item.get('severity') or 'UNKNOWN').upper()


def _agent_instant_reasons(state):
    item = _agent_instant(state)
    out = []
    ...
    if not out:
        ...
        if status and status.upper() not in {'NO EVALUABLE', 'UNKNOWN'}:
            out.append(status)
    return out
...
def evaluate_unified_live_health(telemetry, disks=None, preliminary_score=None, agent_state=None):
    ...
    agent_instant_level = _agent_instant_level(agent_state)

    # Autoridad única: se compara la telemetría certificada del Dashboard, la
    # muestra instantánea REAL del agente y las alertas rolling. Se elige la
    # condición más severa sin modificar ningún valor de sensor.
    winner = instant_level
    authority = 'INSTANT_CERTIFIED_TELEMETRY'
    if RANK.get(agent_instant_level, -1) > RANK.get(winner, -1):
        winner = agent_instant_level
        authority = 'REALTIME_AGENT_CURRENT_SAMPLE'
    elif RANK.get(agent_instant_level, -1) == RANK.get(winner, -1) and RANK.get(winner, -1) >= RANK['WARNING']:
        authority = 'INSTANT_CERTIFIED_AND_AGENT_CURRENT_SAMPLE'

    if RANK.get(agent_level, -1) > RANK.get(winner, -1):
        winner = agent_level
        authority = 'REALTIME_AGENT_ROLLING_EVIDENCE'
    elif RANK.get(agent_level, -1) == RANK.get(winner, -1) and RANK.get(agent_level, -1) >= RANK['WARNING']:
        authority = 'CURRENT_AND_ROLLING_EVIDENCE'
    ...
    reasons = []
    for item in (instant.get('reasons') or []) + _agent_instant_reasons(agent_state) + _active_agent_reasons(agent_state):
        if item and item not in reasons:
            reasons.append(item)
    trace = _tjmax_trace(telemetry)
    return {'version': VERSION, 'policy': POLICY, 'score': round(score, 1) if score is not None else None, 'severity': winner, 'status': status, 'authority': authority, 'instant': instant, 'agent_overall': agent_level, 'agent_instant': agent_instant_level, 'reasons': reasons, 'tjmax_trace': trace, 'sensor_values_unchanged': True, 'synthetic': False, 'estimated': False, 'interpolated': False}
