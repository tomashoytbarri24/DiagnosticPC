...
V4 corrige específicamente el ghosting observado al *agarrar y arrastrar* la
barra de desplazamiento en Windows. El viewport sigue siendo un ``tk.Canvas``
nativo, pero la barra deja de depender de ``CTkScrollbar``: CorePulse dibuja y
controla su propio thumb. De esta forma thumb + ``yview`` se comprometen en el
mismo frame, se puede congelar la geometría mientras dura el drag y se invalida
el área anterior antes de presentar el frame nuevo.
...
class StableScrollHost(ctk.CTkFrame):
    ...
    def __init__(
        ...
        **kwargs,
        ...
        # Tk Canvas mantiene el clipping. El drag no mueve widgets a mano: sólo
        # cambia yview y después invalida el viewport completo.
        ...
        self.content = ctk.CTkFrame(self.canvas, fg_color=self._bg, corner_radius=0, border_width=0)
    ...
    def _scrollbar_command(self, *args):
        ...
        try:
            ...
            if op == 'moveto' and len(args) >= 2:
                self.canvas.yview_moveto(max(0.0, min(1.0, float(args[1]))))
    ...
    def defer_until_idle(self, callback, *, key=None):
        """Agrupa repintados por clave; conserva cada tarea sin clave.

        Una finalización de trabajo no debe perderse porque después se pidió
        filtrar una lista. Los repintados reemplazables usan una clave estable.
        """
        if self._destroyed:
            return
        self._idle_callbacks[object() if key is None else key] = callback
        self._schedule_idle_flush()
