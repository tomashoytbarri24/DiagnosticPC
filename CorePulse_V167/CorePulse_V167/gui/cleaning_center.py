    ...
    def __init__(
        ...
        if analyze_command:
            self.btn_analyze = ctk.CTkButton(
                ...
                hover_color=theme_color('#203650'),
                border_width=1,
...
class ProfessionalCleaningCenterPanel(CleaningCenterPanel):
    ...
    def _build_header(self):
        ...
        hero = build_title_block(
            left,
            eyebrow='Mantenimiento seguro',
            title='Limpieza de sistema',
            subtitle='Herramientas independientes: analiza primero y ejecuta solo lo que necesites.',
            accent=CYAN,
            badges=(('Análisis previo', CYAN), ('Confirmación', ORANGE), ('REAL / N/A', GREEN)),
            title_size=19,
        )
        ...
        ctk.CTkButton(right, text='Volver al monitoreo', width=145, height=32, corner_radius=8, fg_color=theme_color('#17263a'), hover_color=theme_color('#203650'), border_width=1, border_color=theme_color('#29435f'), text_color=TEXT, font=('Segoe UI', 9, 'bold'), command=self.close).pack(side='left')
    ...
    def _build_cards(self):
        ...
        self.ram_card = ProfessionalCleaningToolCard(self.body, 'Liberar RAM', '', BLUE, 'Mide RAM real antes/después. Normal es conservador; Profunda libera varias regiones de memoria administradas por Windows.', 'Normal', self.optimize_ram, analyze_command=self.refresh_ram_card, footer='Profunda puede causar recargas o micro-stutter temporal; el resultado mostrado siempre es medido.', secondary_action_label='Profunda', secondary_action_command=self.optimize_ram_deep)
        ...
        self.storage_card = ProfessionalCleaningToolCard(self.body, 'Liberar Almacenamiento', '', GREEN, 'Escanea las unidades locales y muestra únicamente archivos que CorePulse puede eliminar con seguridad.', 'Liberar espacio', self.clean_storage, analyze_command=self.analyze_storage, footer='Solo muestra archivos eliminables.')
        ...
        for label in ('Confirmación', 'Medición real', 'Acciones separadas'):
            ctk.CTkLabel(self.badges, text=label, font=('Segoe UI', 8, 'bold'), text_color=theme_color('#bcd0e2'), fg_color=theme_color('#163047'), corner_radius=6, padx=6, pady=2).pack(side='left', padx=2)
    ...
    def close(self):
        """Vuelve al Dashboard conservando el centro para reapertura inmediata."""
        try:
    ...
    def _build_footer(self):
        ...
        ctk.CTkLabel(footer, text='Acciones independientes · CorePulse pide confirmación antes de aplicar cambios', font=('Segoe UI', 8), text_color=MUTED, height=16).grid(row=0, column=0, sticky='w', padx=3)
