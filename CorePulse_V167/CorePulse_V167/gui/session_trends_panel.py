...
class SessionTrendsPanel:
    ...
    def __init__(self, parent):
        ...
        hero = build_title_block(
            left,
            eyebrow='Comparativa de sesiones',
            title='Tendencias entre sesiones',
            subtitle='Últimos 10 diagnósticos de esta ejecución. GAME y DESKTOP se comparan siempre por separado.',
            accent=CYAN,
            badges=(('GAME', PURPLE), ('DESKTOP', CYAN), ('Hasta 10 sesiones', GREEN)),
            title_size=20,
        )
        hero['frame'].pack(anchor='w', fill='x')
        self.header_status = ctk.CTkLabel(header, text='Sin sesiones', font=('Segoe UI', 9, 'bold'), text_color=DIM, fg_color=theme_color('#102235'), corner_radius=8, padx=10, pady=5)
    ...
    def render(self, sessions, summary):
        ...
        if getattr(self, 'table_host', None) is not None and self.table_host.is_scrolling():
            ...
            self.table_host.defer_until_idle(lambda: self.render(sessions_copy, summary_copy), key='trends-render')
            return
    ...
    def _render_chart(self, sessions, summary):
        if not sessions:
            self._chart_photo = None
            self.chart_image.configure(image='', text='○  Aún no hay sesiones comparables\nEjecuta diagnósticos para empezar a construir una tendencia.', font=('Segoe UI', 12, 'bold'))
            return
    ...
    def _render_rows(self, sessions):
        ...
        if not sessions:
            state = build_empty_state(
                self.table,
                icon='↗',
                title='Todavía no hay sesiones para comparar',
                description='Cuando finalices diagnósticos, CorePulse mostrará aquí una cronología simple con contexto GAME / DESKTOP y cambios térmicos reales.',
                accent=CYAN,
                compact=True,
            )
            state['frame'].pack(fill='x', padx=6, pady=10)
            return
