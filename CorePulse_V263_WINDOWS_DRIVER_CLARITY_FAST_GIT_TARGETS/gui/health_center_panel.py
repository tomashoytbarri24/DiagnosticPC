...
class HealthCenterPanel:
    ...
    def __init__(self, app, host, *, performance_only=False, external_scroll=None, benchmark_only=False, initial_tab=None, initial_windows_section=None):
        ...
        # V259: Driver Direct consulta el catálogo por ID de hardware y no usa
        # Windows Update Agent. El inventario técnico queda oculto por defecto.
    ...
    def _health_module_card(self, parent, row, column, title, description, status, color, command, button_text='Abrir'):
        """Tarjeta de tamaño uniforme del Centro de salud.

        Todas las tarjetas del hub comparten exactamente la misma geometría. El
        contenido usa una fila flexible y la acción queda anclada abajo, así un
        estado de dos líneas nunca hace crecer sólo una tarjeta.
        """
        ...
        top.grid(row=0, column=0, sticky='ew', padx=13, pady=(11, 4))
        ...
        ctk.CTkLabel(
            top, text=chip_text, font=(FONT, 9, 'bold'), text_color=accent_color,
            anchor='w', justify='left'
        ).grid(row=0, column=1, sticky='ew')
        ctk.CTkLabel(
            top, text=title, font=(FONT, 14, 'bold'), text_color=TEXT,
            anchor='w', justify='left'
        ).grid(row=1, column=1, sticky='ew', pady=(1, 0))

        ctk.CTkLabel(
            top, text=str(status), font=(FONT, 9, 'bold'), text_color=color,
            anchor='e', justify='right', wraplength=138
        ...
        ).grid(row=1, column=0, sticky='nsew', padx=13, pady=(0, 6))

        self._button(card, button_text, command, variant='secondary', height=30).grid(
    ...
    def _render_summary(self):
        ...
        row_count = max(1, (len(module_specs) + 2) // 3)
        for row_index in range(row_count):
            modules.grid_rowconfigure(row_index, weight=0, minsize=174, uniform='health_module_rows')
    ...
    def _render_gaming_benchmark_section(self):
        ...
        # V259: antes de ejecutar no existe un bloque de estado/resultados.
        # Aparece sólo cuando la prueba realmente empezó o ya existe un resultado.
        if 'visual_benchmark' in self._jobs or isinstance(self._visual_bench, dict):
    ...
    def _render_visual_benchmark_card(self, parent):
        ...
        result = self._visual_bench if isinstance(self._visual_bench, dict) else None
        self._visual_benchmark_result_host = None
        if result:
    ...
    def _build_static_benchmark_result_pages(self, shell, result):
        ...
        for key in ('summary', 'gpu', 'system', 'evidence'):
            ...
            page_scroll = StableScrollHost(
                page, fg_color=BG, backend='canvas', wheel_pixels=96,
    ...
    def _refresh_static_benchmark_page(self, key=None):
        ...
        try:
            ...
            staging_scroll = StableScrollHost(
                staging, fg_color=BG, backend='canvas', wheel_pixels=96,
                scrollbar_button_color=BENCH_ACCENT_BORDER,
                scrollbar_button_hover_color=BENCH_ACCENT_HOVER,
            )
    ...
    def _run_visual_benchmark(self):
        ...
        if started:
            # V259: el bloque de progreso no existe antes del primer inicio. Al
            # arrancar se publica una sola vez; los callbacks posteriores sólo
            # actualizan sus widgets existentes.
            self._request_render(1)
    ...
    def _windows_section_status(self, section):
        ...
        if section == 'drivers':
            ...
            if data.get('error'):
                return 'No disponible', AMBER
            problems = int(data.get('review_count') or (int(data.get('device_problems') or 0) + int(data.get('unsigned') or 0)))
            if problems:
    ...
    def _render_analyzer(self, title, data, kind):
        ...
        if kind == 'startup':
            ...
            relevant_local = [
                ...
                if (
                    (int(x.get('hardware_priority') or 0) > 0 and not bool(x.get('microsoft_inbox_generic')))
                    or str(x.get('status') or '').upper() in {'DEVICE_PROBLEM', 'UNSIGNED'}
                )
            ]

            # V259: lectura rápida. La lista técnica completa deja de ocupar la
            # pantalla principal; el usuario ve primero sólo lo accionable.
            summary = ctk.CTkFrame(card, fg_color='transparent')
            summary.pack(fill='x', padx=10, pady=(4, 7))
            for col in range(3):
                summary.grid_columnconfigure(col, weight=1, uniform='driver_summary')
            summary_items = (
                ('HARDWARE PRINCIPAL', str(data.get('important_count', 0)), CYAN),
                ('INCIDENCIAS', str(data.get('device_problems', 0)), RED if data.get('device_problems') else GREEN),
                ('SIN FIRMA', str(data.get('unsigned', 0)), RED if data.get('unsigned') else GREEN),
            )
            for idx, (label, value, accent) in enumerate(summary_items):
                cell = ctk.CTkFrame(summary, fg_color=CARD2, border_width=1, border_color=BORDER, corner_radius=9)
                cell.grid(row=0, column=idx, sticky='nsew', padx=4)
                ctk.CTkLabel(cell, text=label, font=(FONT, 8, 'bold'), text_color=MUTED, anchor='w').pack(fill='x', padx=10, pady=(7, 1))
                ctk.CTkLabel(cell, text=value, font=(FONT, 15, 'bold'), text_color=accent, anchor='w').pack(fill='x', padx=10, pady=(0, 7))

            self._render_driver_update_center(card)

            inv_head = ctk.CTkFrame(card, fg_color='transparent')
            inv_head.pack(fill='x', padx=14, pady=(2, 7))
            left = ctk.CTkFrame(inv_head, fg_color='transparent')
            left.pack(side='left', fill='x', expand=True)
            ctk.CTkLabel(left, text='Inventario técnico', font=(FONT, 10, 'bold'), text_color=TEXT, anchor='w').pack(fill='x')
            ctk.CTkLabel(
                left,
                text=f'{len(relevant_local)} controladores relevantes · incidencias, firma y antigüedad se muestran por separado; componentes virtuales de Windows quedan ocultos',
                font=(FONT, 8), text_color=MUTED, anchor='w', justify='left'
            ).pack(fill='x', pady=(1, 0))
            self._button(
                inv_head, 'Ocultar inventario' if self._show_driver_inventory else 'Ver inventario',
            ...
            if self._show_driver_inventory:
                ...
                ctk.CTkLabel(
                    pager, text=(f'Mostrando {start_i + 1}–{end_i} de {total}' if total else 'Sin controladores relevantes'),
                    font=(FONT, 8), text_color=MUTED, anchor='w'
                ).pack(side='left', fill='x', expand=True)
                if page_count > 1:
                ...
                self._render_compact_table(
                    ...
                    columns=[
                        ...
                        {'title': 'Estado', 'weight': 6, 'wrap': 300},
                    ],
                    rows=rows,
