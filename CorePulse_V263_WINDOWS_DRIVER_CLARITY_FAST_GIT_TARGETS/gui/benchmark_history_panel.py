"""Historial profesional de benchmark de CorePulse.
...
HISTORY_INITIAL_VISIBLE = 3
HISTORY_PAGE_SIZE = 3
HISTORY_READ_LIMIT = 100
HISTORY_FAST_PRELOAD = 5
...
def _visual(session):
    ...
    return gpu if gpu.get("benchmark_method") in {"COREPULSE_GPU_VISUAL_MULTIPHASE_V2", "COREPULSE_GPU_TECHNICAL_MULTIPHASE_V3", "COREPULSE_GPU_AREAS_V4"} else {}
...
def _signature(session):
    ...
    ram = system.get("ram") if isinstance(system.get("ram"), dict) else {}
    try:
        ram_buffer_mb = int(ram.get("buffer_mb")) if ram.get("buffer_mb") is not None else None
    except Exception:
        ram_buffer_mb = None
    return profile, comps, cpu, gpu, renderer if renderer != "n/a" else "", method, ssd_volume, ssd_mode, ram_buffer_mb


def _equivalent(a, b):
    pa, ca, cpua, gpua, ra, ma, va, ia, rama = _signature(a)
    pb, cb, cpub, gpub, rb, mb, vb, ib, ramb = _signature(b)
    ...
    if 'SSD' in ca and (va != vb or ia != ib):
        return False
    if 'RAM' in ca and rama is not None and ramb is not None and rama != ramb:
        return False
    return True


class BenchmarkHistoryPanel:
    def __init__(self, app, host, *, preloaded_sessions=None, total_count=None):
        self.app = app
        self.host = host
        ...
        self._sessions = list(preloaded_sessions or [])
        self._total_count = int(total_count or len(self._sessions) or 0)
        self._preloaded_once = preloaded_sessions is not None
        self._visible_limit = HISTORY_INITIAL_VISIBLE

        toolbar = ctk.CTkFrame(
            self.frame, fg_color=SURFACE, border_width=1,
            border_color=BORDER, corner_radius=10
        )
        toolbar.pack(fill="x", padx=4, pady=(4, 10))
        left = ctk.CTkFrame(toolbar, fg_color="transparent")
        left.pack(fill="x", padx=14, pady=(11, 4))
        ctk.CTkLabel(
            left, text="Historial de benchmark",
            font=(FONT, 16, "bold"), text_color=TEXT, anchor="w"
        ).pack(anchor="w")
        ctk.CTkLabel(
            left,
            text="Sesiones reales guardadas en este PC. Compara sólo ejecuciones equivalentes del mismo equipo.",
            font=(FONT, 9), text_color=TEXT_2, anchor="w", justify="left",
        ).pack(anchor="w", pady=(2, 0))

        # V192: filtros en una segunda fila estable. Antes competían con el
        # título por el ancho del toolbar y durante el primer layout podían
        # quedar montados unos sobre otros.
        filters = ctk.CTkFrame(toolbar, fg_color="transparent")
        filters.pack(fill="x", padx=14, pady=(2, 10))
        ...
        self.profile_menu = ctk.CTkOptionMenu(
            ...
            command=lambda _v: self._filter_changed(), width=118, height=31,
        ...
        self.component_menu = ctk.CTkOptionMenu(
            ...
            command=lambda _v: self._filter_changed(), width=105, height=31,
            fg_color=SURFACE_2, button_color=ACCENT_2, button_hover_color=ACCENT,
            text_color=TEXT, dropdown_fg_color=SURFACE, dropdown_text_color=TEXT,
        )
        ...
        self.status_menu = ctk.CTkOptionMenu(
            ...
            command=lambda _v: self._filter_changed(), width=108, height=31,
            fg_color=SURFACE_2, button_color=ACCENT_2, button_hover_color=ACCENT,
            text_color=TEXT, dropdown_fg_color=SURFACE, dropdown_text_color=TEXT,
        )
        ...
        self.scroll_host = StableScrollHost(
            self.frame, fg_color=BG, backend='place',
            scrollbar_button_color=ACCENT_2,
            scrollbar_button_hover_color=ACCENT,
        )
        self.scroll_host.pack(fill="both", expand=True, padx=0, pady=0)
    ...
    def _read_sessions(self):
        if getattr(self, '_preloaded_once', False):
        ...
        try:
            ...
            sessions = list(store.latest_benchmark_sessions(limit=HISTORY_READ_LIMIT))
    ...
    def refresh(self, reload_from_store=False):
        if not self.frame.winfo_exists():
            return
        if reload_from_store or not self._sessions:
            self._sessions = self._read_sessions()
        elif self._preloaded_once:
            # Consume la primera caché precargada sin volver a consultar SQLite.
            self._preloaded_once = False
    ...
    def set_sessions_cache(self, sessions, total_count=None):
        """Actualiza la caché completa en segundo plano sin repintar la vista activa."""
    ...
    def _render_summary(self, sessions):
        ...
        filters_default = (
            self.profile_filter.get() == "Todos"
            and self.component_filter.get() == "Todos"
            and self.status_filter.get() == "Todos"
        )
        cache_complete = len(self._sessions) >= total_all
        if filters_default:
            count_text = str(total_all)
            count_detail = "guardadas en este PC" if cache_complete else f"{len(self._sessions)} listas ahora · resto cargando en segundo plano"
        else:
            count_text = str(filtered) if cache_complete else f"{filtered}+"
            count_detail = "coinciden con los filtros" if cache_complete else "coincidencias entre las sesiones disponibles ahora"
        self._summary_card(grid, 0, "SESIONES", count_text, count_detail)
        if latest:
            try:
            ...
            self._summary_card(grid, 3, "RENDERER GPU", renderer, "registrado por el backend gráfico" if renderer != "N/A" else "N/A en esta sesión")
        else:
    ...
    def _render_sessions(self, sessions):
        ...
        ctk.CTkLabel(head, text="Ejecuciones recientes", font=(FONT, 11, "bold"), text_color=TEXT, anchor="w").pack(side="left")
        visible = min(len(sessions), max(1, int(self._visible_limit)))
        filters_default = (
            self.profile_filter.get() == "Todos"
            and self.component_filter.get() == "Todos"
            and self.status_filter.get() == "Todos"
        )
        total_label = max(len(sessions), int(self._total_count or 0)) if filters_default else len(sessions)
        ctk.CTkLabel(
            head,
            text=(f"{visible} de {total_label}" if sessions else "0"),
            font=(FONT, 9, "bold"), text_color=MUTED
        ).pack(side="right")
        if not sessions:
        ...
        for session in sessions[:visible]:
            self._render_session_card(session)

        if visible < len(sessions):
            more = ctk.CTkFrame(self.scroll, fg_color="transparent")
            more.pack(fill="x", padx=7, pady=(7, 12))
            remaining = len(sessions) - visible
            step = min(HISTORY_PAGE_SIZE, remaining)
            ctk.CTkLabel(
                more,
                text=f"Mostrando las más recientes · {remaining} más disponibles",
                font=(FONT, 9), text_color=MUTED, anchor="w"
            ).pack(side="left")
            ctk.CTkButton(
                more,
                text=f"Mostrar {step} más",
                command=self._show_more_sessions,
                width=128, height=31,
                fg_color=SURFACE_2, hover_color=ACCENT_2,
    ...
    def _show_more_sessions(self):
        self._visible_limit = min(
            len(self._filtered(self._sessions)),
            max(1, int(self._visible_limit)) + HISTORY_PAGE_SIZE,
        )
        sessions = self._filtered(self._sessions)
        self._clear()
        self._render_summary(sessions)
    ...
    def _history_metric_chip(self, parent, col, title, value, detail, accent):
        chip = ctk.CTkFrame(parent, fg_color=SURFACE_2, border_width=1, border_color=BORDER, corner_radius=8)
        chip.grid(row=0, column=col, sticky="nsew", padx=3)
        ctk.CTkFrame(chip, height=2, fg_color=accent, corner_radius=2).pack(fill="x", padx=8, pady=(6, 5))
        ctk.CTkLabel(chip, text=title, font=(FONT, 8, "bold"), text_color=accent, anchor="w").pack(fill="x", padx=9)
        ctk.CTkLabel(chip, text=value, font=(FONT, 11, "bold"), text_color=TEXT, anchor="w").pack(fill="x", padx=9, pady=(1, 0))
        ctk.CTkLabel(chip, text=detail, font=(FONT, 8), text_color=MUTED, anchor="w").pack(fill="x", padx=9, pady=(0, 7))

    def _render_session_metrics(self, card, session):
        metrics = []
        gpu = _metric(session, "gpu_fps")
        gpu_low = _metric(session, "gpu_low")
        if gpu is not None:
            metrics.append(("GPU · Extreme", f"{gpu:.1f} FPS", f"1% Low {gpu_low:.1f} FPS" if gpu_low is not None else "1% Low N/A", ACCENT))
        cpu = _metric(session, "cpu")
        if cpu is not None:
            metrics.append(("CPU · SHA-256", f"{cpu:.0f} MB/s", "Procesamiento", PURPLE))
        ram = _metric(session, "ram")
        if ram is not None:
            metrics.append(("RAM · Copia", f"{ram:.2f} GB/s", "Ancho de banda", GREEN))
        ssd_r = _metric(session, "ssd_read")
        ssd_w = _metric(session, "ssd_write")
        if ssd_r is not None:
            metrics.append(("SSD · Lectura", f"{ssd_r:.0f} MB/s", f"Escritura {ssd_w:.0f} MB/s" if ssd_w is not None else "Escritura N/A", AMBER))
        if not metrics:
            ctk.CTkLabel(card, text="Sin métricas numéricas válidas guardadas", font=(FONT, 9), text_color=MUTED, anchor="w").pack(fill="x", padx=12, pady=(2, 4))
            return
        row = ctk.CTkFrame(card, fg_color="transparent")
        row.pack(fill="x", padx=9, pady=(3, 5))
        for col in range(len(metrics)):
            row.grid_columnconfigure(col, weight=1, uniform="history_metrics")
        for col, item in enumerate(metrics):
            self._history_metric_chip(row, col, *item)
    ...
    def _render_session_card(self, session):
        ...
        self._render_session_metrics(card, session)
    ...
    def _render_detail(self, card, session):
        ...
        rows = [
            ...
            ("Renderer GPU", _renderer(session)),
