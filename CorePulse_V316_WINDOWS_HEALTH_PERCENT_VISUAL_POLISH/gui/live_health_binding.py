...
def _render(app, telemetry, disks):
    ...
    _cfg(getattr(app, '_health_score', None), text=summary, text_color=MUTED)
    _cfg(
    ...
    try:
        ...
        if getattr(app, '_health_progress', None) is not None:
            app._health_progress.configure(progress_color=color)
            app._health_progress.set(progress_value)
    except Exception:
        pass
    _cfg(getattr(app, '_health_hint', None), text='Pasa el mouse para ver detalles' if reasons else 'Estado saludable en tiempo real', text_color=MUTED)
