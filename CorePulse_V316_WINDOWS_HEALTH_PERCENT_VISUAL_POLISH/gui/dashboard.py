        ...
        try:
            ...
            for idx, line in enumerate(body_lines):
                ctk.CTkLabel(
                    ...
                    anchor='w',
                    wraplength=320,
                ).pack(anchor='w', pady=(6 if idx == 0 and title else 2, 0))
...
def _health_hover_text(app):
    ...
    if reasons and not (len(reasons) == 1 and reasons[0].startswith('No se detectan señales')):
        body = ['Factores que están influyendo ahora:'] + [f'• {item}' for item in reasons[:5]]
    else:
        body = [
            'Sin revisión necesaria por ahora.',
            'No se detectan señales relevantes de RAM saturada, SSD lleno, salud SMART baja ni temperatura alta.',
        ]
...
def _health_status_card(parent):
    ...
    summary.grid(row=3, column=1, sticky='ew', pady=(3, 0))
    ...
    progress.set(0.0)
    progress.grid(row=4, column=1, sticky='ew', pady=(8, 0))
    ...
    hint.grid(row=5, column=1, sticky='w', pady=(3, 0))
