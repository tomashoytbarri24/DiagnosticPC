...
class OverlayConfigPanel:
    ...
    def _refresh_status(self):
        ...
        try:
            ...
            running = False
        if not running:
            configure_changed(self.status_badge, text='DETENIDO', text_color=MUTED)
            configure_changed(self.lbl_rtss, text='RTSS: overlay detenido', text_color=DIM)
            configure_changed(self.lbl_app, text='Juego detectado: N/A')
            configure_changed(self.lbl_policy, text='FPS medidos · N/A sin datos')
            configure_changed(self.btn_toggle, text='Iniciar Overlay')
            return
        try:
        ...
        configure_changed(self.status_badge, text='ACTIVO' if available else 'ESPERANDO RTSS', text_color=GREEN if available else WARN)
        configure_changed(self.lbl_rtss, text=f"RTSS: conectado · v{status.get('rtss_version')}" if available else f"RTSS: {error or 'no disponible'}", text_color=GREEN if available else WARN)
        configure_changed(self.lbl_app, text=f"Juego detectado: {app_name or 'N/A'}")
        configure_changed(self.lbl_policy, text='FPS medidos · N/A sin datos')
        configure_changed(self.btn_toggle, text='Detener Overlay')
