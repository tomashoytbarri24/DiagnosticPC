...
def run_complete_diagnostic(
    ...
    *, telemetry_sampler: TelemetrySampler = None, progress_callback: ProgressCallback = None, cancel_check: CancelCheck = None, state_callback=None,
    ...
    """Completa la sesión pasiva con Windows y benchmark estándar observado."""
    started = time.perf_counter()
    ...
    def benchmark_progress(frac, stage, detail=''):
        _emit(progress_callback, 0.28 + 0.68 * float(frac), f'Benchmark · {stage}', detail)

    phase_begin('benchmark', 'Benchmark estándar')
    _emit(progress_callback, 0.28, 'Benchmark estándar', 'Midiendo rendimiento CPU, RAM, SSD y GPU por separado')
    try:
    ...
    windows_partial = any(bool((windows.get(key) or {}).get('error')) for key in ('startup', 'services', 'stability', 'drivers') if isinstance(windows.get(key), dict))
    ...
    phase_partial = bool(baseline_partial or hardware_partial or windows_partial or observation['status'] != 'OK' or benchmark_partial)
    ...
    phase_rows = [phases.get(k) or {} for k in ('desktop', 'hardware', 'windows', 'benchmark', 'load_observation', 'correlation')]
