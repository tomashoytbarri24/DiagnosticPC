> Versión actual: **V327** · Driver Hub multi-fuente con fabricante oficial + Catalog.


> **Integraciones futuras:** revisar `COREPULSE_NON_REGRESSION_CONTRACT.md` y ejecutar `pytest -q` antes de reemplazar almacenamiento, drivers, temas o navegación.
# CorePulse — Windows only

CorePulse se desarrolla, valida y distribuye exclusivamente para Windows 10/11 x64.
...
## Runtime automático V298

Al abrir `main.py` o `corepulse_launcher.py` con CPython 3.12+ x64, si faltan dependencias CorePulse crea y valida automáticamente un runtime aislado para ese mismo minor. Ya no es obligatorio ejecutar previamente `instalar_dependencias.bat`.
