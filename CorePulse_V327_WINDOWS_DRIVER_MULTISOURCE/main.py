...
import sys
from core.python_compat import bootstrap_validated_runtime, enforce_minimum_python
bootstrap_validated_runtime(__file__)
...
from gui.resize_render_guard import install as _install_resize_render_guard
# V159: agrupa redibujos de tamaño por ventana durante resize, conservando
# el manejador original de CTk. Se instala antes de crear cualquier widget.
_install_resize_render_guard()
import threading
import time
import psutil
import os
...
from gui.internal_navigation import activate_internal_page, stage_internal_page, commit_internal_page, attach_internal_page_panel, abort_internal_page, clear_internal_page, show_dashboard
...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        # V145: el botón X nativo oculta CorePulse en la bandeja para mantener
        # monitoreo/agente activos. El cierre real queda reservado a
        # "Salir de CorePulse" y a flujos internos explícitos (update/restart).
        ...
        self._startup_ui_sample_count = 0
        self._startup_storage_health_ready = False
        self._startup_storage_health_worker_running = False
        self._startup_storage_wait_started = 0.0
        ...
        # V320 invariant: real SMART/NVMe health is persistent evidence. A later
        # transient read failure must never replace a previously measured % with N/A.
        try:
            from core.storage_summary_health import load_storage_summary_health_cache
            self.storage_health_cache = load_storage_summary_health_cache()
        except Exception:
            self.storage_health_cache = {}
        self._storage_health_cache_at = time.time() if self.storage_health_cache else 0.0
        self._storage_health_refresh_running = False
        self._storage_health_refresh_after_id = None
        ...
        # V142: el diagnóstico completo continúa después de la observación pasiva.
        self._complete_diagnostic_running = False
        self._complete_diagnostic_progress = 0.0
        self._complete_diagnostic_stage = None
        self._complete_diagnostic_started_at = None
        # V150: cancelación cooperativa + token para ignorar callbacks de una ejecución cancelada.
        self._diagnostic_cancel_event = threading.Event()
        self._diagnostic_run_token = 0
        self._diagnostic_run = None
        self._diagnostic_worker = None
        self._diagnostic_poll_id = None
        ...
        self._sidebar_pending_collapsed = None
        self._sidebar_motion_layers = []
        self._sidebar_transition_after_id = None
        self._sidebar_transition_cover = None
        self._sidebar_transition_edge = None
        self._sidebar_transition_snapshot = None
        self._sidebar_transition_snapshot_image = None
        self._sidebar_transition_snapshot_after_id = None
        self._sidebar_hover_map = []
        self._sidebar_preview_jobs = []
        self._sidebar_hover_active_key = None
        self._sidebar_toggle_hover_visible = False
        self._sidebar_edge_widgets = {}
        self._sidebar_edge_tracking_enabled = False
        self._sidebar_last_pointer = None
        ...
        self.card_health_sidebar = ctk.CTkFrame(self.sidebar, fg_color=BG_CARD, border_width=1, border_color=BORDER_COLOR, corner_radius=16)
        ...
        self.btn_diagnostic = ctk.CTkButton(self.sidebar, text='Iniciar diagnóstico', fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#f4f7fb'), font=('Segoe UI', 11, 'bold'), height=34, corner_radius=8, command=lambda: self.start_diagnostic_session(force_new=True))
        ...
        self.btn_smart_alerts = ctk.CTkButton(self.sidebar, text='🚨 Alertas técnicas', fg_color='#1d4ed8', hover_color='#1e40af', text_color='#ffffff', font=('Segoe UI', 11, 'bold'), height=34, corner_radius=8, command=self.open_smart_alert_window)
        ...
        self.alert_summary_bar = ctk.CTkFrame(self.main_content, fg_color=BG_CARD, border_width=1, border_color=BORDER_COLOR, corner_radius=16)
        ...
        self.disk_widgets = {}
        self.frame_charts = ctk.CTkFrame(self.main_content, fg_color=role_color('surface_2'), border_width=1, border_color=BORDER_COLOR, corner_radius=0)
        ...
        # V224: se descarta el experimento de sidebar plegable/cascada.
        # El lateral queda fijo y estable desde el arranque.
    ...
    def _sidebar_target_width(self):
        """V254: lateral algo más compacto, sin apretar etiquetas."""
        mode = str(getattr(self, '_layout_mode', 'large') or 'large')
        if mode == 'compact':
            return 220
        if mode == 'standard':
            return 228
    ...
    def _clear_sidebar_motion_layers(self):
        layers = list(getattr(self, '_sidebar_motion_layers', []) or [])
        for layer in layers:
            try:
                layer.place_forget()
                layer.destroy()
    ...
    def _clear_sidebar_transition_cover(self):
        """Retira la máscara de transición y deja el dashboard en estado estable."""
        try:
            after_id = getattr(self, '_sidebar_transition_after_id', None)
            if after_id is not None:
                self.after_cancel(after_id)
        except Exception:
            pass
        self._sidebar_transition_after_id = None
        cover = getattr(self, '_sidebar_transition_cover', None)
    ...
    def _ensure_sidebar_transition_cover(self):
        """Crea un velo angosto reutilizable junto al borde del contenido.

        V303 reemplaza la máscara de pantalla completa de V302 por una micro-
        transición localizada. El dashboard permanece visible todo el tiempo;
        sólo se cubren unos pocos píxeles junto al borde que cambia de posición.
        """
        cover = getattr(self, '_sidebar_transition_cover', None)
        try:
            exists = bool(cover is not None and cover.winfo_exists())
        except Exception:
            exists = False
        if not exists:
            cover = ctk.CTkFrame(
            ...
            # Tres bandas muy estrechas simulan un degradado sin tapar el panel.
            band_a = ctk.CTkFrame(cover, fg_color='#0b2138', width=8, corner_radius=0)
            band_b = ctk.CTkFrame(cover, fg_color='#0a192b', width=7, corner_radius=0)
            band_c = ctk.CTkFrame(cover, fg_color='#081321', width=5, corner_radius=0)
            edge = ctk.CTkFrame(cover, fg_color=COLOR_CPU, width=2, corner_radius=0)
            self._sidebar_transition_cover = cover
            self._sidebar_transition_bands = (band_a, band_b, band_c)
            self._sidebar_transition_edge = edge
        return cover

    def _layout_sidebar_transition_bands(self, width=22):
        cover = getattr(self, '_sidebar_transition_cover', None)
        if cover is None:
            return
        width = max(2, int(width))
        try:
            cover.configure(width=width)
        except Exception:
            pass
        bands = tuple(getattr(self, '_sidebar_transition_bands', ()) or ())
        # Distribución proporcional para que al encogerse no aparezcan bloques.
        sizes = (
            max(1, int(width * 0.36)),
            max(1, int(width * 0.30)),
            max(1, int(width * 0.22)),
        )
        x = 0
        for band, band_w in zip(bands, sizes):
            try:
                band.place(x=x, y=0, width=band_w, relheight=1.0, anchor='nw')
                x += band_w
            except Exception:
                pass
        edge = getattr(self, '_sidebar_transition_edge', None)
        if edge is not None:
            try:
                edge.place(x=max(0, width - 2), y=0, width=2, relheight=1.0, anchor='nw')
                edge.lift()
            except Exception:
                pass

    def _sidebar_native_hwnds(self):
        """Devuelve un único HWND raíz para el commit del layout.

        V309 evita congelar simultáneamente el cliente Tk y su wrapper Win32.
        En las pruebas visuales esa doble invalidación podía dejar durante un
        frame buffers viejos de widgets hijos (texto/tarjetas duplicadas).
        """
        try:
            user32 = ctypes.windll.user32
            client = int(self.winfo_id())
            if not client:
                return []
            GA_ROOT = 2
            root = int(user32.GetAncestor(client, GA_ROOT) or 0)
            return [root or client]
        except Exception:
            return []

    def _set_sidebar_native_redraw(self, enabled):
        """Suspende/reanuda sólo el HWND raíz durante el commit final."""
        WM_SETREDRAW = 0x000B
        try:
            user32 = ctypes.windll.user32
        except Exception:
            return []
        handles = self._sidebar_native_hwnds()
        wparam = 1 if enabled else 0
        for hwnd in handles:
            try:
                user32.SendMessageW(hwnd, WM_SETREDRAW, wparam, 0)
            except Exception:
                pass
        return handles

    def _flush_sidebar_native_redraw(self, handles=None):
        """Invalida una sola vez el frame final, sin repintados duplicados."""
        ...
        try:
            ...
            return
        for hwnd in list(handles or self._sidebar_native_hwnds()):
            try:
                user32.RedrawWindow(hwnd, None, None, flags)
            except Exception:
                pass

    def _sidebar_drawer_height(self):
        try:
            return max(220, int(self.winfo_height()) - 28)
        except Exception:
            return 720

    def _sidebar_hide_reopen_glyph(self):
        glyph = getattr(self, '_sidebar_reopen_button', None)
    ...
    def _finish_sidebar_chart_reflow(self):
        """Matplotlib queda fuera de la ruta crítica del clic."""
        try:
            from gui.dashboard_layout import request_chart_reflow
            request_chart_reflow(self, redraw=True)
        except Exception:
            pass
        self._chart_reflow_pending = False

    def _commit_sidebar_drawer_state(self, collapsed, target_width):
        """Commit final estabilizado antes de volver a publicar el frame.

        V311 mantiene WM_SETREDRAW desactivado durante un ciclo idle extra.
        CustomTkinter puede así procesar sus redraws internos con la geometría
        definitiva sin que Windows muestre buffers viejos entre medio.
        """
        handles = self._set_sidebar_native_redraw(False)
        self._layout_transition = True
        try:
            try:
            ...
            # Si el commit geométrico falla, nunca dejar el HWND congelado.
            try:
                self._set_sidebar_native_redraw(True)
            finally:
                self._flush_sidebar_native_redraw(handles)
            self._layout_transition = False
            self._sidebar_animating = False
            raise

        released = {'done': False}

        def release_final_frame():
            if released['done']:
                return
            released['done'] = True
            try:
                # Un segundo settle idle permite que los CTkCanvas recalculen
                # texto, bordes y fondos con la geometría final todavía oculta.
                self.update_idletasks()
            except Exception:
                pass
            finally:
            ...
            try:
                from gui.dashboard import _sync_sidebar_visibility_controls
                _sync_sidebar_visibility_controls(self)
            except Exception:
                pass
            self._sidebar_animating = False
            self._sidebar_anim_after_id = None
            ...
            # Matplotlib permanece fuera del gesto y se redibuja después del
            # frame estable, evitando que el canvas participe del commit.
            try:
                self.after(4, self._finish_sidebar_chart_reflow)
            except Exception:
                self._finish_sidebar_chart_reflow()

            pending = getattr(self, '_sidebar_pending_collapsed', None)
            self._sidebar_pending_collapsed = None
            if pending is not None and bool(pending) != bool(self._sidebar_collapsed):
                try:
                    self.after(1, lambda: self.set_sidebar_collapsed(bool(pending), animated=True))
                except Exception:
                    pass

        try:
            self.after_idle(release_final_frame)
        except Exception:
            release_final_frame()

    def _run_sidebar_drawer(self, collapsed, target_width, duration_ms=52):
        """Desliza sólo el sidebar; el dashboard permanece geométricamente quieto.

        Este es el cambio clave de V309. V307 animaba el sidebar a la vez que el
        dashboard se reacomodaba, haciendo visibles estados intermedios. V309
        conserva el ancho actual del contenido durante los ~52 ms del gesto y
        realiza un único commit al terminar.
        """
        self._sidebar_animating = True
        self._sidebar_pending_collapsed = None
        height = self._sidebar_drawer_height()
        dock_x = 12
        hidden_x = -int(target_width) - 6

        # Cierre: sacar del grid pero conservar la columna reservada, de modo que
        # el contenido NO cambie de tamaño mientras el panel se desliza.
        if collapsed:
            try:
                self.grid_columnconfigure(0, weight=0, minsize=target_width)
                self.sidebar.grid_remove()
                self.sidebar.place(
                    x=dock_x, y=14, width=target_width, height=height, anchor='nw'
                )
                self.sidebar.lift()
            except Exception:
                self._commit_sidebar_drawer_state(True, target_width)
                return
            start_x, end_x = dock_x, hidden_x
        else:
            # Apertura: el contenido sigue expandido y el sidebar entra como
            # drawer superpuesto. El docking ocurre sólo al final.
            self._sidebar_hide_reopen_glyph()
            try:
                self.grid_columnconfigure(0, weight=0, minsize=0)
                self.sidebar.grid_remove()
                self.sidebar.place(
                    x=hidden_x, y=14, width=target_width, height=height, anchor='nw'
                )
                self.sidebar.lift()
            except Exception:
                self._commit_sidebar_drawer_state(False, target_width)
                return
            start_x, end_x = hidden_x, dock_x

        # No llamar update()/update_idletasks() en cada tick: eso era una fuente
        # importante de latencia y reentrancia de Tk en versiones anteriores.
        steps = 4
        interval = max(8, int(duration_ms / steps))
        delta = float(end_x - start_x)
        ...
        def tick(index=0):
            if index >= steps:
                try:
                    self.sidebar.place_configure(x=end_x)
                except Exception:
                    pass
                self._commit_sidebar_drawer_state(collapsed, target_width)
                return

            ratio = ease_out_cubic((index + 1) / float(steps))
            x = int(round(start_x + delta * ratio))
            try:
                ...
                return
            try:
                self._sidebar_anim_after_id = self.after(interval, lambda: tick(index + 1))
            except Exception:
                self._commit_sidebar_drawer_state(collapsed, target_width)
    ...
    def set_sidebar_collapsed(self, collapsed, *, animated=True):
        """Oculta/muestra el sidebar con drawer asíncrono y commit atómico.

        El layout principal no se recalcula durante la animación. Esto elimina el
        efecto de tarjetas duplicadas/estiradas del video de V308 y reduce la
        sensación de lag porque el relayout completo y el render síncrono de
        Matplotlib quedan fuera del clic.
        """
        collapsed = bool(collapsed)
        if getattr(self, '_sidebar_animating', False):
            self._sidebar_pending_collapsed = collapsed
            return
        if collapsed == bool(getattr(self, '_sidebar_collapsed', False)):
            return

        target_width = self._sidebar_target_width()
        # V312: la ruta del clic sólo toca el drawer actual. Los sistemas de
        # máscara/capas de versiones anteriores ya no participan aquí; evitamos
        # invalidaciones de widgets innecesarias antes del primer frame.
        try:
        ...
        if not animated:
            self._commit_sidebar_drawer_state(collapsed, target_width)
            return
        self._run_sidebar_drawer(collapsed, target_width, duration_ms=52)

    def toggle_sidebar_collapse(self):
        """Alterna visibilidad del sidebar lateral."""
        self.set_sidebar_collapsed(not bool(getattr(self, '_sidebar_collapsed', False)), animated=True)

    def _install_sidebar_edge_tracking(self):
        """V224: deshabilitado; no instala bindings globales de hover/cascada."""
        self._sidebar_edge_tracking_enabled = False
        return
    ...
    def _reveal_main_window(self):
        ...
        # V216: la geometría se reafirma DESPUÉS de cerrar el Startup Gate.
        # De esta forma Windows no puede publicar la raíz en estado zoomed/
        # casi fullscreen por herencia de estado o por el orden de reveal.
        try:
        ...
        try:
            ...
            pass
        try:
            apply_preferred_launch_geometry(self, force=True)
            self.update_idletasks()
        except Exception:
            logger.exception('[STARTUP] No se pudo reafirmar geometría normal de inicio')
        ...
        # V217: al publicar una raíz que estuvo withdrawn, el Window Manager de
        # Windows puede reajustar la geometría en el primer frame. Reafirmamos
        # el tamaño clásico después de deiconify y una única vez tras el settle.
        def _settle_startup_geometry():
            try:
                self.attributes('-fullscreen', False)
            except Exception:
                pass
            ...
            try:
                ...
                logger.exception('[STARTUP] No se pudo estabilizar la geometría clásica de inicio')

        _settle_startup_geometry()
        try:
            self.after(120, _settle_startup_geometry)
        except Exception:
            pass
    ...
    @staticmethod
    def _storage_health_record_has_percent(record):
        if not isinstance(record, dict):
            return False
        try:
            return record.get('health') is not None and 0.0 <= float(record.get('health')) <= 100.0
        except Exception:
            return False

    def _merge_storage_health_records(self, incoming):
        """Fusiona evidencia sin permitir que un fallo temporal borre un % real.

        Si la lectura nueva no tiene salud cuantitativa, conserva también label,
        source y wear previos. V321 preservaba `health` pero podía sobrescribir
        `health_label` con ``Salud N/A``, que era exactamente lo visible en UI.
        """
        if not isinstance(incoming, dict) or not incoming:
            return False
        current = dict(getattr(self, 'storage_health_cache', {}) or {})
        health_fields = {
            'health', 'health_label', 'health_source', 'health_derived',
            'wear_percent', 'wear_source', 'wear_quantitative_reliable',
        ...
        for key, value in incoming.items():
            ...
            keep_previous_health = (
                not self._storage_health_record_has_percent(value)
                and self._storage_health_record_has_percent(previous)
            )
            for field, field_value in value.items():
                if keep_previous_health and field in health_fields:
                    continue
            ...
            if merged != previous:
                changed = True
            current[key] = merged
        self.storage_health_cache = current
        if current:
            self._storage_health_cache_at = time.time()
        if changed:
            try:
                from core.storage_summary_health import save_storage_summary_health_cache
                save_storage_summary_health_cache(current)
            except Exception:
                pass
        return changed

    def _storage_health_ready_for_telemetry(self, telemetry):
        if not isinstance(telemetry, dict):
            return False
        devices = telemetry.get('_storage_inventory')
        if not isinstance(devices, list) or not devices:
            devices = telemetry.get('_storage_devices')
        if not isinstance(devices, list) or not devices:
            return True
        cache = getattr(self, 'storage_health_cache', {}) or {}
        for index, _device in enumerate(devices):
            if not self._storage_health_record_has_percent(cache.get(index)):
                return False
        return True

    def _start_startup_storage_health_worker(self, telemetry):
        """Resuelve la salud lenta durante el splash, nunca en el hilo Tk."""
        if getattr(self, '_startup_storage_health_worker_running', False):
            return
        if self._storage_health_ready_for_telemetry(telemetry):
            self._startup_storage_health_ready = True
            return
        self._startup_storage_health_worker_running = True
        snapshot = copy.deepcopy(telemetry if isinstance(telemetry, dict) else {})

        def worker():
            try:
                from core.storage_summary_health import collect_storage_summary_health
                result = collect_storage_summary_health(snapshot, include_slow_fallbacks=True)
                if result:
                    self._merge_storage_health_records(result)
                self._startup_storage_health_ready = self._storage_health_ready_for_telemetry(snapshot)
            except Exception:
                self._log_throttled_exception('startup_storage_health_full', 'No se pudo completar salud de almacenamiento durante el splash')
            finally:
                self._startup_storage_health_worker_running = False
        threading.Thread(target=worker, daemon=True, name='CorePulse-StartupStorageHealth').start()

    def _schedule_storage_health_refresh(self):
        """Actualiza salud/capacidad de discos en dos fases sin bloquear la UI.

        Primero publica NVMe/LHM local y luego enriquece con Storage Reliability /
        smartctl en segundo plano. Un fallo temporal nunca borra un porcentaje real
        obtenido previamente.
        """
        if not getattr(self, 'is_running', False):
            return
        if getattr(self, '_storage_health_refresh_running', False):
            return

        telemetry = copy.deepcopy(getattr(self, 'latest_telemetry', {}) or {})
        devices = telemetry.get('_storage_inventory') if isinstance(telemetry, dict) else None
        if not isinstance(devices, list) or not devices:
            devices = telemetry.get('_storage_devices') if isinstance(telemetry, dict) else None
        if not isinstance(devices, list) or not devices:
            try:
                self._storage_health_refresh_after_id = self.after(1200, self._schedule_storage_health_refresh)
            except Exception:
                pass
            return
        ...
        def worker():
            ...
            try:
                from core.storage_summary_health import collect_storage_summary_health
                fast_result = collect_storage_summary_health(telemetry, include_slow_fallbacks=False)
                had_result = bool(fast_result)
                if fast_result:
                    publish(fast_result)

                full_result = collect_storage_summary_health(telemetry, include_slow_fallbacks=True)
                had_result = bool(full_result) or had_result
                if full_result:
                    publish(full_result)
            except Exception:
                ...
                self._storage_health_refresh_running = False
                delay = 300000 if had_result else 5000
                try:
                    self._storage_health_refresh_after_id = self.after(delay, self._schedule_storage_health_refresh)
                except Exception:
                    self._storage_health_refresh_after_id = None
    ...
    def open_benchmark(self):
        ...
        try:
            ...
            try:
                panel.prepare_for_commit()
            except Exception:
                pass

            # V279: pre-mapeamos el shell detrás de la vista actual y sólo lo
            # elevamos en el siguiente frame. Así no existe un frame intermedio
            # vacío entre Resumen y Benchmark.
            if not stage_internal_page(self, 'benchmark', host, panel):
                self.benchmark_panel = None
                abort_internal_page(self, 'benchmark', host, panel)
                return

            def publish_benchmark():
                if not commit_internal_page(self, 'benchmark', host, panel):
                    if self.benchmark_panel is panel:
                        self.benchmark_panel = None

            try:
                self.after(16, publish_benchmark)
            except Exception:
                publish_benchmark()
        except Exception as exc:
            self.benchmark_panel = None
    ...
    def open_health_center(self, tab='summary', windows_section=None):
        """Abre Centro de Salud directamente en el destino solicitado.

        V144 prepara la pestaña/sección profunda antes de publicar el host para
        evitar el flash Resumen → Windows → subsección observado desde Diagnóstico.
        """
        if not self.is_running:
            return
        ...
        if target_tab != 'windows':
            target_section = 'summary'

        # Si la página ya está cacheada pero oculta, se prepara el destino mientras
        # sigue fuera del viewport. activate_internal_page sólo la eleva después.
        cached_panel = None
        try:
            cached = getattr(self, '_internal_page_cache', {}).get('health_center')
            if isinstance(cached, dict):
                cached_panel = cached.get('panel')
        except Exception:
            cached_panel = None
        active_panel = getattr(self, 'health_center_panel', None)
        prepared_panel = cached_panel or active_panel
        if prepared_panel is not None and hasattr(prepared_panel, 'navigate_to'):
            try:
                prepared_panel.navigate_to(target_tab, target_section)
            except Exception:
                pass
        ...
        try:
            ...
            if reused and self.health_center_panel is not None:
                try:
                    self.health_center_panel.navigate_to(target_tab, target_section)
                except Exception:
                    pass
            ...
            panel = HealthCenterPanel(
                self, host, initial_tab=target_tab, initial_windows_section=target_section
            )
    ...
    # V164: entrar por Iniciar diagnóstico arranca una ejecución Total 6.0 nueva y conserva la evidencia final en la pestaña.
    def start_diagnostic_session(self, force_new=False):
        ...
        if (self.diagnostic_session.active or self._complete_diagnostic_running
                or (self._diagnostic_run and self._diagnostic_run.state == DiagnosticState.PREPARING)):
            try:
                panel = show_diagnostic_experience(self)
                if self._complete_diagnostic_running or (self._diagnostic_run and self._diagnostic_run.state == DiagnosticState.PREPARING):
                    panel.update_complete_progress({
                        'progress': self._complete_diagnostic_progress,
                        'stage': self._complete_diagnostic_stage or 'Diagnóstico completo',
                        'detail': 'CorePulse continúa consolidando evidencia real.',
                    })
                else:
                    state = self.realtime_agent.get_state()
                    info = dict(self.diagnostic_session.readiness(state))
                    info['progress'] = float(info.get('progress', 0.0) or 0.0) * 0.22
                    info['complete_stage'] = 'Estado en escritorio'
        ...
        self._complete_diagnostic_running = False
        self._complete_diagnostic_progress = 0.0
        self._complete_diagnostic_stage = 'Estado en escritorio'
        self._complete_diagnostic_started_at = time.time()
        self._stop_diagnostic_callbacks()
        self._diagnostic_run_token += 1
        self._diagnostic_run = DiagnosticRun(self._diagnostic_run_token)
        self._diagnostic_cancel_event = self._diagnostic_run.cancel_event
        diagnostic_panel = show_diagnostic_experience(self)
        diagnostic_panel.reset_for_run()
        try:
        ...
        self.btn_diagnostic.configure(state='normal', text='Diagnóstico completo 0%', fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#b8c4d4'))
        refresh_navigation_state(self)
        self._prepare_diagnostic_run(self._diagnostic_run_token)

    def _stop_diagnostic_callbacks(self):
        for name in ('diagnostic_after_id', '_diagnostic_poll_id'):
    ...
    def _prepare_diagnostic_run(self, run_token):
        self._diagnostic_poll_id = None
        if run_token != self._diagnostic_run_token or not self.is_running:
            return
        run = self._diagnostic_run
        if run.cancel_event.is_set():
            return
        previous = self._diagnostic_worker
        if previous is not None and previous.is_alive():
            # Acepta Repetir inmediatamente, pero no mezcla la nueva línea base
            # con la carga que está terminando de la sesión anterior.
            panel = self.diagnostic_experience_panel
            if panel is not None and panel.winfo_exists():
                panel.update_complete_progress({'progress': 0, 'stage': 'Preparando nueva sesión',
                    'detail': 'Esperando que se liberen las cargas de la ejecución anterior…'})
            self._diagnostic_poll_id = self.after(100, lambda t=run_token: self._prepare_diagnostic_run(t))
            return
        self._diagnostic_worker = None
        run.transition(DiagnosticState.RUNNING_BASELINE)
        self.diagnostic_session.start()
        self._update_diagnostic_countdown(run_token)

    def _poll_diagnostic_worker(self, run_token):
        self._diagnostic_poll_id = None
        if run_token != self._diagnostic_run_token or not self.is_running:
            return
        run = self._diagnostic_run
        progress, result = run.drain()
        if progress is not None:
            self._apply_complete_diagnostic_progress(*progress, run_token=run_token)
        if result is not None:
            self._complete_diagnostic_finished(*result, run_token=run_token)
            return
        if not run.cancel_event.is_set() and self._complete_diagnostic_running:
            self._diagnostic_poll_id = self.after(100, lambda t=run_token: self._poll_diagnostic_worker(t))

    def cancel_diagnostic_session(self):
        """Cancela el diagnóstico actual sin publicar evidencia parcial como resultado final."""
        active = bool(getattr(self.diagnostic_session, 'active', False)) if self.diagnostic_session is not None else False
        run = self._diagnostic_run
        preparing = bool(run and run.state == DiagnosticState.PREPARING)
        if not active and not self._complete_diagnostic_running and not preparing:
            return
        if run is not None:
            run.cancel()
        try:
            ...
            pass
        self._stop_diagnostic_callbacks()
        # Invalida callbacks del worker en curso; el hilo termina por cancelación cooperativa.
        self._diagnostic_run_token += 1
        if self.diagnostic_after_id is not None:
            try:
                self.after_cancel(self.diagnostic_after_id)
            except Exception:
                pass
            self.diagnostic_after_id = None
        try:
            cancel = getattr(self.diagnostic_session, 'cancel', None)
            if callable(cancel):
                cancel()
        except Exception:
            pass
        ...
        self.diagnostic_score_snapshot = None
        try:
            self.btn_diagnostic.configure(width=220, state='normal', text='Iniciar diagnóstico', fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#f4f7fb'))
            self.btn_pdf.configure(state='disabled', text='PDF · al finalizar')
        except Exception:
            pass
        refresh_navigation_state(self)
    ...
    def _update_diagnostic_countdown(self, run_token=None):
        if run_token is not None and run_token != self._diagnostic_run_token:
            return
        self.diagnostic_after_id = None
        if not self.is_running:
            return
        if self.diagnostic_session.active:
            try:
                ...
                state = {}
            raw_info = self.diagnostic_session.readiness(state)
            info = dict(raw_info)
            # La observación pasiva representa el 22% del diagnóstico completo.
            complete_progress = float(raw_info.get('progress', 0.0) or 0.0) * 0.22
            info['progress'] = complete_progress
            info['complete_stage'] = 'Estado en escritorio'
            progress = int(round(complete_progress * 100.0))
            progress = max(0, min(22, progress))
            panel = getattr(self, 'diagnostic_experience_panel', None)
            try:
                if panel is not None and panel.winfo_exists():
                    panel.update_progress(info, state)
                ...
                pass
            self.btn_diagnostic.configure(width=220, text=f'Diagnóstico completo {progress:3d}%', fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#b8c4d4'))
            self.btn_pdf.configure(state='disabled')
            if self.diagnostic_session.should_finish(state):
                self._finish_diagnostic_session()
                return
            self.diagnostic_after_id = self.after(500, lambda t=self._diagnostic_run_token: self._update_diagnostic_countdown(t))
            return
        self.diagnostic_after_id = None

    def _finish_diagnostic_session(self):
        """Congela la fase pasiva y continúa con el Diagnóstico Total 6.0."""
        if not self.is_running or self._complete_diagnostic_running or not self.diagnostic_session.active:
            return
        try:
            state = self.realtime_agent.get_state()
        except Exception:
            state = {}
        base_result = self.diagnostic_session.finish(state)
        self.diagnostic_result = None
        self._diagnostic_run.transition(DiagnosticState.RUNNING_HARDWARE)
        self.diagnostic_telemetry_snapshot = copy.deepcopy(self.latest_telemetry if isinstance(self.latest_telemetry, dict) else {})
        if time.time() - getattr(self, '_storage_health_cache_at', 0) <= 360:
            self.diagnostic_telemetry_snapshot['_storage_health_cache'] = copy.deepcopy(getattr(self, 'storage_health_cache', {}))
        ...
        self._complete_diagnostic_running = True
        self._complete_diagnostic_progress = 0.22
        self._complete_diagnostic_stage = 'Componentes y sensores'
        self.btn_diagnostic.configure(width=220, state='normal', text='Diagnóstico completo 22%', fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#b8c4d4'))
        self.btn_pdf.configure(state='disabled', text='PDF · al finalizar')
        panel = getattr(self, 'diagnostic_experience_panel', None)
        try:
            if panel is not None and panel.winfo_exists():
                panel.update_complete_progress({'progress': 0.22, 'stage': 'Estado en escritorio completado', 'detail': 'Continúa: componentes → Windows/integridad → benchmark → estrés → ventiladores → audio → limpieza segura → IA/vigencia → resultado.'})
        except Exception:
            pass

        telemetry_snapshot = copy.deepcopy(self.diagnostic_telemetry_snapshot or {})
        disks_snapshot = copy.deepcopy(self.diagnostic_disks_snapshot or [])
        run_token = self._diagnostic_run_token
        run = self._diagnostic_run
        ...
        def telemetry_sampler():
            try:
                with self.telemetry_lock:
                    return copy.deepcopy(self.latest_telemetry if isinstance(self.latest_telemetry, dict) else {})
            except Exception:
                return {}

        def progress_callback(local_fraction, stage, detail=''):
            overall = 0.22 + 0.78 * max(0.0, min(1.0, float(local_fraction)))
            run.post_progress(overall, str(stage), str(detail or ''))

        def worker():
            try:
                from core.complete_diagnostic import run_complete_diagnostic
                result = run_complete_diagnostic(
                    base_result, telemetry_snapshot, disks_snapshot,
                    telemetry_sampler=telemetry_sampler, progress_callback=progress_callback,
                    cancel_check=cancel_event.is_set,
                    state_callback=run.transition,
                )
                error = None
            except Exception as exc:
                logger.exception('[DIAGNOSTIC_V150] Falló el diagnóstico completo')
                result = copy.deepcopy(base_result)
                result['overall_status'] = 'NO_EVALUABLE'
                result.setdefault('findings', []).append({
                    'component': 'COREPULSE', 'status': 'NO_EVALUABLE',
                    'title': 'Diagnóstico completo incompleto',
                    'explanation': 'Una fase interna no pudo finalizar. CorePulse conserva la evidencia disponible sin atribuir un fallo al hardware.',
                    'evidence': [f'{type(exc).__name__}: {exc}'],
                    'rule_source': 'CompleteDiagnosticSession safety fallback',
                })
                result['complete_diagnostic'] = {
                    'version': '6.0-v164', 'status': 'ERROR',
                    'error': f'{type(exc).__name__}: {exc}',
                    'repair_actions_executed': False, 'pdf_optional': False, 'finalized': False,
                }
                error = str(exc)
            run.post_result(result, error, state)

        run.worker = threading.Thread(target=worker, name='CorePulse-CompleteDiagnostic', daemon=True)
        self._diagnostic_worker = run.worker
        run.worker.start()
        self._diagnostic_poll_id = self.after(100, lambda t=run_token: self._poll_diagnostic_worker(t))

    def _apply_complete_diagnostic_progress(self, progress, stage, detail='', run_token=None):
        if run_token is not None and run_token != self._diagnostic_run_token:
            return
        if not self.is_running or not self._complete_diagnostic_running:
            return
        self._complete_diagnostic_progress = max(0.22, min(0.995, float(progress)))
        self._complete_diagnostic_stage = str(stage or 'Diagnóstico completo')
        pct = int(round(self._complete_diagnostic_progress * 100.0))
        try:
            self.btn_diagnostic.configure(text=f'Diagnóstico completo {pct:3d}%')
        except Exception:
            pass
        panel = getattr(self, 'diagnostic_experience_panel', None)
        try:
            if panel is not None and panel.winfo_exists():
                panel.update_complete_progress({
                    'progress': self._complete_diagnostic_progress,
                    'stage': self._complete_diagnostic_stage,
                    'detail': detail,
                })
        except Exception:
            pass

    def _complete_diagnostic_finished(self, result, error=None, state=None, run_token=None):
        if run_token is not None and run_token != self._diagnostic_run_token:
            return
        if not self.is_running:
            return
        run = self._diagnostic_run
        if run is None or run.cancel_event.is_set() or run.state in {
                DiagnosticState.COMPLETED, DiagnosticState.CANCELLED, DiagnosticState.ERROR}:
            return
        if not is_finalized_result(result):
            block = (result or {}).get('complete_diagnostic') or {}
            self.cancel_diagnostic_session()
            if str(block.get('status') or '').upper() != 'CANCELLED':
                run.state = DiagnosticState.ERROR
                panel = self.diagnostic_experience_panel
                if panel is not None and panel.winfo_exists():
                    panel.show_error(error or block.get('error') or 'No se pudo finalizar el diagnóstico.')
            return
        run.transition(DiagnosticState.COMPLETED)
        self._stop_diagnostic_callbacks()
        self._complete_diagnostic_running = False
        self._complete_diagnostic_progress = 1.0
        self._complete_diagnostic_stage = 'Diagnóstico completo'
        self.diagnostic_result = result if isinstance(result, dict) else {}
        try:
            from core.diagnostic_history import load_previous_complete_result, build_diagnostic_comparison
            previous = load_previous_complete_result()
            complete_block = self.diagnostic_result.get('complete_diagnostic') if isinstance(self.diagnostic_result.get('complete_diagnostic'), dict) else None
            if complete_block is not None:
                complete_block['comparison'] = build_diagnostic_comparison(self.diagnostic_result, previous)
        except Exception:
            logger.exception('[DIAGNOSTIC_V150] No se pudo construir comparación con el diagnóstico anterior')
        try:
            from core.diagnostic_pipeline import integrate_current_diagnostic_pipeline
            self.current_recommendation_pipeline = integrate_current_diagnostic_pipeline(
                self.diagnostic_result,
                copy.deepcopy(self.diagnostic_telemetry_snapshot or {}),
                copy.deepcopy(self.diagnostic_disks_snapshot or []),
                output_path=str(data_path('current_diagnostic_recommendations.json')),
            )
            self.diagnostic_result['_intelligent_recommendations'] = self.current_recommendation_pipeline
        except Exception as recommendation_error:
            self.current_recommendation_pipeline = {
                'version': VERSION_LABEL, 'status': 'ERROR', 'scope': 'CURRENT_DIAGNOSTIC_ONLY',
                'error': str(recommendation_error), 'recommendation_count': 0,
                'history_used_as_current_fault_source': False,
            }
            self.diagnostic_result['_intelligent_recommendations'] = self.current_recommendation_pipeline
        try:
            from core.complete_diagnostic import save_complete_result
            self.diagnostic_json_path = save_complete_result(self.diagnostic_result)
        except Exception:
            self.diagnostic_json_path = None
            logger.exception('[DIAGNOSTIC_V157] No se pudo guardar el resultado completo')
        try:
            if self.session_trend_collector is not None:
                self.session_trend_collector.record_diagnostic(self.diagnostic_result, state=state or {})
                self._refresh_session_trends_ui()
        except Exception:
            pass
        # El benchmark ejecutado dentro del diagnóstico también forma parte del
        # historial de benchmark: no se pierde por haber sido orquestado desde
        # el botón de diagnóstico completo.
        try:
            complete = self.diagnostic_result.get('complete_diagnostic') if isinstance(self.diagnostic_result.get('complete_diagnostic'), dict) else {}
            benchmark = complete.get('benchmark') if isinstance(complete.get('benchmark'), dict) else None
            store = getattr(self, 'health_history_store', None)
            if is_completed_benchmark(benchmark) and store is not None and hasattr(store, 'record_benchmark_session'):
                tele = self.diagnostic_telemetry_snapshot if isinstance(self.diagnostic_telemetry_snapshot, dict) else {}
                gpu_result = benchmark.get('gpu') if isinstance(benchmark.get('gpu'), dict) else {}
                store.record_benchmark_session(
                    benchmark,
                    profile=str(benchmark.get('profile') or 'STANDARD'),
                    components=list(benchmark.get('selected_components') or ('cpu', 'ram', 'ssd', 'gpu')),
                    comparison={},
                    hardware={
                        'cpu_name': tele.get('cpu_name'),
                        'gpu_name': tele.get('gpu_name'),
                        'renderer': gpu_result.get('renderer'),
                        'source': 'COMPLETE_DIAGNOSTIC_V150',
                    },
                )
        except Exception:
            logger.exception('[DIAGNOSTIC_V150] No se pudo guardar el benchmark integrado en historial')

        status = self.diagnostic_result.get('overall_status', 'NO_EVALUABLE')
        self.btn_diagnostic.configure(width=220, state='normal', text='Iniciar diagnóstico', fg_color='transparent', hover_color=theme_color('#14253b'), text_color=theme_color('#f4f7fb'))
        ...
        if not panel_visible:
            complete = self.diagnostic_result.get('complete_diagnostic') if isinstance(self.diagnostic_result.get('complete_diagnostic'), dict) else {}
            total_duration = self.diagnostic_result.get('complete_duration_seconds') or self.diagnostic_result.get('duration_seconds') or 0
            message = (
                f'Resultado: {status}\n'
                f'Duración total: {float(total_duration):.0f} s\n'
                f'Fases: telemetría + componentes + Windows/integridad + benchmark + estrés + ventiladores + audio + limpieza segura + IA/vigencia\n\n'
                f'El PDF es opcional y ya está habilitado.'
            )
            if error:
                message += f'\n\nParte del diagnóstico no pudo completarse: {error}'
            if self.diagnostic_json_path:
                message += f'\n\nEvidencia guardada en:\n{self.diagnostic_json_path}'
            cp_info(self, 'Diagnóstico completo finalizado', message)
    ...
    def export_pdf_report(self):
        ...
        if self._pdf_export_in_progress:
            ...
            return
        if not is_finalized_result(self.diagnostic_result):
            if self.diagnostic_session.active:
                try:
                    state = self.realtime_agent.get_state()
                except Exception:
                ...
                detail = 'Completa el diagnóstico de CorePulse. El PDF estará disponible al finalizar escritorio, hardware, Windows, benchmark, telemetría y correlación.'
    ...
    def _restore_pdf_button(self):
        if self.is_running:
            try:
                ready = is_finalized_result(self.diagnostic_result)
                self.btn_pdf.configure(state='normal' if ready else 'disabled',
                                       text='Generar PDF · listo' if ready else 'PDF · al finalizar')
            except Exception:
                pass
            panel = getattr(self, 'diagnostic_experience_panel', None)
            try:
                if panel is not None and panel.winfo_exists():
                    if is_finalized_result(self.diagnostic_result):
                        panel.set_pdf_ready()
                    else:
                        panel.btn_pdf.configure(state='disabled', text='PDF · al finalizar')
            except Exception:
                pass
            refresh_navigation_state(self)
    ...
    def create_metric_card(self, parent, title):
        card = ctk.CTkFrame(parent, fg_color=BG_CARD, border_width=1, border_color=BORDER_COLOR, corner_radius=16)
        ...
        try:
            ...
            while self.is_running:
                ...
                try:
                    ...
                    # V321: en el primer ciclo hacemos únicamente la lectura NVMe
                    # nativa rápida antes de construir las tarjetas. Así el splash
                    # absorbe el warm-up y el Resumen nace con el % real cuando el
                    # firmware lo expone, en vez de corregirse 1-3 s después.
                    if not self._startup_first_telemetry_acquired:
                        try:
                            from core.storage_summary_health import collect_storage_summary_health
                            fast_health = collect_storage_summary_health(telemetry, include_slow_fallbacks=False)
                            if isinstance(fast_health, dict) and fast_health:
                                self._merge_storage_health_records(fast_health)
                            self._startup_storage_health_ready = self._storage_health_ready_for_telemetry(telemetry)
                            if not self._startup_storage_health_ready:
                                if not self._startup_storage_wait_started:
                                    self._startup_storage_wait_started = time.monotonic()
                                self._start_startup_storage_health_worker(telemetry)
                        except Exception:
                            self._log_throttled_exception('startup_storage_health', 'No se pudo precargar salud NVMe durante el arranque')
                ...
                # Durante el gate tomamos la segunda muestra enseguida para que
                # CPU/GPU/frecuencia lleguen estabilizados antes de mostrar la UI.
                startup_pending = not bool(getattr(self, '_startup_first_telemetry_applied', False))
                target_period = 0.30 if startup_pending else 1.0
                sleep_time = max(0.05 if startup_pending else 0.2, target_period - elapsed)
    ...
    def _complete_startup_after_first_telemetry(self, telemetry):
        """Completa el gate después de dos muestras reales ya renderizadas."""
        if self._startup_first_telemetry_applied or not isinstance(telemetry, dict):
            return
        self._startup_ui_sample_count = int(getattr(self, '_startup_ui_sample_count', 0) or 0) + 1
        primary_keys = ('cpu_usage', 'ram_usage', 'gpu_usage', 'cpu_temp', 'gpu_temp', 'cpu_ghz')
        available = sum(1 for key in primary_keys if telemetry.get(key) is not None)
        if self._startup_ui_sample_count < 2:
            self.update_startup_component(
                'services', 0.96, 'Estabilizando sensores…',
                f'Primera lectura real aplicada · {available} métricas principales disponibles.'
            )
            return
        storage_ready = self._storage_health_ready_for_telemetry(telemetry)
        if not storage_ready:
            if not self._startup_storage_wait_started:
                self._startup_storage_wait_started = time.monotonic()
                self._start_startup_storage_health_worker(telemetry)
            waited = max(0.0, time.monotonic() - float(self._startup_storage_wait_started or 0.0))
            if waited < 4.0 and getattr(self, '_startup_storage_health_worker_running', False):
                self.update_startup_component(
                    'services', 0.98, 'Leyendo salud de almacenamiento…',
                    'Esperando el porcentaje SMART/NVMe real antes de mostrar el Resumen.'
                )
                return
        ...
        self._startup_mark(
            ...
            storage_health_ready=bool(self._storage_health_ready_for_telemetry(telemetry)),
    ...
    def apply_telemetry_to_ui(self, telemetry, disks):
        ...
        try:
            ...
            if not getattr(self, '_dashboard_telemetry_wrapped', False):
                configure_changed(self.lbl_cpu_title, text=f"CPU: {telemetry['cpu_name']}")
            cpu_raw = telemetry.get('cpu_usage')
            cpu_usage = max(0.0, min(100.0, float(cpu_raw))) if cpu_raw is not None else None
            configure_changed(self.lbl_cpu, text=f'{cpu_usage:.1f}%' if cpu_usage is not None else 'N/A')
            if cpu_usage is not None:
                if not self.bar_cpu.winfo_manager():
                    self.bar_cpu.pack(fill='x', padx=10, pady=(0, 4), before=self.lbl_cpu_temp)
                set_changed(self.bar_cpu, cpu_usage / 100.0)
            else:
                self.bar_cpu.pack_forget()
            cpu_temp = telemetry.get('cpu_temp')
            configure_changed(self.lbl_cpu_temp, text=f'Temp: {cpu_temp:.1f} °C' if cpu_temp is not None else 'Temp: N/A')
            ram_raw = telemetry.get('ram_usage')
            ram_usage = max(0.0, min(100.0, float(ram_raw))) if ram_raw is not None else None
            configure_changed(self.lbl_ram, text=f'{ram_usage:.1f}%' if ram_usage is not None else 'N/A')
            if ram_usage is not None:
                if not self.bar_ram.winfo_manager():
                    self.bar_ram.pack(fill='x', padx=10, pady=(0, 4), before=self.lbl_ram_gb)
                set_changed(self.bar_ram, ram_usage / 100.0)
            else:
                self.bar_ram.pack_forget()
            ram_used = telemetry.get('ram_used_gb')
            ram_total = telemetry.get('ram_total_gb')
            configure_changed(self.lbl_ram_gb, text=f'{ram_used:.2f} GB / {ram_total:.2f} GB' if ram_used is not None and ram_total is not None else 'N/A')
            if not getattr(self, '_dashboard_telemetry_wrapped', False):
                configure_changed(self.lbl_gpu_title, text=f"GPU: {telemetry['gpu_name']}")
            gpu_raw = telemetry.get('gpu_usage')
            gpu_usage = max(0.0, min(100.0, float(gpu_raw))) if gpu_raw is not None else None
            configure_changed(self.lbl_gpu, text=f'{gpu_usage:.1f}%' if gpu_usage is not None else 'N/A')
            if gpu_usage is not None:
                if not self.bar_gpu.winfo_manager():
                    self.bar_gpu.pack(fill='x', padx=10, pady=(0, 4), before=self.lbl_gpu_temp)
                set_changed(self.bar_gpu, gpu_usage / 100.0)
            else:
                self.bar_gpu.pack_forget()
            gpu_temp = telemetry.get('gpu_temp')
            configure_changed(self.lbl_gpu_temp, text=f'Temp: {gpu_temp:.1f} °C' if gpu_temp is not None else 'Temp: N/A')
            try:
            ...
            score = calculate_preliminary_score(telemetry['cpu_usage'], telemetry['ram_usage'], telemetry['cpu_temp'], telemetry['gpu_temp'], disks)
            self.latest_score = score
            if not getattr(self, '_live_health_binding_active', False):
                if isinstance(score, (int, float)):
                    configure_changed(self.lbl_health_val, text=f'{score:.1f}%')
                    if score < 50:
                        configure_changed(self.lbl_health_status, text='ESTADO CRÍTICO', text_color='#ef4444')
                        configure_changed(self.lbl_health_val, text_color='#ef4444')
                    elif score < 70:
                        configure_changed(self.lbl_health_status, text='ADVERTENCIA', text_color='#f59e0b')
                        configure_changed(self.lbl_health_val, text_color='#f59e0b')
                    elif score < 85:
                        configure_changed(self.lbl_health_status, text='ESTADO ESTABLE', text_color='#38bdf8')
                        configure_changed(self.lbl_health_val, text_color='#38bdf8')
                    else:
                        configure_changed(self.lbl_health_status, text='ESTADO ÓPTIMO', text_color=COLOR_RAM)
                        configure_changed(self.lbl_health_val, text_color=COLOR_RAM)
                else:
                    configure_changed(self.lbl_health_val, text='N/A', text_color=COLOR_TEXT_DIM)
                    configure_changed(self.lbl_health_status, text='NO EVALUABLE', text_color=COLOR_TEXT_DIM)
    ...
    def open_smart_alert_window(self):
        ...
        try:
            ...
            cp_error(self, 'Alertas técnicas', f'No se pudo abrir Alertas técnicas:\n\n{exc}')
    ...
    def restore_from_tray(self):
        if not getattr(self, 'is_running', False):
            return
        self._minimized_to_tray = False
        restore_state = str(getattr(self, '_tray_restore_state', '') or '')
        restore_geometry = getattr(self, '_tray_restore_geometry', None)
        try:
            self.deiconify()
            if restore_state == 'zoomed':
                try:
                    ...
                    pass
            elif restore_state == 'normal':
                try:
                    self.state('normal')
                    if restore_geometry:
                        self.geometry(str(restore_geometry))
                except Exception:
                    pass
            self.lift()
            self.focus_force()
    ...
    def minimize_to_tray(self):
        ...
        try:
            self._tray_restore_state = str(self.state() or 'normal')
            self._tray_restore_geometry = self.geometry()
        except Exception:
            self._tray_restore_state = 'normal'
            self._tray_restore_geometry = None
        try:
            if tray_ready:
                self.withdraw()
                tray.notify_minimized()
            else:
                # Si la bandeja todavía no terminó de iniciar, nunca cerramos
                # el proceso por un clic en X: queda minimizado en la barra.
                self.iconify()
        except Exception:
            try:
                self.iconify()
    ...
    def on_close(self):
        ...
        from gui.resize_render_guard import release as release_resize_guard
        release_resize_guard(self)
        self.cancel_diagnostic_session()
        self._stop_diagnostic_callbacks()
        self.is_running = False
        for attr in ('resize_timer', '_resize_after_id', 'telemetry_after_id', 'chart_after_id', 'diagnostic_after_id', 'agent_after_id', '_battery_health_refresh_after_id', '_recovery_heartbeat_after'):
