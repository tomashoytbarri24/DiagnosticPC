...
def main():
    ...
    results = [
        ...
        check('agent_card_no_false_normal_state', "'NINGUNA SOSTENIDA': 'Sin alertas sostenidas'" in dashboard_layout and 'NINGUNA SOSTENIDA' in agent_reaction and 'CRÍTICA INSTANTÁNEA' in agent_reaction),
        check('cpu_transition_loader', 'Cargando información del procesador…' in main_text and 'commit_internal_page(self, page_key, host, panel=None)' in main_text and 'self._defer_ui_call(build_panel, 16)' in main_text),
    ]
    ok = all(results)
