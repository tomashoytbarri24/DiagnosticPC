...
Versión actual: **V161**, basada directamente en CorePulse_V160.zip.

El Diagnóstico Completo reúne estado en escritorio, hardware/batería/almacenamiento, Windows, benchmark existente de CPU/RAM/SSD/GPU, análisis de telemetría durante esas cargas y correlación final. No ejecuta pruebas de estrés independientes ni espera un cooldown posterior al estrés.

Abre **Iniciar_CorePulse.bat** desde la carpeta completa **v161**. La entrega incluye **CorePulse_V161.zip** y **VALIDACION_V161.md**, con resultados, limitaciones y pasos de prueba.

La protección térmica usa los límites existentes; cancelar impide exportar el diagnóstico o registrar el benchmark como completado. Los sensores no disponibles permanecen N/A. Los cinco archivos protegidos permanecen idénticos a V160.

## Historial conservado

## V158 — Redibujo CTk coalescido durante resize
- `gui/resize_render_guard.py` coalesce los redibujos internos de CustomTkinter (`CTkBaseClass._draw`) mientras la ventana está en resize activo, aplicándolos una sola vez por widget al soltar el borde en vez de una vez por píxel arrastrado.
- Complementa el detector trailing de V148 (que sólo protegía el trabajo propio de CorePulse) cubriendo el costo interno de CustomTkinter, que quedaba fuera de ese debounce.
- Ciclo de vida del Diagnóstico Completo, publicación Git, drivers, benchmark y bandeja de V157 quedan sin cambios de comportamiento.

## V137 — Historial de benchmark 2.0
- Benchmark incorpora una vista propia `Historial` junto a `Ejecutar`, sin duplicar el almacenamiento introducido en V114.
- Lista hasta 100 sesiones recientes, con filtros por perfil y componente, fecha/hora, duración, renderer OpenGL real y métricas persistidas.
- `Ver detalle` expone CPU/GPU supervisada, renderer, componentes, estado y resultados de la sesión seleccionada.
- La comparación automática sólo calcula variaciones contra una ejecución equivalente del mismo perfil/componentes y hardware compatible.
- Nuevas sesiones guardan también el renderer en la identidad de hardware para evitar comparar GPUs OpenGL distintas cuando ambas están identificadas.
- REAL_OR_NA se mantiene: no hay rankings externos, estimaciones ni métricas rellenadas.
