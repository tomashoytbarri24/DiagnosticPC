...
def build_component_assessments(result: Dict[str, Any]) -> List[Dict[str, Any]]:
    ...
    def add(key, title, status, summary, reason, evidence, facets, action_text):
        reports.append({
            ...
            'evidence': [str(x) for x in evidence if str(x or '').strip()][:3],
            'facets': list(facets)[:3],
    ...
    gpu_state = _component_state(gpu_find, gpu_load, 'gpu')
    gpu_ev = []
    ...
    add('gpu', 'GPU', gpu_state,
        _finding_summary(gpu_find, 'Sin alertas observadas bajo carga' if gpu_state == 'NORMAL' else 'Evidencia parcial'),
        _finding_reason(gpu_find, 'Rendimiento y comportamiento observados durante la misma carga de benchmark; no certifica estabilidad prolongada.'), gpu_ev,
        [('Escritorio', 'MEDIDO' if _desktop_gpu_available(result) else 'N/A', 'INFO' if _desktop_gpu_available(result) else 'NO_EVALUABLE'),
...
def build_component_evidence(result: Dict[str, Any], key: str) -> Dict[str, Any]:
    ...
    if key == 'audio':
        ...
        sections.append(_section('Benchmark', [
            ('Estado', str(bench.get('status') or 'N/A').upper()),
            ('Resultado', f"{_fmt_metric(bench.get('value'), 1)} {str(bench.get('unit') or '').strip()}".strip()),
            ('Escenas', ', '.join(str(x.get('key')) for x in (bench.get('scene_results') or []) if isinstance(x, dict)) or 'N/A'),
            ...
            ('Duración', _fmt_metric(bench.get('duration_s'), 1, ' s')),
        ], note='Rendimiento medido por escenas DirectX 11 progresivas de CorePulse; el valor principal corresponde a la escena High y no equivale a los FPS de un juego concreto.'))
        ...
        for idx, drive in enumerate(drives, 1):
            ...
            health_rows.extend([
                ...
                ('Estado Windows', drive.get('windows_health_status')),
    ...
    sections.sort(key=lambda section: order.get(section['title'], 3))
    return {
        'key': key,
...
def build_diagnostic_overview(result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if counts['CRITICAL']:
        ...
        summary = f"Sin alertas prioritarias en lo evaluable; {counts['NO_EVALUABLE']} área(s) quedaron con evidencia limitada."
    else:
        summary = 'Sin acciones prioritarias detectadas en las áreas evaluadas.'

    phase_text = f'{completed}/{total_phases} fases' if total_phases else 'Cobertura de fases N/A'
