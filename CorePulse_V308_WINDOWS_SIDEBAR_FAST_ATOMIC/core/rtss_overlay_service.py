...
class RTSSOverlayService:
    ...
    def __init__(self):
        ...
        self.last_app = None
        self.last_text = None
        self._last_rtss_start_attempt = 0.0
        self._rtss_start_result = None
        if not self.rtss.available:
            self._ensure_rtss_started(force=True)
        self.thread = threading.Thread(target=self._loop, daemon=True, name='CorePulse-RTSS')
        self.thread.start()

    def _ensure_rtss_started(self, force=False):
        now = time.monotonic()
        if not force and now - self._last_rtss_start_attempt < 3.0:
            return self._rtss_start_result
        self._last_rtss_start_attempt = now
        try:
            self._rtss_start_result = start_rtss()
        except Exception as exc:
            self._rtss_start_result = (False, f'{type(exc).__name__}: {exc}')
        return self._rtss_start_result
    ...
    def _loop(self):
        while self.running:
            try:
                if not self.rtss.available:
                    started, detail = self._ensure_rtss_started() or (False, 'Sin resultado de inicio')
                    self.last_error = (
                        'RTSS iniciándose automáticamente…'
                        if started
                        else f'RTSS no disponible: {detail}'
                    )
                    time.sleep(0.5)
                    continue
