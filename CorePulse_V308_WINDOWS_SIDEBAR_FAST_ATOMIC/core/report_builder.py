...
def _header(identity: Dict[str, Any], score: Any, diag: Dict[str, Any], st):
    logo_path = resource_path('assets', 'CorePulseIcon.png')
...
def _diagnostic_summary(diag: Dict[str, Any], st):
    ...
    rows = [
        ...
        [status, base._duration(base._pick(diag, 'complete_duration_seconds', 'duration_seconds', 'elapsed_seconds')), _clean(base._pick(diag, 'sample_count')), _fmt(metrics['cpu_temp_avg'], ' °C'), _fmt(metrics['cpu_temp_max'], ' °C'), _fmt(metrics['ram_usage_avg'], '%'), _fmt(metrics['gpu_temp_max'], ' °C')],
    ]
...
def _complete_diagnostic_flowables(diag: Dict[str, Any], st):
    complete = diag.get('complete_diagnostic') if isinstance(diag.get('complete_diagnostic'), dict) else None
    if not complete:
        return []
    windows = complete.get('windows') if isinstance(complete.get('windows'), dict) else {}
    observation = complete.get('load_observation') or {}
    benchmark = complete.get('benchmark') if isinstance(complete.get('benchmark'), dict) else {}
    stress = complete.get('stress') if isinstance(complete.get('stress'), dict) else {}
    fans = complete.get('fan_test') if isinstance(complete.get('fan_test'), dict) else {}
    audio = complete.get('audio_test') if isinstance(complete.get('audio_test'), dict) else {}
    cleanup = complete.get('cleanup') if isinstance(complete.get('cleanup'), dict) else {}
    ai_analysis = complete.get('ai_analysis') if isinstance(complete.get('ai_analysis'), dict) else {}
    ...
    rows = [
        ...
        ['Escritorio', f"{_clean(diag.get('sample_count'))} muestras · {_clean((diag.get('adaptive_diagnostic') or {}).get('finish_reason'), 'N/A')}"],
        ['Hardware, batería y almacenamiento', _clean(((complete.get('phases') or {}).get('hardware') or {}).get('status'), 'N/A')],
        ['Windows', f"Inicio: {_clean(startup.get('count'))} · Servicios: {_clean(services.get('count'))} · Estabilidad: {_clean(stability.get('severity'), 'N/A')} · Drivers con problema: {_clean(drivers.get('device_problems'), '0')}"],
        ['Benchmark', f"{_clean(benchmark.get('profile_label') or benchmark.get('profile'), 'N/A')} · {_fmt(benchmark.get('duration_s'), ' s', 1)} · ranking externo: no"],
        ['Telemetría durante benchmark', _clean(observation.get('status'), 'N/A')],
        ['Estrés', f"{_clean(stress.get('status'), 'N/A')} · score de rendimiento: no"],
        ['Ventiladores', _clean(fans.get('status'), 'N/A')],
        ['Audio', _clean(audio.get('status'), 'N/A')],
        ['Limpieza segura', f"{_clean(cleanup.get('status'), 'N/A')} · archivos: {_clean(cleanup.get('deleted_files'), '0')} · bytes: {_clean(cleanup.get('deleted_bytes'), '0')}"],
        ['IA / vigencia', f"{_clean(ai_analysis.get('status'), 'N/A')} · año: {_clean(ai_analysis.get('current_year') or ai_analysis.get('year'), 'N/A')}"],
        ['Correlación', _clean(((complete.get('phases') or {}).get('correlation') or {}).get('status'), 'N/A')],
    ]
    out = [_section('3.1 Diagnóstico Total 6.0: fases y evidencia', st), Spacer(1, 1.5 * mm), _table(rows, [31 * mm, 146 * mm], st, font_scale='small'), Spacer(1, 1.8 * mm)]

    # V146: el PDF refleja el mismo informe vivo que la pantalla final.
    assessments = build_component_assessments(diag)
    overview = build_diagnostic_overview(diag)
    out += [_callout(
        'Resumen integral',
        f"{_clean(overview.get('headline'), 'N/A')}. {_clean(overview.get('summary'), '')}",
        st,
        accent=RED if overview.get('critical_count') else ORANGE if overview.get('warning_count') else BLUE_2 if overview.get('na_count') else GREEN,
        background=RED_SOFT if overview.get('critical_count') else ORANGE_SOFT if overview.get('warning_count') else BLUE_SOFT if overview.get('na_count') else GREEN_SOFT,
    ), Spacer(1, 1.5 * mm)]
    priority = select_priority_assessment(assessments)
    if priority:
        out += [_callout(
            'Prioridad del diagnóstico',
            f"{_clean(priority.get('title'))}: {_clean(priority.get('summary'))}. La reparación sigue siendo una acción explícita del usuario.",
            st, accent=RED if _clean(priority.get('status')).upper() == 'CRITICAL' else ORANGE,
            background=RED_SOFT if _clean(priority.get('status')).upper() == 'CRITICAL' else ORANGE_SOFT,
        ), Spacer(1, 1.5 * mm)]
    component_rows = [['Componente', 'Estado', 'Dimensiones', 'Evidencia']]
    state_names = {'NORMAL': 'SIN ALERTAS', 'WARNING': 'REVISAR', 'CRITICAL': 'CRÍTICO', 'NO_EVALUABLE': 'N/A', 'INFO': 'NO APLICA'}
    for item in assessments:
        facets = ' · '.join(f"{_clean(x[0])}: {_clean(x[1])}" for x in (item.get('facets') or []) if isinstance(x, (list, tuple)) and len(x) >= 2)
        evidence = ' · '.join(_clean(x) for x in (item.get('evidence') or [])) or _clean(item.get('reason'))
        component_rows.append([
            _clean(item.get('title')),
            state_names.get(_clean(item.get('status')).upper(), _clean(item.get('status'))),
            facets or 'N/A',
            evidence or 'N/A',
        ])
    out += [Paragraph('<b>Resultado por componente</b>', st['heading']), _table(component_rows, [25 * mm, 25 * mm, 62 * mm, 65 * mm], st, font_scale='small'), Spacer(1, 1.8 * mm)]
    coverage = complete.get('phase_coverage') if isinstance(complete.get('phase_coverage'), dict) else {}
    if coverage:
        out += [_callout(
            'Cobertura del diagnóstico',
            f"Fases registradas: {_clean(coverage.get('completed'), '0')}/{_clean(coverage.get('total'), '0')}. " + ('La ejecución fue parcial; los faltantes permanecen como N/A.' if coverage.get('partial') else 'La ejecución integral terminó sin fases faltantes.'),
            st, accent=BLUE_2, background=BLUE_SOFT,
        ), Spacer(1, 1.5 * mm)]
    ...
    if comparison.get('available'):
        ...
        if changes:
            text = ' · '.join(f"{_clean(x.get('title'))}: {_clean(x.get('previous'))} → {_clean(x.get('current'))}" for x in changes[:6])
        else:
            text = 'Sin cambios de estado en los seis componentes respecto del diagnóstico completo anterior.'
        out += [_callout('Comparación con diagnóstico anterior', text + ' El historial es informativo y no se usa para crear fallos actuales.', st, accent=CYAN, background=SOFT), Spacer(1, 1.5 * mm)]
    evidence_rows = [['Componente', 'Fase', 'Dato', 'Valor']]
    for item in assessments:
        detail = build_component_evidence(diag, _clean(item.get('key')).lower())
        for section in (detail.get('sections') or []):
            if not isinstance(section, dict):
                continue
            phase = _clean(section.get('title'), 'Evidencia')
    ...
    if len(evidence_rows) > 1:
        out += [Paragraph('<b>Evidencia detallada por componente</b>', st['heading']), _table(evidence_rows, [25 * mm, 28 * mm, 48 * mm, 76 * mm], st, font_scale='small'), Spacer(1, 1.8 * mm)]
    bench_rows = [['Componente', 'Estado', 'Resultado medido']]
    for key, label in (('cpu','CPU'), ('ram','RAM'), ('ssd','SSD'), ('gpu','GPU')):
        item = benchmark.get(key) if isinstance(benchmark.get(key), dict) else {}
        value = item.get('value')
        unit = _clean(item.get('unit'), '')
        measured = f"{_fmt(value, (' ' + unit) if unit else '', 2)}" if _num(value) is not None else _clean(item.get('reason'), 'N/A')
        bench_rows.append([label, _clean(item.get('status'), 'N/A'), measured])
    out += [Paragraph('<b>Benchmark medido</b>', st['heading']), _table(bench_rows, [30 * mm, 30 * mm, 117 * mm], st, font_scale='small'), Spacer(1, 1.8 * mm)]
    safety = _clean(benchmark.get('safety_stop'), '')
    if safety:
        out += [_callout('Protección térmica activada', safety, st, accent=RED, background=RED_SOFT), Spacer(1, 1.5 * mm)]
    cleanup_mb = None
    try:
        cleanup_mb = float(cleanup.get('deleted_bytes') or 0) / (1024 * 1024)
    except Exception:
        cleanup_mb = None
    cleanup_text = f"Limpieza segura registrada: {_clean(cleanup.get('deleted_files'), '0')} archivo(s) · {cleanup_mb:.1f} MB." if cleanup_mb is not None else 'Limpieza segura: N/A.'
    out += [Paragraph('El benchmark mide rendimiento y la prueba de estrés evalúa estabilidad bajo carga; son fases distintas. ' + cleanup_text + ' Las comprobaciones DISM/SFC/CHKDSK son diagnósticas y no reparan Windows automáticamente.', st['small'])]
    return out
...
def _build_story(telemetry: Dict[str, Any], disks: List[Dict[str, Any]], score: Any, diagnostic_result: Dict[str, Any], ai: Dict[str, Any], ctx: Dict[str, Any], inventory: Dict[str, Any]):
    ...
    story += _complete_diagnostic_flowables(diag, st)
    if attention:
...
def build_pdf_report(telemetry, disks, score, output_path, diagnostic_result: Dict[str, Any], ai: Dict[str, Any], ctx: Dict[str, Any] | None = None):
    ...
    if diagnostic_result.get('complete_diagnostic') is not None:
        from core.diagnostic_lifecycle import is_finalized_result
        if not is_finalized_result(diagnostic_result):
            raise ValueError('Un diagnóstico cancelado o incompleto no puede generar un PDF final.')
