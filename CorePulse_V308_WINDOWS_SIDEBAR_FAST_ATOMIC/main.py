...
class App(ctk.CTk):
    ...
    def _sidebar_native_hwnds(self):
        """Devuelve los HWND de Tk que deben quedar congelados durante el reflow.

        En Windows, ``winfo_id`` suele apuntar al cliente Tk y su padre al HWND
        envolvente del toplevel. Se congelan ambos cuando existen para impedir que
        DWM publique dibujos parciales de widgets hijos durante el cambio de grid.
        """
        handles = []
        try:
            ...
            if client:
                ...
                parent = int(ctypes.windll.user32.GetParent(client) or 0)
    ...
    def _set_sidebar_native_redraw(self, enabled):
        """Suspende/reanuda el repintado nativo sin ocultar la UI con overlays."""
        WM_SETREDRAW = 0x000B
        try:
    ...
    def _flush_sidebar_native_redraw(self, handles=None):
        """Publica una sola imagen final de la ventana y todos sus hijos."""
        ...
        try:
            ...
            return
        handles = list(handles or self._sidebar_native_hwnds())
    ...
    def _finish_sidebar_native_transition(self, handles):
        """Publica rápido el estado final y difiere sólo el redraw de Matplotlib.

        V308 conserva el commit atómico de V306, pero elimina las dos fuentes de
        latencia del gesto: el relayout completo forzado y el render
        síncrono de Matplotlib. El árbol Tk queda estable mientras el HWND no repinta; luego
        Windows publica un único frame final y Matplotlib actualiza con
        ``draw_idle`` fuera del clic.
        """
        try:
            self.update_idletasks()
            try:
                from gui.dashboard_layout import _apply_layout
                # El tamaño de la ventana no cambia al ocultar/mostrar sidebar;
                # por tanto el breakpoint suele ser el mismo y el fast-path es
                # suficiente para sincronizar almacenamiento/charts.
                _apply_layout(self, force=False)
            except Exception:
                pass
            self.update_idletasks()
        ...
        # El redraw de la figura no forma parte del tiempo de respuesta del clic.
        def finish_charts():
            try:
        ...
        try:
            self.after_idle(finish_charts)
        except Exception:
            finish_charts()

    def _show_sidebar_transition_cover(self):
        """Muestra sólo un velo de borde; nunca oculta el dashboard completo."""
        cover = self._ensure_sidebar_transition_cover()
        try:
            cover.place(x=0, y=0, width=22, relheight=1.0, anchor='nw')
            self._layout_sidebar_transition_bands(22)
            cover.lift()
            # Sólo se procesa geometría pendiente; no se fuerza una pintura negra
            # de toda el área como ocurría con la máscara completa de V302.
            self.update_idletasks()
        except Exception:
            pass

    def _reveal_sidebar_transition_cover(self, duration_ms=64):
        """Retira el velo de borde con una contracción corta y discreta."""
        cover = getattr(self, '_sidebar_transition_cover', None)
        if cover is None:
        ...
        widths = (18, 13, 9, 5, 2)
        interval = max(10, int(duration_ms // len(widths)))
        ...
        def tick(index=0):
            ...
            width = widths[index]
            try:
                cover.place(x=0, y=0, width=width, relheight=1.0, anchor='nw')
                self._layout_sidebar_transition_bands(width)
                cover.lift()
            except Exception:
                self._clear_sidebar_transition_cover()
                return
            try:
                self._sidebar_transition_after_id = self.after(interval, lambda: tick(index + 1))
            except Exception:
                self._clear_sidebar_transition_cover()

        tick(0)

    def _settle_sidebar_transition(self):
        """Consolida la nueva geometría y retira la micro-transición."""
        self._sidebar_transition_after_id = None
        try:
            self.update_idletasks()
        except Exception:
        ...
        try:
            ...
            pass
        self._reveal_sidebar_transition_cover(duration_ms=64)

    def _sidebar_boundary_x(self):
        try:
            return int(self.sidebar.cget('width'))
        except Exception:
            return 230
    ...
    def _position_sidebar_toggle(self):
        try:
            ...
            pass
        button = getattr(self, '_sidebar_toggle_button', None)
    ...
    def _paint_sidebar_motion_layers(self, boundary_x, strength=1.0):
        """Compatibilidad con el antiguo efecto de borde; no altera geometría."""
        self._clear_sidebar_motion_layers()
        try:
            total_h = max(int(self.winfo_height()) - 24, 220)
        except Exception:
            total_h = 760
        strength = max(0.0, min(1.0, float(strength)))
        colors = ('#12365a', '#0e2947', '#0a1d33')
        widths = (6, 4, 2)
        offsets = (3, 9, 14)
        layers = []
        for color, layer_w, offset in zip(colors, widths, offsets):
            try:
                layer = ctk.CTkFrame(
                    self,
                    fg_color=color,
                    corner_radius=4,
                    width=max(1, int(layer_w * strength)),
                    height=total_h,
                )
                layer.place(x=int(boundary_x) + offset, y=12, anchor='nw')
                layer.lift()
                layers.append(layer)
            except Exception:
                pass
        self._sidebar_motion_layers = layers

    def _run_sidebar_motion_effect(self, start_x, end_x, duration_ms=130):
        """Compatibilidad con la estela histórica; V302 usa la máscara principal."""
        try:
        ...
        def tick(index=0):
            ...
            ratio = ease_out(raw)
            boundary = int(round(float(start_x) + delta * ratio))
            self._paint_sidebar_motion_layers(boundary, strength=max(0.0, 1.0 - raw))
            self._position_sidebar_toggle()
            if index >= steps:
                self._sidebar_animating = False
                self._clear_sidebar_motion_layers()
                return
            self._sidebar_anim_after_id = self.after(max(12, duration_ms // steps), lambda: tick(index + 1))

        tick(0)

    def set_sidebar_collapsed(self, collapsed, *, animated=True):
        """Oculta/muestra el sidebar sin exponer estados intermedios de layout.

        V308 vuelve a la estrategia estable de V306: no mueve el sidebar real ni
        expone geometrías intermedias. El repintado nativo se congela únicamente
        durante el commit corto de Tk/CTk; Matplotlib termina con draw_idle después
        de publicar el frame final para que el clic responda mucho más rápido.
        """
        ...
        # Limpieza defensiva de mecanismos históricos; ninguno participa en V306.
        try:
            self._clear_sidebar_transition_cover()
        except Exception:
            pass
        ...
        try:
            ...
            # Todo el settle ocurre de forma síncrona mientras el HWND no repinta.
            self._finish_sidebar_native_transition(handles)
        except Exception:
            self._layout_transition = False
            self._sidebar_animating = False
            try:
