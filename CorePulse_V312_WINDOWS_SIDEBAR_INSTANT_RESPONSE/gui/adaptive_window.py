...
AUTO_MAX_W = 1720
AUTO_MAX_H = 960
PRESETS = {'Automático': None, 'Compacto': (1180, 680), 'Recomendado': (1280, 720), 'Amplio': (1560, 860)}
LEGACY_PRESET_SIZES = {(1280, 740): ('Automático', None, None), (1400, 780): ('Automático', None, None), (1500, 830): ('Automático', None, None), (1280, 800): ('Automático', None, None), (1400, 850): ('Automático', None, None), (1500, 900): ('Automático', None, None), (1280, 720): ('Automático', None, None)}
...
def _settings_path():
    return data_path('window_preferences.json')
    ...
    try:
        ...
        if path.exists():
            ...
            if isinstance(raw, dict):
                defaults.update(raw)
                # V218 adopta una ventana inicial tipo Steam (1280x720). Las versiones
                # 145-216 podían guardar un preset Recomendado de 1560x860 o
                # similar, que en algunos equipos/DPI se veía casi maximizado.
                # Sólo migramos presets administrados por CorePulse; un tamaño
                # Personalizado escogido por el usuario se conserva intacto.
                try:
                    preset_name = str(raw.get('preset') or '')
                    rw = int(raw.get('width', 0) or 0)
                    rh = int(raw.get('height', 0) or 0)
                    if preset_name == 'Recomendado' and (rw > PREFERRED_W or rh > PREFERRED_H):
                        defaults['width'], defaults['height'] = (PREFERRED_W, PREFERRED_H)
                    elif preset_name == 'Automático':
                        defaults['width'], defaults['height'] = (PREFERRED_W, PREFERRED_H)
                except Exception:
                    pass
                try:
...
def _automatic_size(app):
    """Devuelve un tamaño de inicio tipo escritorio, no casi maximizado.

    V218: el modo Automático prioriza una geometría tipo Steam de 1280x720. Si el
    monitor es más pequeño, reduce la ventana respetando márgenes y mínimos.
    No expande CorePulse en pantallas grandes: para eso existe el preset Amplio
    o el botón nativo de maximizar.
    """
    sw, sh = _screen_bounds(app)
    max_w = max(960, sw - SCREEN_MARGIN_X)
    max_h = max(640, sh - SCREEN_MARGIN_Y)

    width = min(PREFERRED_W, max_w)
    height = min(PREFERRED_H, max_h)

    # En pantallas pequeñas, preservar margen visible sin
    # convertir la app en fullscreen.
    if max_w < PREFERRED_W or max_h < PREFERRED_H:
        scale = min(max_w / float(PREFERRED_W), max_h / float(PREFERRED_H), 1.0)
        width = int(PREFERRED_W * scale)
        height = int(PREFERRED_H * scale)
...
def apply_window_size(app, width, height, persist=False, preset=None):
    ...
    # Durante el Startup Gate la geometría se calcula sin publicar el Dashboard.
    if not getattr(app, '_startup_gate_active', False):
        try:
            app.state('normal')
        except Exception:
            pass
...
def apply_preferred_launch_geometry(app, *, force=False):
    """Aplica la geometría de inicio.

    `force=True` se usa al cerrar el Startup Gate para garantizar que Windows
    publique CorePulse en estado normal/centrado y no herede accidentalmente
    una geometría maximizada.
    """
    if getattr(app, '_launch_geometry_applied', False) and not force:
        return
