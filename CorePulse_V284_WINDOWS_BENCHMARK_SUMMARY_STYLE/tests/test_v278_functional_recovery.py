...
def test_requested_v278_ui_contracts():
    ...
    assert '_render_driver_update_center' in health
...
def test_v278_scroll_contracts_and_storage_alignment():
...
def test_v278_publish_ui_and_action_templates():
    ...
    assert 'CorePulse workflow schema: v2-long-folder' in dev_text
    assert "^CorePulse_V(\\d+)(?:_|$)" in dev_text
    assert "paths:" in dev_text and "CorePulse_V*/**" in dev_text
    assert 'actions/checkout@v7' in dev_text
    assert 'actions/setup-python@v7' in dev_text
    assert 'ubuntu-24.04' in dev_text
    assert 'CorePulse workflow schema: v2-long-folder' in stable_text
    assert 'CorePulse_V${VERSION}.zip' in dev_text
    assert 'CorePulse_V${VERSION}.zip' in stable_text


def test_v278_remote_branch_publish_preserves_local_checkout(tmp_path, monkeypatch):
    import subprocess
    ...
    source = tmp_path / 'CorePulse_V279_WINDOWS_VISUAL_CONSISTENCY_BENCHMARK_UPDATE_CLEANUP'
    (source / 'core').mkdir(parents=True)
    (source / 'main.py').write_text('print("V279")\n', encoding='utf-8')
    (source / 'core' / 'version.py').write_text('VERSION = "279"\n', encoding='utf-8')
    ...
    assert ctx['release_label'] == 'V283'

    result = pub.publish_current_version(
        repo, 'V278 test publish', target_branch='main', expected_context=ctx
    )
    ...
    # Main remoto contiene V278, pero conserva FASE/.github/versiones anteriores.
    git(repo, 'fetch', 'origin', 'main')
    tree = git(repo, 'ls-tree', '-r', '--name-only', 'origin/main').stdout.splitlines()
    assert 'CorePulse_V279_WINDOWS_VISUAL_CONSISTENCY_BENCHMARK_UPDATE_CLEANUP/main.py' in tree
    ...
    assert 'CorePulse_V279_WINDOWS_VISUAL_CONSISTENCY_BENCHMARK_UPDATE_CLEANUP/main.py' not in dev_tree



def test_v278_driver_counts_and_old_filter(monkeypatch):
...
def test_v278_retarget_is_local_and_instant(tmp_path, monkeypatch):
    import core.developer_publisher as pub
    repo = tmp_path / 'DiagnosticPC'
    (repo / '.github' / 'workflows').mkdir(parents=True)
    (repo / '.github' / 'workflows' / 'stable.yml').write_text('# CorePulse workflow schema: v2-long-folder\non:\n  push:\n    branches:\n      - main\n', encoding='utf-8')
    ctx = {
        ...
        'publish_folder': 'CorePulse_V279_WINDOWS_VISUAL_CONSISTENCY_BENCHMARK_UPDATE_CLEANUP',
    ...
    assert new['release_label'] == 'V283'
    assert new['workflow_detected'] is True
    assert not any(args and args[0] == 'ls-remote' for args in calls)


def test_v278_legacy_workflow_is_not_claimed_as_compatible(tmp_path):
    import core.developer_publisher as pub
    repo = tmp_path / 'DiagnosticPC'
    wf = repo / '.github' / 'workflows'
    wf.mkdir(parents=True)
    (wf / 'corepulse-development-release.yml').write_text(
        "name: CorePulse Development Release\n"
        "on:\n  push:\n    branches:\n      - '**/corepulse-dev'\n"
        "# old matcher: re.fullmatch(r'(?i)CorePulse_V(\\d+)', folder.name)\n",
        encoding='utf-8',
    )
    result = pub._workflow_support(repo, 'maxi/corepulse-dev')
    assert result['workflow_detected'] is False
    assert result.get('workflow_legacy') is True
    assert 'esquema antiguo' in result['workflow_note']


def test_v278_long_folder_workflow_contract():
    import re
    dev = (ROOT / 'developer' / 'github_actions_templates' / 'corepulse-development-release.yml').read_text(encoding='utf-8')
    rx = re.compile(r'(?i)^CorePulse_V(\d+)(?:_|$)')
    assert rx.match('CorePulse_V278_WINDOWS_RELEASE_AUTOMATION_FIX').group(1) == '278'
    assert rx.match('CorePulse_V182').group(1) == '182'
    assert not rx.match('FASE 1')
    assert "Publica una versión por commit" in dev
    assert "core/version.py" in dev
