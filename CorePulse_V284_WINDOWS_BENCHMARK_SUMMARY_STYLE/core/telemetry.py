...
def _cert_cpu_details(sensors):
    ...
    def _usable_cpu_temp(sensor):
        value = _finite(sensor.get('value')) if isinstance(sensor, dict) else None
        return sensor if value is not None and 0.0 < value <= 150.0 else None

    package = _usable_cpu_temp(_pick_named(temps, ['CPU Package', 'Package', 'Tctl/Tdie']))
    core_max = _usable_cpu_temp(_pick_named(temps, ['Core Max']))
    core_avg = _usable_cpu_temp(_pick_named(temps, ['Core Average']))
    ...
    if not core_clocks:
        # Algunos equipos Intel/OEM publican los relojes como Core #n o Core n.
        # Se usan sólo sensores Clock reales y se excluye explícitamente el bus.
        core_clocks = [
            s for s in clocks
            if 'bus' not in str(s.get('sensor_name', '')).lower()
            and _finite(s.get('value')) is not None
            and _finite(s.get('value')) > 0
        ]
    ...
    summary_temp = package or core_max or core_avg
    if summary_temp is None:
        valid_temps = [s for s in temps if _finite(s.get('value')) is not None and 0.0 < _finite(s.get('value')) <= 150.0]
        summary_temp = max(valid_temps, key=lambda item: _finite(item.get('value'))) if valid_temps else None
    ...
    result = {
        ...
        'summary_temp_c': round(summary_temp['value'], 1) if summary_temp else None,
    ...
    result['_metrics'] = {
        ...
        'summary_temp_c': _raw_meta(summary_temp, '°C'),
...
_ENUM_READY = threading.Event()
_ENUM_REFRESHING = False
...
def _refresh_enum_cache_worker():
    global _ENUM_REFRESHING
    try:
        # Inventario de SO es secundario: nunca debe bloquear el snapshot de sensores.
        gpu = _windows_gpu_inventory()
        storage = _windows_storage_inventory()
        with _ENUM_LOCK:
            _ENUM_CACHE['gpu'] = gpu
            _ENUM_CACHE['storage'] = storage
...
def _enum_cached(wait=False, force=False):
    global _ENUM_REFRESHING
    now = time.time()
    with _ENUM_LOCK:
        if force:
            _ENUM_CACHE['ts'] = 0.0
        fresh = _ENUM_CACHE['ts'] > 0.0 and now - _ENUM_CACHE['ts'] <= _ENUM_CACHE_SECONDS
        snapshot = copy.deepcopy(_ENUM_CACHE)
        if fresh:
            return snapshot
        if not _ENUM_REFRESHING:
            _ENUM_REFRESHING = True
            _ENUM_READY.clear()
            threading.Thread(target=_refresh_enum_cache_worker, daemon=True, name='CorePulse-HardwareInventory').start()
    if wait:
        _ENUM_READY.wait(8.0)
        with _ENUM_LOCK:
            return copy.deepcopy(_ENUM_CACHE)
    # La telemetría visible usa de inmediato LHM/psutil y se enriquece en el
    # siguiente ciclo cuando Win32 termine; evita 2 PowerShell síncronos al abrir.
    return snapshot
...
def get_universal_hardware_inventory(force_refresh=False):
    ...
    os_inv = _enum_cached(wait=True, force=bool(force_refresh))
...
def _certify_snapshot_without_inventory(snapshot):
    ...
    _apply_nested_certification(cpu, {'package_temp_c': '°C', 'summary_temp_c': '°C', 'core_max_temp_c': '°C', 'core_average_temp_c': '°C', 'distance_to_tjmax_min_c': '°C', 'clock_avg_ghz': 'GHz', 'clock_max_ghz': 'GHz', 'bus_clock_mhz': 'MHz', 'package_power_w': 'W', 'total_load_percent': '%', 'core_voltage_v': 'V'}, snapshot_ts)
    for gpu in gpus:
    ...
    cpu_temp_field = 'package_temp_c' if (cpu.get('_certified_metrics') or {}).get('package_temp_c', {}).get('last_real_value') is not None else 'summary_temp_c'
    cpu_temp_meta = _meta_from_nested(cpu, cpu_temp_field)
    cpu_clock_meta = _meta_from_nested(cpu, 'clock_avg_ghz')
    cpu_clock_value = (cpu.get('_certified_metrics') or {}).get('clock_avg_ghz', {}).get('last_real_value')
    if cpu_clock_value is None:
        raw_clock_meta = metrics.get('cpu_ghz') if isinstance(metrics.get('cpu_ghz'), dict) else {}
        cpu_clock_value = s.get('cpu_ghz')
        cpu_clock_meta = {
            'source': raw_clock_meta.get('source') or 'psutil.cpu_freq / Windows',
            'sensor': raw_clock_meta.get('sensor'),
            'sensor_timestamp': raw_clock_meta.get('timestamp') or snapshot_ts,
        }
