...
class BenchmarkPanel:
    ...
    def __init__(self, app, host):
        ...
        back = ctk.CTkButton(
            header, text='←', command=lambda: show_dashboard(self.app),
            width=42, height=38, corner_radius=10, fg_color='transparent',
            hover_color=theme_color('#102840'), border_width=1, border_color=BORDER,
            text_color=TEXT, font=(FONT, 18, 'bold'),
        )
        back.pack(side='left', pady=(4, 0), padx=(0, 12))

        titles = ctk.CTkFrame(header, fg_color='transparent')
        titles.pack(side='left', fill='x', expand=True, padx=(0, 14))
        ...
        self._content.pack(fill='both', expand=True, padx=16, pady=(0, 10))
    ...
    def _build_loading_state(self, parent, title, detail):
        shell = ctk.CTkFrame(parent, fg_color=CARD, border_width=1, border_color=BORDER, corner_radius=12, height=132)
        shell.pack(fill='x', padx=8, pady=(8, 0))
        shell.pack_propagate(False)
        ctk.CTkLabel(shell, text=title, font=(FONT, 15, 'bold'), text_color=TEXT, anchor='w').pack(fill='x', padx=16, pady=(14, 3))
        ctk.CTkLabel(shell, text=detail, font=(FONT, BENCH_BODY), text_color=MUTED, anchor='w').pack(fill='x', padx=16, pady=(0, 10))
        bar = ctk.CTkProgressBar(
            shell, height=6, corner_radius=999, progress_color=ACCENT,
    ...
    def _ensure_run_panel(self):
        ...
        # Conservamos el shell visible mientras se construye el árbol pesado.
        # Así el primer clic siempre tiene respuesta visual inmediata.
        panel = HealthCenterPanel(
            self.app, self._run_host, performance_only=True, benchmark_only=True
        )
