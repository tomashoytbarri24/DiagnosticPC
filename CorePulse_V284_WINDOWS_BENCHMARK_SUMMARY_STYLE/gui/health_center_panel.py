...
class HealthCenterPanel:
    ...
    def _render_gaming_benchmark_section(self):
        ...
        descriptions = {
            'gpu': ('GPU', 'G', 'DirectX 11: Valle / Bosque / Lago / Extreme · 49 s medidos + preparación.', BENCH_ACCENT),
            'cpu': ('CPU', 'C', 'Rendimiento SHA-256: 1 hilo + multinúcleo.', CYAN),
            'ram': ('RAM', 'R', 'Copia continua de bloques grandes de memoria.', GREEN),
            'ssd': ('SSD', 'S', 'Escritura + flush + lectura secuencial de archivo temporal.', AMBER),
        }
        self._benchmark_component_vars = {}
        self._benchmark_component_checks = {}
        for index, key in enumerate(('gpu', 'cpu', 'ram', 'ssd')):
            row_idx, col = divmod(index, 2)
            grid.grid_columnconfigure(col, weight=1, uniform='benchmark_components')
            title, icon_text, description, accent = descriptions[key]
            cell = ctk.CTkFrame(grid, fg_color=BENCH_SURFACE_SOFT, border_width=1, border_color=BENCH_BORDER, corner_radius=12)
            cell.grid(row=row_idx, column=col, sticky='nsew', padx=4, pady=4)

            shell = ctk.CTkFrame(cell, fg_color='transparent')
            shell.pack(fill='both', expand=True, padx=(0, 0), pady=(0, 0))
            ctk.CTkFrame(shell, width=4, fg_color=accent, corner_radius=12).pack(side='left', fill='y', padx=(1, 0), pady=1)
            body = ctk.CTkFrame(shell, fg_color='transparent')
            body.pack(side='left', fill='both', expand=True, padx=(10, 10), pady=(10, 10))

            top = ctk.CTkFrame(body, fg_color='transparent')
            top.pack(fill='x')
            icon_box = ctk.CTkLabel(
                top, text=icon_text, width=34, height=34, corner_radius=10,
                fg_color=theme_color('#0d2b45') if accent == BENCH_ACCENT else theme_color('#0d332b') if accent == GREEN else theme_color('#152750') if accent == CYAN else theme_color('#33260d'),
                text_color=accent, font=(FONT, 13, 'bold')
            )
            ...
            ctk.CTkLabel(heading, text='Componente', font=(FONT, 8, 'bold'), text_color=accent, anchor='w').pack(anchor='w')
            ctk.CTkLabel(heading, text=title, font=(FONT, BENCH_FONT_SECTION, 'bold'), text_color=TEXT, anchor='w').pack(anchor='w', pady=(2, 0))

            var = ctk.BooleanVar(value=bool(self._benchmark_component_flags.get(key, True)))
            self._benchmark_component_vars[key] = var
            check = ctk.CTkCheckBox(
                top, text='', variable=var, width=24, height=24, checkbox_width=22, checkbox_height=22,
                corner_radius=6, border_width=2, command=lambda k=key, v=var: self._on_benchmark_component_change(k, v),
                fg_color=accent, hover_color=accent, border_color=BENCH_ACCENT_BORDER, checkmark_color=BG
            )
            check.pack(side='right', padx=(8, 0))
            self._benchmark_component_checks[key] = check
            if 'visual_benchmark' in self._jobs:
                try: check.configure(state='disabled')
                except Exception: pass

            ctk.CTkLabel(body, text=description, font=(FONT, BENCH_FONT_META), text_color=MUTED, anchor='w', justify='left', wraplength=430).pack(fill='x', pady=(8, 0))
    ...
    def _render_visual_benchmark_card(self, parent):
        ...
        # V279: antes de iniciar no se reserva una tarjeta vacía de estado.
        # La superficie aparece recién cuando hay progreso real o un resultado.
    ...
    def _run_visual_benchmark(self):
        ...
        # La tarjeta de estado estuvo oculta mientras el benchmark estaba inactivo.
        # Se publica exactamente al comenzar la ejecución, sin dejar hueco previo.
        try:
            status_shell = getattr(self, '_visual_benchmark_card_shell', None)
            if status_shell is not None and status_shell.winfo_exists() and not status_shell.winfo_ismapped():
                status_shell.pack(fill='x', padx=12, pady=(0, 10))
                self.scroll._schedule_geometry(0)
        except Exception:
            pass
    ...
    def _render_analyzer(self, title, data, kind):
        ...
        if kind == 'startup':
            ...
            self._kv(card, 'Incidencias de Windows', data.get('device_problems', 0), RED if data.get('device_problems') else GREEN)
            self._kv(card, 'Sin firma reconocida', data.get('unsigned', 0), RED if data.get('unsigned') else GREEN)
            self._kv(card, 'Antiguos de terceros (>5 años) · comprobar versión', data.get('older_than_5y', 0), AMBER if data.get('older_than_5y') else TEXT)
            ctk.CTkLabel(
                card,
            ...
            # Gestión de actualizaciones recuperada sin sustituir el diseño de
            # inventario de V277. El hub se integra como bloque funcional adicional.
    ...
    def _render_driver_update_center(self, parent):
        """Driver Hub V259: búsqueda directa por ID de hardware, sin WUA."""
        box = ctk.CTkFrame(
            parent, fg_color=BENCH_SURFACE_SOFT, border_width=1,
            border_color=ACTION_BORDER, corner_radius=11,
        )
        box.pack(fill='x', padx=14, pady=(7, 10))
        ...
        ctk.CTkLabel(
            title, text='ACTUALIZACIONES DE CONTROLADORES', font=(FONT, 10, 'bold'),
            text_color=CYAN, anchor='w'
        ).pack(fill='x')
        ctk.CTkLabel(
            title,
            text='Busca por ID de hardware en el catálogo directo, descarga el paquete firmado y lo instala con PnPUtil. No usa Windows Update Agent.',
            font=(FONT, 8), text_color=TEXT2, anchor='w', justify='left', wraplength=760,
        ).pack(fill='x', pady=(2, 0))

        scanning = 'driver_updates_scan' in self._jobs
        scan = self._button(
            head, 'Buscando…' if scanning else 'Buscar actualizaciones',
            self._scan_driver_updates, variant='primary', width=142, height=31,
        )
        scan.pack(side='right', padx=(10, 0))
        if scanning:
            try: scan.configure(state='disabled')
            except Exception: pass

        result = self._driver_updates
        if result is None:
            ctk.CTkLabel(
                box,
                text='Pulsa “Buscar actualizaciones”. CorePulse prioriza incidencias, paquetes antiguos de terceros y hardware físico importante; sólo muestra una actualización cuando puede confirmar una versión superior compatible.',
                font=(FONT, 9), text_color=MUTED, anchor='w', justify='left', wraplength=1040,
            ).pack(fill='x', padx=12, pady=(4, 11))
            return

        error = str(result.get('error') or '').strip() if isinstance(result, dict) else ''
        if error:
            ctk.CTkLabel(
                box, text='No se pudo consultar el catálogo: ' + error,
                font=(FONT, 9), text_color=AMBER, anchor='w', justify='left', wraplength=1040,
            ).pack(fill='x', padx=12, pady=(4, 11))
            return

        items = list(result.get('items') or []) if isinstance(result, dict) else []
        checked = int(result.get('checked_devices') or 0) if isinstance(result, dict) else 0
        count = len(items)

        status_row = ctk.CTkFrame(box, fg_color='transparent')
        status_row.pack(fill='x', padx=12, pady=(2, 7))
        ctk.CTkLabel(
            status_row,
            text=(f'{count} actualización' + ('es' if count != 1 else '') + f' encontrada' + ('s' if count != 1 else '') + f' · {checked} dispositivos principales revisados'
                  if count else f'0 actualizaciones confirmadas · {checked} dispositivos relevantes comprobados'),
            font=(FONT, 9, 'bold'), text_color=CYAN if count else GREEN, anchor='w'
        ).pack(side='left', fill='x', expand=True)

        bulk_busy = 'driver_updates_download_all' in self._jobs or 'driver_updates_install_all' in self._jobs
        if items:
            installable = [x for x in items if bool(x.get('bulk_install_allowed', True))]
            downloaded_all = all(bool(x.get('is_downloaded')) for x in items)
            btn_install_all = self._button(
                status_row, 'Instalando…' if 'driver_updates_install_all' in self._jobs else 'Instalar todo',
                self._install_all_driver_updates, variant='primary', width=116, height=29
            )
            btn_install_all.pack(side='right', padx=(6, 0))
            btn_download_all = self._button(
                status_row, 'Descargando…' if 'driver_updates_download_all' in self._jobs else 'Descargar todo',
                self._download_all_driver_updates, variant='secondary', width=124, height=29
            )
            btn_download_all.pack(side='right', padx=(6, 0))
            if bulk_busy or not installable:
                try: btn_install_all.configure(state='disabled')
                except Exception: pass
            if bulk_busy or downloaded_all:
                try: btn_download_all.configure(state='disabled')
                except Exception: pass
        ...
        for item in items:
            ...
            firmware = str(item.get('driver_class') or '').upper() == 'FIRMWARE'

            row = ctk.CTkFrame(box, fg_color=CARD, border_width=1, border_color=BORDER, corner_radius=9, height=76)
            row.pack(fill='x', padx=10, pady=(0, 6))
            row.pack_propagate(False)
            row.grid_columnconfigure(0, weight=1)

            info = ctk.CTkFrame(row, fg_color='transparent')
            info.grid(row=0, column=0, sticky='nsew', padx=(11, 8), pady=8)
            device_name = str(item.get('device_name') or item.get('title') or 'Controlador')
            ctk.CTkLabel(
                info, text=device_name, font=(FONT, 10, 'bold'), text_color=TEXT,
            ...
            meta = f'{current}  →  {available}'
            if provider:
                meta += f'   ·   {provider}'
            if size != 'Tamaño N/A':
                meta += f'   ·   {size}'
            ctk.CTkLabel(
                info, text=meta, font=(FONT, 8), text_color=CYAN if available != 'N/A' else MUTED,
                anchor='w', justify='left', wraplength=720,
            ).pack(fill='x', pady=(3, 0))

            state_text = 'Firmware · individual' if firmware else ('Descargado' if downloaded else 'Disponible')
            state_color = AMBER if firmware else (GREEN if downloaded else CYAN)
            chip = ctk.CTkFrame(row, fg_color='transparent', border_width=1, border_color=state_color, corner_radius=999)
            chip.grid(row=0, column=1, sticky='e', padx=(0, 8), pady=8)
            ctk.CTkLabel(chip, text=state_text, font=(FONT, 8, 'bold'), text_color=state_color).pack(padx=8, pady=3)

            if downloaded:
                action_text = 'Instalando…' if busy else 'Instalar'
                command = lambda uid=update_id, rev=revision, title=device_name, fw=firmware: self._install_driver_update(uid, rev, title, firmware=fw)
                variant = 'primary'
            else:
                action_text = 'Descargando…' if busy else 'Descargar'
                command = lambda uid=update_id, rev=revision, title=device_name: self._download_driver_update(uid, rev, title)
                variant = 'secondary'
            btn = self._button(row, action_text, command, variant=variant, width=108, height=30)
            btn.grid(row=0, column=2, sticky='e', padx=(0, 10), pady=8)
            if busy or bulk_busy:
                try: btn.configure(state='disabled')
                except Exception: pass

    def _scan_driver_updates(self):
    ...
    def _download_driver_update(self, update_id, revision, title=''):
        job = 'driver_update_' + re.sub(r'[^a-zA-Z0-9]', '', str(update_id))[-12:] + '_' + str(int(revision or 0))
        if job in self._jobs:
            return
        def work():
            try:
                return download_driver_update(update_id, revision)
            except Exception as exc:
    ...
    def _install_driver_update(self, update_id, revision, title='', *, firmware=False):
        job = 'driver_update_' + re.sub(r'[^a-zA-Z0-9]', '', str(update_id))[-12:] + '_' + str(int(revision or 0))
        if job in self._jobs:
            return
        warning = '\n\nEste paquete es firmware. Se excluye de “Instalar todo” y puede requerir reinicio.' if firmware else ''
        try:
            accepted = messagebox.askyesno(
                'Instalar controlador',
                f'¿Instalar “{title or "este controlador"}”?\n\nCorePulse usará el paquete firmado descargado y PnPUtil; Windows validará compatibilidad antes de aplicarlo.{warning}',
                parent=self.frame,
            )
        except Exception:
            accepted = False
        ...
        def work():
            try:
                return install_driver_update(update_id, revision)
            except Exception as exc:
                return {'ok': False, 'error': str(exc)}
        def done(result, error):
            payload = result if isinstance(result, dict) else {'ok': False, 'error': error}
            if payload.get('ok'):
                ...
                try: self.app.after(900, self._scan_driver_updates)
                except Exception: pass
            else:
    ...
    def _install_all_driver_updates(self):
        ...
        def done(result, error):
            ...
            try: self.app.after(1200, self._scan_driver_updates)
            except Exception: pass
