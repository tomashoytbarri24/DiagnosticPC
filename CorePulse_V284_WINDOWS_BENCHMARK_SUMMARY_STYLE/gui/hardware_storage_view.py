...
def _style_storage(app, disks_data=None):
    ...
    for idx, widgets in getattr(app, 'disk_widgets', {}).items():
        ...
        if isinstance(mounts, str) and mounts.strip().upper() in {'N/A', 'NA', 'NONE', 'UNKNOWN'}:
            mounts = ''
        ...
        _cfg(exact, text='   |   '.join(parts) if parts else 'Información de capacidad no disponible', font=(FONT, 9), text_color=storage_accent, anchor='w')
        ...
        if health is not None:
            ...
            fallback = str(health_status or '').strip()
            if fallback and fallback.casefold() not in {'n/a', 'na', 'unknown'}:
                _cfg(badge, text=fallback.title(), font=(FONT, 10, 'bold'), text_color=MUTED)
            else:
                _cfg(badge, text='Salud no disponible', font=(FONT, 10, 'bold'), text_color=MUTED)
