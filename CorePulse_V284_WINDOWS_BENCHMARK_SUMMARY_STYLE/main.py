...
class App(ctk.CTk):
    ...
    def __init__(self):
        ...
        # V282: generación monotónica para que los gráficos no redibujen cinco
        # veces la misma muestra de telemetría. La adquisición sigue a 1 Hz.
        self._telemetry_history_generation = 0
        self._chart_last_history_generation = -1
        self._agent_ui_signature = None
    ...
    def _paint_sidebar_motion_layers(self, boundary_x, strength=1.0):
        """Estela visual corta en el borde; no cambia la geometría del dashboard."""
    ...
    def _run_sidebar_motion_effect(self, start_x, end_x, duration_ms=130):
        """Anima sólo una estela visual entre los bordes; el layout ya está en su estado final."""
        try:
    ...
    def set_sidebar_collapsed(self, collapsed, *, animated=True):
        """Oculta o muestra el sidebar, con botón ☰ persistente para reabrirlo."""
        ...
        try:
            ...
            pass
        try:
            from gui.dashboard import _rebuild_sidebar, _sync_sidebar_visibility_controls
            if not self._sidebar_collapsed:
                _rebuild_sidebar(self)
            _sync_sidebar_visibility_controls(self)
        except Exception:
            pass
        try:
            from gui.dashboard_layout import request_stable_layout_sync
            request_stable_layout_sync(self, force=True)
        except Exception:
            pass
        try:
            self.after(10, self._position_sidebar_toggle)
        except Exception:
            pass

    def toggle_sidebar_collapse(self):
        """Alterna visibilidad del sidebar lateral."""
        self.set_sidebar_collapsed(not bool(getattr(self, '_sidebar_collapsed', False)), animated=False)
    ...
    def _schedule_storage_health_refresh(self):
        ...
        def publish(result):
            ...
            for key, value in result.items():
                ...
                keep_previous_health = value.get('health') is None and previous.get('health') is not None
                health_fields = {
    ...
    def update_charts_fast(self):
        """Actualiza las curvas sólo cuando existe una muestra nueva y el Dashboard es visible.

        V282 elimina redibujos redundantes: la telemetría llega ~1 Hz, mientras
        que la UI antes intentaba blit 5 veces por segundo incluso con los mismos
        datos o cuando otra página interna estaba abierta.
        """
        if not self.is_running:
            return
        ...
        next_delay = 250
        try:
            dashboard_visible = getattr(self, '_active_internal_page', None) in (None, '', 'dashboard')
            if not dashboard_visible:
                next_delay = 500
                return

            generation = int(getattr(self, '_telemetry_history_generation', 0) or 0)
            last_generation = int(getattr(self, '_chart_last_history_generation', -1) or -1)

            # Si Matplotlib perdió su background (resize/theme), se permite el
            # draw aunque no haya una muestra nueva para reconstruirlo.
            if self.background is None and (not self.is_resizing):
                self.canvas.draw_idle()
            elif generation != last_generation and (not self.is_resizing):
                with self.telemetry_lock:
                ...
                self.ax_gpu.draw_artist(self.line_gpu)
                self.canvas.blit(self.fig.bbox)
                self._chart_last_history_generation = generation
        except Exception:
            self.background = None
        finally:
            if self.is_running:
                self.chart_after_id = self.after(next_delay, self.update_charts_fast)
            ...
            while self.is_running:
                ...
                try:
                    ...
                    with self.telemetry_lock:
                        ...
                        self._telemetry_history_generation = int(getattr(self, '_telemetry_history_generation', 0) or 0) + 1
    ...
    def _refresh_alert_history_ui(self):
        ...
        if not hasattr(self, 'alert_history_store'):
            return
        # V282: no tocar disco/SQLite si la página de historial no está visible.
        panel = getattr(self, 'alert_history_panel', None)
        if panel is None:
            return
        try:
    ...
    def _update_agent_ui(self):
        ...
        try:
            ...
            overall = str(state.get('overall', 'UNKNOWN') or 'UNKNOWN')
            game = state.get('game') or {}
            game_name = str(game.get('name') or '').replace('\\', '/').split('/')[-1]

            # V282: la tarjeta, el título de tray y el explicador de alertas sólo
            # se actualizan si cambió el estado real del agente. El heartbeat del
            # agente sigue funcionando con su cadencia original.
            ...
            signature = (
                mode, overall, game_name,
                repr(alerts.get('active') or []),
                repr(findings),
            )
            changed = signature != getattr(self, '_agent_ui_signature', None)
            if changed:
                self._agent_ui_signature = signature
                self.tray_service.notify_state()
                self._refresh_agent_status_panel(state)
                self._refresh_smart_alert_ui(state)

            # El historial sólo se consulta si su panel está realmente visible.
