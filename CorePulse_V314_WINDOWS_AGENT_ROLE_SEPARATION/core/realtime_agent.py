...
logger = logging.getLogger('CorePulse.Agent')
...
class RealTimeAgent:
    ...
    def _rtss_game(self):
        try:
            ...
            logger.debug('[GAME] RTSS no entregó una aplicación válida', exc_info=True)
        return None
    ...
    def _loop(self):
        while self.running:
            try:
                ...
                logger.exception('[ERROR][AGENT] Falló un ciclo del agente en tiempo real')
                with self.lock:
                    self.latest_state = {'mode': 'UNKNOWN', 'context': 'UNKNOWN', 'game_detected': False, 'game_foreground': False, 'game': None, 'overall': 'ERROR', 'instant': {'severity': 'UNKNOWN', 'status': 'NO EVALUABLE', 'reasons': [], 'source': 'REALTIME_AGENT_CURRENT_SAMPLE', 'synthetic': False, 'estimated': False}, 'observation': None, 'alerts': {'active': [], 'history': [], 'active_count': 0, 'history_count': 0}, 'findings': [{'component': 'AGENT', 'level': 'ERROR', 'title': 'El agente encontró un error interno', 'detail': 'CorePulse mantuvo la aplicación activa. El detalle técnico fue registrado en los logs.', 'evidence': [], 'context': 'UNKNOWN', 'active': True}], 'timestamp': time.time()}
    ...
    def stop(self):
        ...
        try:
            ...
            logger.debug('[AGENT] El hilo ya estaba detenido o no respondió al join', exc_info=True)
        try:
            self.rtss.close()
        except Exception:
            logger.debug('[AGENT] RTSS ya estaba cerrado o no pudo cerrarse limpiamente', exc_info=True)
