...
def _save_ai_snapshot(ai: Dict[str, Any]) -> None:
    ...
    try:
        path = data_path('current_ai_report.json')
...
def generate_pdf_report(telemetry, disks, score, output_path, diagnostic_result=None, *args, **kwargs):
    ...
    complete = diagnostic_result.get('complete_diagnostic') if isinstance(diagnostic_result.get('complete_diagnostic'), dict) else {}
    frozen_ai = complete.get('ai_analysis') if isinstance(complete.get('ai_analysis'), dict) else diagnostic_result.get('_ai_analysis') if isinstance(diagnostic_result.get('_ai_analysis'), dict) else None
    # El PDF reutiliza exactamente la interpretación IA congelada al finalizar el diagnóstico.
    # Sólo vuelve a llamar al proveedor si el resultado proviene de una versión antigua sin esa fase.
    ai = frozen_ai if isinstance(frozen_ai, dict) and frozen_ai else analyze_report_with_ai(diagnostic_result, clean_telemetry, clean_disks)
