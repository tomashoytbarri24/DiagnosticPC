...
def _build_header(app):
    ...
    # V227: reemplaza el duplicado genérico por el Estado del agente real.
    right = ctk.CTkFrame(hero, fg_color='transparent', width=350)
    right.pack(side='right', fill='y', padx=(8, 4), pady=(3, 3))
    ...
    title_row = ctk.CTkFrame(agent, fg_color='transparent', height=18)
    title_row.grid(row=0, column=0, sticky='ew', padx=11, pady=(7, 1))
    ...
    badge = ctk.CTkLabel(
        title_row, text='Tiempo real', height=16,
        font=(FONT, 7, 'bold'), text_color=COLORS['primary'],
        fg_color=role_color('accent_2'), corner_radius=8, padx=7, pady=1
    )
    badge.grid(row=0, column=1, sticky='e')

    status_row = ctk.CTkFrame(agent, fg_color='transparent', height=18)
    status_row.grid(row=1, column=0, sticky='ew', padx=11, pady=(1, 0))
    dot = ctk.CTkLabel(status_row, text='●', width=12, height=16, font=(FONT, 9, 'bold'), text_color=COLORS['green'])
    dot.pack(side='left', padx=(0, 5))
    status = ctk.CTkLabel(status_row, text='INICIALIZANDO', height=16, font=(FONT, 9, 'bold'), text_color=COLORS['primary'], anchor='w')
    status.pack(side='left')
    mode = ctk.CTkLabel(status_row, text='Escritorio', height=16, font=(FONT, 7, 'bold'), text_color=COLORS['muted'], anchor='e')
    mode.pack(side='right')

    state_row = ctk.CTkFrame(agent, fg_color='transparent', height=17)
    state_row.grid(row=2, column=0, sticky='ew', padx=11, pady=(1, 0))
    state_label = ctk.CTkLabel(state_row, text='Evaluando evidencia', height=16, font=(FONT, 8, 'bold'), text_color=COLORS['primary'], anchor='w')
    state_label.pack(side='left', fill='x', expand=True)
    app._header_last_update = ctk.CTkLabel(
        state_row, text='Actualizado: esperando', height=15, font=(FONT, 7),
        text_color=COLORS['muted'], anchor='e'
    )
    app._header_last_update.pack(side='right', padx=(6, 0))

    detail = ctk.CTkLabel(
        agent, text='Acumulando evidencia del sistema…', height=14,
        font=(FONT, 7), text_color=COLORS['muted'], anchor='w', justify='left',
        wraplength=325
    )
    detail.grid(row=3, column=0, sticky='ew', padx=11, pady=(1, 6))
