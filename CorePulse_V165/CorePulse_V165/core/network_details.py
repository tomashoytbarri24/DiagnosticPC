...
class NetworkTrafficSampler:
    ...
    def sample(self, interface_name=None):
        ...
        if current is not None and self.previous is not None and self.previous_ts is not None:
            ...
            elapsed = sample_clock - self.previous_ts
            if old is not None and elapsed > 0.05:
                recv_delta = int(current.bytes_recv) - int(old.bytes_recv)
                sent_delta = int(current.bytes_sent) - int(old.bytes_sent)
                # Si Windows reinicia un contador no hay una tasa verificable
                # para ese intervalo; conservar N/A y renovar la base.
                if recv_delta >= 0:
                    out['download_bps'] = recv_delta / elapsed
                if sent_delta >= 0:
                    out['upload_bps'] = sent_delta / elapsed
        self.previous = counters
        self.previous_ts = sample_clock
        return out

    def reset(self):
        """La próxima lectura establece una base nueva, sin promediar una pausa."""
        self.previous = None
