...
class ManagedPowerPlans:
    ...
    def _ensure(self, mode, protected, *, require_owned=False):
        ...
        if not cleanup['success']:
            return cleanup
        aliases_present = sorted(
            guid for guid, row in schemes.items()
            if guid not in WINDOWS_MODE_SCHEMES.values()
            and row.get('name', '').casefold() in COREPULSE_PROFILE_ALIASES[mode]
        )
        return {'success': True, 'guid': chosen, 'mode': mode, 'name': schemes[chosen]['name'],
                'created': created, 'reused': not created, 'builtin': False, 'corepulse_copy': True,
                'managed_persistent': self.owned_record(record),
                # Contrato explícito para la UI/tests: un modo nunca necesita una
                # copia nueva si ya existe una candidata CorePulse reutilizable.
                'duplicate_guard': True,
                'same_mode_aliases_detected': aliases_present,
                'cleanup': cleanup}
