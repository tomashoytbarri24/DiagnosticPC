...
from core.before_after import start_operation, finish_operation, mark_operation
BG_MAIN = theme_color('#0b0f19')
BG_CARD = theme_color('#151c2c')
BG_CARD_2 = theme_color('#111827')
...
class CleaningCenterPanel(ctk.CTkFrame):
    ...
    def __init__(self, app):
        ...
        self.storage_scan = None
        self._closed = False
        self._before_after_sessions = {}
    ...
    def _build_header(self):
        ...
        ctk.CTkButton(header, text='← Volver al Dashboard', width=155, height=32, corner_radius=7, fg_color=theme_color('#26354d'), hover_color=theme_color('#334867'), command=self.close).grid(row=0, column=1, padx=14, pady=12)
    ...
    def _build_cards(self):
        ...
        self.storage_card = CleaningToolCard(self.body, 'Liberar Almacenamiento', '▱', GREEN, 'Escanea las unidades locales y muestra sólo archivos que CorePulse puede eliminar de forma segura.', 'Liberar espacio', self.clean_storage, analyze_command=self.analyze_storage, footer='Solo muestra archivos eliminables.')
    ...
    def close(self):
        # Navegar atrás no destruye el panel: la caché de navegación lo oculta y
        # permite volver sin reconstruir todas las tarjetas.
        try:
    ...
    def _start_measurement(self, key, action, metadata=None):
        try:
            telemetry = dict(getattr(self.app, 'latest_telemetry', {}) or {})
            disks = list(getattr(self.app, 'latest_disks', []) or [])
            battery = getattr(self.app, 'battery_health_cache', None)
            self._before_after_sessions[str(key)] = start_operation(
                action, telemetry, disks, battery,
                metadata=metadata or {},
            )
        except Exception:
            self._before_after_sessions[str(key)] = None

    def _finish_measurement_later(self, key, metadata=None, delay=1.6):
        session = self._before_after_sessions.pop(str(key), None)
        if not session:
            return
        def worker():
            try:
                ...
                disks = list(getattr(self.app, 'latest_disks', []) or [])
                battery = getattr(self.app, 'battery_health_cache', None)
                finish_operation(session, telemetry, disks, battery, metadata=metadata or {})
            except Exception:
                try:
                    mark_operation(session, status='measurement_failed', note='La acción terminó, pero no se pudo obtener la medición posterior.')
                except Exception:
                    pass
        threading.Thread(target=worker, name='CorePulseCleaningBeforeAfter', daemon=True).start()

    def _fail_measurement(self, key, exc=None):
        session = self._before_after_sessions.pop(str(key), None)
        if not session:
            return
        try:
            mark_operation(session, status='failed', note=str(exc or 'La acción no pudo completarse.'))
        except Exception:
            pass
    ...
    def optimize_ram(self):
        if not messagebox.askyesno('CorePulse - RAM segura', 'No se cerrarán aplicaciones ni se modificarán working sets ajenos.\n\n¿Continuar?', parent=self.app):
            return
        self._start_measurement('ram_safe', 'Limpieza RAM segura')
        self.ram_card.set_busy(True, 'Midiendo y optimizando memoria...')
        self._thread(lambda: optimize_ram_safely(settle_seconds=0.35, snapshot_samples=3, run_registered_cache_clearers=True), self._ram_done, lambda exc: self._ram_fail(exc, 'ram_safe'))
    ...
    def _ram_done(self, result):
        ...
        self._finish_measurement_later('ram_safe', {'recovered_mb': recovered, 'mode': 'safe'}, delay=1.2)

    def _ram_fail(self, exc, key=None):
        self.ram_card.set_busy(False)
        if key:
            self._fail_measurement(key, exc)
    ...
    def optimize_ram_deep(self):
        if not is_administrator():
            ...
            return
        if not messagebox.askyesno('CorePulse - RAM profunda', 'Esta acción recorta working sets y solicita a Windows liberar caché del sistema, páginas modificadas y memoria standby.\nPuede provocar recargas o micro-stutter temporal en aplicaciones abiertas.\n\nCorePulse medirá el resultado real antes/después.\n\n¿Continuar?', parent=self.app):
            return
        self._start_measurement('ram_deep', 'Limpieza RAM profunda')
        self.ram_card.set_busy(True, 'Ejecutando liberación profunda de Windows...')
        self._thread(lambda: optimize_ram_deep(settle_seconds=1.15, snapshot_samples=5, purge_standby=True), self._ram_deep_done, lambda exc: self._ram_fail(exc, 'ram_deep'))
    ...
    def _ram_deep_done(self, result):
        ...
        if not result.get('success'):
            ...
            self._fail_measurement('ram_deep', result.get('message'))
        ...
        self.ram_card.set_result(f'{recovered / 1024.0:.2f} GB', f"Antes: {float(before.get('used_percent') or 0):.1f}% • Después: {float(after.get('used_percent') or 0):.1f}%\nRegiones liberadas: {regions_done}/{regions_total} • Procesos recortados: {int(result.get('working_sets_trimmed') or 0)}", color=GREEN, title='LIBERACIÓN PROFUNDA MEDIDA')
        self.lbl_last_action.configure(text=f"RAM profunda: {recovered:.1f} MB • {time.strftime('%H:%M:%S')}")
        self._finish_measurement_later('ram_deep', {'recovered_mb': recovered, 'mode': 'deep'}, delay=1.5)
    ...
    def clean_cache(self):
        if not messagebox.askyesno('CorePulse - Limpiar Caché', 'Se borrarán solo archivos TEMP permitidos. Los bloqueados se omitirán.\n\n¿Continuar?', parent=self.app):
            return
        self._start_measurement('cache', 'Limpieza de caché')
        self.cache_card.set_busy(True, 'Eliminando temporales permitidos...')
        self._thread(_delete_allowed_temp_files, self._cache_clean_done, lambda exc: (self._fail_measurement('cache', exc), self._cache_fail(exc)))
    ...
    def _cache_clean_done(self, result):
        ...
        self._finish_measurement_later('cache', {'deleted_bytes': int(freed), 'deleted_files': int(result.get('deleted_files') or 0)}, delay=1.2)
    ...
    def analyze_storage(self):
        self.storage_card.set_busy(True, 'Escaneando unidades y ubicaciones eliminables...')
        self._thread(scan_safe_storage_cleanup, self._storage_scan_done, self._storage_scan_fail)

    def _storage_scan_done(self, result):
        self.storage_scan = result
        self.storage_card.set_busy(False)
        total = int(result.get('total_bytes') or 0)
        files = int(result.get('total_files') or 0)
        units = int(result.get('units_scanned') or 0)
        categories = [row for row in (result.get('categories') or []) if int(row.get('bytes') or 0) > 0]
        if total <= 0:
            self.storage_card.set_result('Nada que limpiar', f'{units} unidad(es) analizadas • No se encontraron archivos eliminables.', color=GREEN, title='ANÁLISIS COMPLETADO')
            return
        names = ' • '.join(str(row.get('label') or '') for row in categories[:3])
        extra = max(0, len(categories) - 3)
        if extra:
            names += f' • +{extra} más'
        detail = f'{units} unidad(es) • {files:,} archivos'
        if names:
            detail += f'\n{names}'
        self.storage_card.set_result(_fmt_bytes(total), detail, color=GREEN, title='PUEDES LIBERAR')

    def _storage_scan_fail(self, exc):
        self.storage_card.set_busy(False)
        messagebox.showerror('CorePulse - Almacenamiento', str(exc), parent=self.app)

    def clean_storage(self):
        result = self.storage_scan
        if not result:
            messagebox.showinfo('CorePulse - Almacenamiento', 'Primero pulsa Analizar para escanear las unidades.', parent=self.app)
            return
        total = int(result.get('total_bytes') or 0)
        if total <= 0:
            messagebox.showinfo('CorePulse - Almacenamiento', 'No hay archivos eliminables en el último análisis.', parent=self.app)
            return
        categories = [row for row in (result.get('categories') or []) if int(row.get('bytes') or 0) > 0]
        lines = [f"• {row.get('label')}: {_fmt_bytes(row.get('bytes') or 0)}" for row in categories]
        prompt = (
            f"CorePulse puede eliminar {_fmt_bytes(total)} sin tocar archivos personales ni ubicaciones desconocidas.\n\n"
            + '\n'.join(lines)
            + '\n\nLos archivos que cambiaron, estén en uso o dejen de cumplir la regla se omitirán.\n\n¿Liberar espacio ahora?'
        )
        if not messagebox.askyesno('CorePulse - Liberar almacenamiento', prompt, parent=self.app):
            return
        self._start_measurement('storage', 'Limpieza de almacenamiento', {'planned_bytes': int(total)})
        self.storage_card.set_busy(True, 'Eliminando únicamente lo incluido en el análisis...')
        self._thread(lambda: delete_scanned_candidates(result), self._storage_clean_done, lambda exc: (self._fail_measurement('storage', exc), self._storage_scan_fail(exc)))

    def _storage_clean_done(self, result):
        self.storage_scan = result.get('after') or None
        self.storage_card.set_busy(False)
        deleted = int(result.get('deleted_bytes') or 0)
        files = int(result.get('deleted_files') or 0)
        skipped = int(result.get('skipped') or 0)
        measured = result.get('free_space_delta_bytes')
        detail = f'{files:,} archivos eliminados • {skipped:,} omitidos'
        if measured is not None:
            detail += f'\nEspacio libre medido: {"+" if int(measured) >= 0 else ""}{_fmt_bytes(int(measured))}'
        self.storage_card.set_result(_fmt_bytes(deleted), detail, color=GREEN, title='ELIMINADO')
        self.lbl_last_action.configure(text=f"Almacenamiento: {_fmt_bytes(deleted)} • {time.strftime('%H:%M:%S')}")
        self._finish_measurement_later('storage', {'deleted_bytes': deleted, 'deleted_files': files, 'free_space_delta_bytes': measured}, delay=1.4)
