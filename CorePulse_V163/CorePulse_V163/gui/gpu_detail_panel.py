...
class GPUDetailPanel:
    ...
    def _build(self):
        ...
        hero = build_title_block(
            titles,
            eyebrow='Gráficos · Vista multi-GPU',
            title='Detalles avanzados de GPU',
            subtitle='Inventario de Windows + sensores reales en tiempo real',
            accent=PURPLE,
            badges=(('Multi-GPU', PURPLE), ('Sensores reales', GREEN), ('REAL_OR_NA', AMBER)),
            title_size=19,
        )
    ...
    def _update_selector(self, gpus):
        ...
        if signature == self._selector_signature:
            for i, button in enumerate(self._selector_buttons):
                try:
                    active = i == selected
                    configure_changed(button, fg_color=theme_color('#173550') if active else theme_color('#0d1828'), border_color=theme_color('#2b668f') if active else BORDER, text_color=CYAN if active else TEXT_2)
                except Exception:
                    pass
            return
        ...
        for i, gpu in enumerate(gpus):
            ...
            button = ctk.CTkButton(
                ...
                fg_color=theme_color('#173550') if active else theme_color('#0d1828'), hover_color=theme_color('#164f7d'),
    ...
    def _update_sensor_rows(self, sensors):
        ...
        for sensor in valid:
            ...
            configure_changed(type_lbl, text=TYPE_LABELS.get(kind, kind.upper() or 'SENSOR'), text_color=TYPE_COLORS.get(kind, CYAN))
            configure_changed(name_lbl, text=str(sensor.get('name') or 'Sensor'))
            configure_changed(value_lbl, text=_display_sensor_value(sensor), text_color=_sensor_value_color(sensor))
            configure_changed(source_lbl, text=str(sensor.get('source') or 'N/A').replace('LibreHardwareMonitorLib', 'LibreHardwareMonitor'))
            configure_changed(age_lbl, text=_age_text(sensor.get('timestamp')))
        active = set(keys)
        ...
        if not valid and not self.lbl_no_sensors.winfo_manager():
            self.lbl_no_sensors.pack(fill='x', padx=8, pady=12)
        configure_changed(self.lbl_sensor_status, text=f'{len(valid)} sensores válidos' if valid else 'Sin sensores disponibles', text_color=GREEN if valid else MUTED)

    def _apply_static_rows(self, gpu, all_gpus):
        os_inv = gpu.get('os_inventory') if isinstance(gpu.get('os_inventory'), dict) else {}
        configure_changed(self._spec_labels['vendor'], text=str(os_inv.get('vendor') or 'N/A'))
        configure_changed(self._spec_labels['video_processor'], text=str(os_inv.get('video_processor') or 'N/A'))
        configure_changed(self._spec_labels['driver'], text=str(os_inv.get('driver_version') or 'N/A'))
        configure_changed(self._spec_labels['windows_status'], text=windows_status_text(os_inv.get('os_status')))
        ...
        configure_changed(self._spec_labels['wmi_vram'], text=wmi_vram_text, text_color=wmi_vram_color)
        ...
        if width is not None and height is not None:
            ...
            resolution = 'N/A'
        configure_changed(self._spec_labels['resolution'], text=resolution, text_color=MUTED if display_elsewhere and width is None and not mode else TEXT)
        ...
        if refresh is not None:
            ...
            refresh_text = 'N/A'
        configure_changed(self._spec_labels['refresh_rate'], text=refresh_text, text_color=MUTED if refresh is None else TEXT)
        configure_changed(self._spec_labels['hardware_type'], text=human_gpu_hardware_type(gpu.get('hardware_type')))
        pnp = str(os_inv.get('pnp_device_id') or 'N/A')
        configure_changed(self._spec_labels['pnp_id'], text=pnp if len(pnp) <= 58 else pnp[:55] + '…')
        ...
        configure_changed(self._spec_labels['inventory_source'], text=source_text)
        configure_changed(self.lbl_identity_source, text=f'Inventario GPU: {source_text}', text_color=GREEN if (sources or gpu.get('source')) else MUTED)
    ...
    def set_active(self, active):
        """Pausa refrescos periódicos cuando la ficha queda cacheada y oculta."""
        active = bool(active)
        ...
        if self._alive:
            try:
                self._after_id = self.frame.after(1, self.refresh)
            except Exception:
                self._after_id = None
    ...
    def refresh(self):
        ...
        try:
            ...
            configure_changed(self.lbl_freshness, text=f'Actualización: {_age_text(snapshot_stamp)}')

            if self._selected_index is None or not (0 <= self._selected_index < len(gpus)):
                configure_changed(self.lbl_title, text='Detalles avanzados de GPU')
                configure_changed(self.usage_value, text='N/A', text_color=MUTED)
                configure_changed(self.temp_value, text='N/A', text_color=MUTED)
                configure_changed(self.vram_value, text='N/A', text_color=MUTED)
                configure_changed(self.power_value, text='N/A', text_color=MUTED)
                return

            gpu = gpus[self._selected_index]
            self._selected_name = gpu.get('name')
            configure_changed(self.lbl_title, text=str(gpu.get('name') or f'GPU {self._selected_index}'))
            ...
            configure_changed(self.usage_value, text=_fmt(usage, 1, '%'), text_color=PURPLE if _num(usage) is not None else MUTED)
            configure_changed(self.usage_detail, text='GPU Core · LibreHardwareMonitor' if _num(usage) is not None else 'Sensor de carga no disponible')
            configure_changed(self.temp_value, text=_fmt(temp, 1, ' °C'), text_color=_thermal_color(temp))
            configure_changed(self.temp_detail, text='GPU Core · LibreHardwareMonitor' if _num(temp) is not None else 'Sensor térmico no disponible')
            if used_gb is not None and total_gb is not None:
                ...
                vram_text = 'N/A'
            configure_changed(self.vram_value, text=vram_text, text_color=CYAN if total_gb is not None else MUTED)
            configure_changed(self.vram_detail, text=('Total certificado por LibreHardwareMonitor' if total_source == 'LibreHardwareMonitor' else 'Total de inventario según Windows (WMI)') if total_gb is not None else 'VRAM no disponible')
            configure_changed(self.power_value, text=_fmt(power, 2, ' W'), text_color=PURPLE if _num(power) is not None else MUTED)
            configure_changed(self.power_detail, text='GPU Package · sensor real' if _num(power) is not None else 'Sensor de potencia no disponible')

            if not self._is_scrolling():
                self._apply_static_rows(gpu, gpus)
                configure_changed(self._advanced_labels['hotspot'], text=_fmt(gpu.get('hotspot_c'), 1, ' °C'), text_color=_thermal_color(gpu.get('hotspot_c')))
                configure_changed(self._advanced_labels['core_clock'], text=_fmt(gpu.get('core_clock_mhz'), 1, ' MHz'))
                configure_changed(self._advanced_labels['memory_clock'], text=_fmt(gpu.get('memory_clock_mhz'), 1, ' MHz'))
                configure_changed(self._advanced_labels['memory_load'], text=_fmt(gpu.get('memory_usage_percent'), 1, '%'))
                configure_changed(self._advanced_labels['vram_used'], text=_fmt(used_gb, 2, ' GB'))
                configure_changed(self._advanced_labels['vram_total'], text=_fmt(_gb_from_mb(total_mb), 2, ' GB'))
                configure_changed(self._advanced_labels['fan_rpm'], text=_fmt(gpu.get('fan_rpm'), 0, ' RPM'))
                configure_changed(self._advanced_labels['fan_control'], text=_fmt(gpu.get('fan_control_percent'), 1, '%'))
                configure_changed(self._advanced_labels['voltage'], text=_fmt(gpu.get('core_voltage_v'), 4, ' V'))
                configure_changed(self._advanced_labels['power'], text=_fmt(power, 2, ' W'))
                ...
                configure_changed(self._advanced_labels['sensor_count'], text=_fmt_int(sensor_count))
                provider = str(gpu.get('source') or 'N/A').replace('LibreHardwareMonitorLib', 'LibreHardwareMonitor')
                configure_changed(self._advanced_labels['provider'], text=provider)
