...
from gui.professional_visuals import build_title_block, build_empty_state
...
class SmartAlertPanel:
    ...
    def __init__(self, parent):
        ...
        hero = build_title_block(
            left,
            eyebrow='Supervisión del agente',
            title='Alertas y estado del agente',
            subtitle='Condiciones instantáneas y sostenidas explicadas con evidencia real del agente.',
            accent=COLORS['INFO'],
            badges=(('Instantáneo', COLORS['OBSERVING']), ('Sostenido', COLORS['WARNING']), ('Evidencia real', COLORS['NORMAL'])),
            title_size=20,
        )
        hero['frame'].pack(anchor='w', fill='x')
        self.header_status = ctk.CTkLabel(header, text='NORMAL', font=('Segoe UI', 9, 'bold'), text_color=COLORS['NORMAL'], fg_color=theme_color('#102235'), corner_radius=8, padx=10, pady=5)
    ...
    def _render_empty_state(self, *, icon, title, description, accent):
        if self.scroll is None:
            return
        ...
        state['frame'].pack(fill='x', padx=8, pady=12)
    ...
    def render(self, state, diagnostic):
        ...
        try:
            signature = repr((
                (state or {}).get('overall'),
                (state or {}).get('context'),
                (diagnostic or {}).get('instant'),
                (diagnostic or {}).get('explanations'),
            ))
        except Exception:
            signature = None
        if signature is not None and signature == self._last_render_signature:
            return
        ...
        if host is not None and host.is_scrolling():
            ...
            host.defer_until_idle(lambda: self.render(state_copy, diagnostic_copy), key='alerts-render')
            return
        self._clear()
        ...
        if not items:
            ...
            if instant_level in {'ELEVATED', 'WARNING', 'CRITICAL', 'ERROR'}:
                ...
                self._render_empty_state(icon='◷', title='Aún no hay una alerta sostenida', description='La lectura actual está en observación. CorePulse seguirá acumulando evidencia antes de declarar una alerta real.', accent=instant_color)
                ...
                self._render_empty_state(icon='…', title='Todavía no hay un diagnóstico activo', description='El agente está observando la sesión y aún no existe evidencia suficiente para explicar una condición.', accent=COLORS['OBSERVING'])
            else:
                ...
                self.hero_evidence.configure(text=f'Contexto: {context}')
                self._render_empty_state(icon='✓', title='Todo tranquilo por ahora', description='No hay alertas sostenidas activas. CorePulse continuará supervisando el equipo en tiempo real.', accent=COLORS['NORMAL'])
            return
        primary = items[0]
