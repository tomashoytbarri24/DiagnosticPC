...
class AlertHistoryPanel:
    ...
    def __init__(self, parent):
        ...
        hero = build_title_block(
            left,
            eyebrow='Registro de la sesión',
            title='Historial de alertas',
            subtitle='Máximo 10 alertas de esta ejecución. Al reiniciar CorePulse, el historial comienza vacío.',
            accent=LEVEL_COLORS['INFO'],
            badges=(('Activas', LEVEL_COLORS['INFO']), ('Críticas', LEVEL_COLORS['CRITICAL']), ('Resueltas', LEVEL_COLORS['NORMAL'])),
            title_size=20,
        )
        hero['frame'].pack(anchor='w', fill='x')
        self.lbl_summary = ctk.CTkLabel(header, text='Sin eventos', font=('Segoe UI', 9, 'bold'), text_color=DIM, fg_color=theme_color('#102235'), corner_radius=8, padx=10, pady=5)
    ...
    def _sync_empty_state(self, has_rows):
        if has_rows:
        ...
        if self._empty_state is None:
            state = build_empty_state(
                self.scroll,
                icon='◷',
                title='Aún no hay alertas registradas',
                description='Cuando CorePulse confirme una alerta sostenida, aparecerá aquí con su nivel, evidencia y hora de la sesión.',
                accent=LEVEL_COLORS['INFO'],
                compact=False,
            )
            self._empty_state = state['frame']
            self._empty_state.pack(fill='x', padx=6, pady=12)
    ...
    def render(self, rows, summary=None):
        if getattr(self, 'scroll_host', None) is not None and self.scroll_host.is_scrolling():
            ...
            self.scroll_host.defer_until_idle(lambda: self.render(rows_copy, summary_copy), key='history-render')
            return False
        ...
        for key in desired:
            try:
                ...
                pass
        self._sync_empty_state(bool(desired))
        ...
        self.lbl_summary.configure(text='Sin eventos' if not rows else f'{len(rows)} eventos registrados')
