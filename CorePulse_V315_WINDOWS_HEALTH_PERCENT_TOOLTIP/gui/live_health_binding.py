...
def _render(app, telemetry, disks):
    ...
    if isinstance(result.get('score'), (int, float)):
        if not reasons or (len(reasons) == 1 and reasons[0].startswith('No se detectan señales')):
            summary = f"{result['status']} · sin revisión necesaria"
        elif len(reasons) == 1:
            summary = f"{result['status']} · 1 factor influyendo"
        else:
            summary = f"{result['status']} · {len(reasons)} factores influyendo"
    else:
        summary = 'Sin evidencia suficiente para evaluar la salud global'
