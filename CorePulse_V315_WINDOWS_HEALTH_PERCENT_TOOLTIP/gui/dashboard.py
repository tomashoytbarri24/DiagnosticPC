...
def _bind_hover_tooltip(root, text_provider, *, delay_ms=320):
    """Tooltip liviano para explicar una tarjeta sin abrir ventanas modales."""
    ...
    def position_tip(event=None):
        ...
        if tip is None:
            return
        try:
            x = (event.x_root + 16) if event is not None else (root.winfo_pointerx() + 16)
            y = (event.y_root + 16) if event is not None else (root.winfo_pointery() + 16)
    ...
    def show_tip(event=None):
        cancel_pending()
        destroy_tip()
        message = str(text_provider() or '').strip()
        if not message:
            return
        try:
            ...
            try:
                ...
                pass
            shell = ctk.CTkFrame(tip, fg_color=COLORS['surface'], border_width=1, border_color=COLORS['border'], corner_radius=10)
...
def _health_hover_text(app):
    ...
    if not isinstance(result, dict):
        return 'CorePulse aún está recopilando evidencia real para calcular la salud del sistema.'
    ...
    if isinstance(score, (int, float)):
        lines.append(f'Índice de salud: {float(score):.1f}% · {status}')
    else:
        lines.append(f'Índice de salud: N/A · {status}')
    if reasons:
        lines.append('Factores que están influyendo en el porcentaje:')
        for item in reasons[:6]:
            lines.append(f'• {item}')
    else:
        lines.append('No se detectan señales relevantes de RAM saturada, SSD lleno, desgaste crítico ni temperatura alta.')
    lines.append('100% significa que no hay señales relevantes para revisar en las métricas evaluadas.')
    return '\n'.join(lines)
...
def _build_system_band(app):
    ...
    try:
        health_card._cp_title_label.configure(text='ÍNDICE DE SALUD')
    except Exception:
        pass
    app._health_status.configure(text='--%', text_color=COLORS['green'], justify='left', anchor='w', wraplength=185)
