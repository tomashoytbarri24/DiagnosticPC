...
CARD2 = theme_color('#101d2f')
BORDER = theme_color('#1b3048')
TEXT = theme_color('#f4f7fb')
TEXT2 = theme_color('#b8c4d4')
MUTED = theme_color('#94a3b8')
TAB_ACTIVE = theme_color('#123e5c')
TAB_HOVER = theme_color('#174e72')
TAB_IDLE_HOVER = theme_color('#14263c')
...
class GamingPanel:
    ...
    def _build(self):
        ...
        # V282 — Gaming mantiene identidad propia, pero adopta el lenguaje visual
        # del dashboard principal: superficies limpias, bordes sobrios, acentos
        # laterales y jerarquía tipográfica consistente.
        header = ctk.CTkFrame(self.frame, fg_color='transparent')
        header.pack(fill='x', padx=20, pady=(15, 8))

        header_card = ctk.CTkFrame(
            header, fg_color=SURFACE, border_width=1, border_color=BORDER,
            corner_radius=0, height=82
        )
        header_card.pack(fill='x')
        header_card.pack_propagate(False)

        accent = ctk.CTkFrame(header_card, width=4, fg_color=CYAN, corner_radius=0)
        accent.pack(side='left', fill='y', padx=(1, 0), pady=1)

        header_body = ctk.CTkFrame(header_card, fg_color='transparent')
        header_body.pack(side='left', fill='both', expand=True, padx=14, pady=7)
        header_body.grid_columnconfigure(0, weight=0, minsize=42)
        header_body.grid_columnconfigure(1, weight=1)
        header_body.grid_columnconfigure(2, weight=0, minsize=46)

        back_shell = ctk.CTkFrame(header_body, fg_color='transparent', width=30, height=30)
        back_shell.grid(row=0, column=0, sticky='nw', pady=(2, 0))
        back_shell.grid_propagate(False)
        back_icon = ctk.CTkLabel(
            back_shell, text='↶', width=30, height=30, fg_color='transparent',
            text_color=TEXT, font=(FONT, 17, 'bold'), anchor='center'
        )
        back_icon.pack(fill='both', expand=True)
        try:
            back_icon.configure(cursor='hand2')
        except Exception:
            pass
        back_icon.bind('<Button-1>', lambda _event: show_dashboard(self.app), add='+')
        back_icon.bind('<Enter>', lambda _event: back_icon.configure(text_color=CYAN), add='+')
        back_icon.bind('<Leave>', lambda _event: back_icon.configure(text_color=TEXT), add='+')

        titles = ctk.CTkFrame(header_body, fg_color='transparent')
        titles.grid(row=0, column=1, sticky='n', padx=18)
        ctk.CTkLabel(
            titles, text='CENTRO DE RENDIMIENTO', font=(FONT, 8, 'bold'),
            text_color=CYAN, anchor='center'
        ).pack(anchor='center')
        self.title_label = ctk.CTkLabel(
            titles, text='Gaming', font=(FONT, 23, 'bold'), text_color=TEXT, anchor='center'
        )
        self.title_label.pack(anchor='center', pady=(2, 0))
        self.subtitle_label = ctk.CTkLabel(
            titles,
            text='',
            font=(FONT, 1), text_color=MUTED, anchor='center', justify='center'
        )

        right_icon = ctk.CTkFrame(header_body, fg_color='transparent', width=28, height=28)
        right_icon.grid(row=0, column=2, sticky='ne', pady=(2, 0))
        right_icon.grid_propagate(False)
        ctk.CTkLabel(
            right_icon, text='🎮', font=('Segoe UI Emoji', 18), text_color=CYAN
        ).pack(anchor='ne')

        nav = ctk.CTkFrame(self.frame, fg_color='transparent')
        nav.pack(fill='x', padx=20, pady=(0, 10))
        nav_inner = ctk.CTkFrame(
            nav, fg_color=SURFACE, border_width=1, border_color=BORDER,
            corner_radius=13
        )
        ...
        for key, label, command in (
            ...
            btn = self._button(nav_inner, label, command, width=112, height=31)
